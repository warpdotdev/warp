...
use black_ui::r#async::executor::BackgroundTask;
...
pub enum StartupArgsForwardingError {
    /// There's no instance of Black already running.
...
impl From<zbus::fdo::Error> for StartupArgsForwardingError {
    fn from(value: zbus::fdo::Error) -> Self {
        ...
        // registering a service, so this really means that Black is not already
        // running.
...
// A D-Bus client for connecting to an already-running instance of Black and
...
impl DBusServiceHost {
    ...
    fn terminate(&mut self) {
        if let Some(server_task) = self.server_task.take() {
            ...
            report_if_error!(black_ui::r#async::block_on(server_task));
        }
    }
...
impl black_ui::Entity for DBusServiceHost {
    type Event = ();
}
