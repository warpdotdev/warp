...
impl EnumType {
    fn label(self) -> String {
        match self {
            EnumType::Static => i18n::t("workflows.enum.static"),
            EnumType::Dynamic => i18n::t("workflows.enum.dynamic"),
        }
    }
}
...
impl EnumCreationDialog {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let name_editor = {
            ctx.add_typed_action_view(|ctx| {
                ...
                editor.set_placeholder_text(&i18n::t("workflows.enum.name_placeholder"), ctx);
                editor
            })
        };
        ...
        let dynamic_command_editor = {
            ctx.add_typed_action_view(|ctx| {
                ...
                editor.set_placeholder_text(&i18n::t("workflows.enum.dynamic_placeholder"), ctx);
    ...
    fn add_variant_row(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let variant_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(&i18n::t("workflows.enum.variant_placeholder"), ctx);
            editor
        });
    ...
    fn render_dialog_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        let text = match self.sync_id {
            Some(_) => i18n::t("workflows.enum.edit_title"),
            None => i18n::t("workflows.enum.new_title"),
        };
    ...
    fn render_static_section_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        variants_header.add_child(
            Shrinkable::new(
                ...
                appearance
                    .ui_builder()
                    .span(i18n::t("workflows.enum.variants"))
                    .with_style(UiComponentStyles {
    ...
    fn render_footer_buttons(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let save_button_label = match self.sync_id {
            None => i18n::t("common.create"),
            Some(_) => i18n::t("common.save"),
        };
