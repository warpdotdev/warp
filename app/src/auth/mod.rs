            ...
            || num_unsaved_files > 0)
    {
        ...
        if num_long_running_commands > 0 {
            let key = if num_long_running_commands > 1 {
                "auth.logout.running_processes"
            } else {
                "auth.logout.running_process"
            };
            info_text_vec
                .push(i18n::t(key).replace("{count}", &num_long_running_commands.to_string()));

            button_data.push(ModalButton::for_app(
                i18n::t("auth.logout.show_running_processes"),
                move |ctx| {
        ...
        if num_shared_sessions > 0 {
            let key = if num_shared_sessions > 1 {
                "auth.logout.shared_sessions"
            } else {
                "auth.logout.shared_session"
            };
            info_text_vec.push(i18n::t(key).replace("{count}", &num_shared_sessions.to_string()));
        }

        if num_unsaved_objects > 0 {
            let key = if num_unsaved_objects > 1 {
                "auth.logout.unsynced_objects"
            } else {
                "auth.logout.unsynced_object"
            };
            info_text_vec.push(i18n::t(key).replace("{count}", &num_unsaved_objects.to_string()));
        }

        if num_unsaved_files > 0 {
            let key = if num_unsaved_files > 1 {
                "auth.logout.unsaved_files"
            } else {
                "auth.logout.unsaved_file"
            };
            info_text_vec.push(i18n::t(key).replace("{count}", &num_unsaved_files.to_string()));
        }

        button_data.push(ModalButton::for_app(i18n::t("common.cancel"), move |ctx| {
            send_telemetry_sync_from_app_ctx!(
