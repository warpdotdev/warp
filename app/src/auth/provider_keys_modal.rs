//! "Add API key" modal shown from the onboarding "Choose how to access AI" slide
//! (and reusable elsewhere). Collects the user's own OpenAI / Anthropic / Google
//! provider keys. Lives in the app crate (not the onboarding crate) because it
//! reuses `EditorView` for the text inputs and persists through `ApiKeyManager`,
//! neither of which the onboarding crate depends on.
...
pub enum ProviderKeysModalEvent {
    Cancelled,
    /// The user clicked "Add keys". Each field is `Some` only when non-empty.
    Save {
...
impl ProviderKeysModalView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let openai_input = Self::make_key_editor("sk-...", ctx);
        let anthropic_input = Self::make_key_editor("sk-ant-...", ctx);
        let google_input = Self::make_key_editor("AIzaSy...", ctx);
    ...
    fn make_key_editor(placeholder: &str, ctx: &mut ViewContext<Self>) -> ViewHandle<EditorView> {
