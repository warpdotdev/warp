...
fn publish_skill_creates_symlink() {
    ...
    let target = publish_skill(skill_root.path(), "github", &skill_dir).unwrap();
...
fn publish_skill_uses_the_real_skill_name() {
    ...
    publish_skill(skill_root.path(), "linear", &skill_dir).unwrap();
...
fn publish_skill_overrides_an_existing_symlink() {
    ...
    let published = publish_skill(skill_root.path(), "github", &new_skill_dir).unwrap();
...
#[test]
fn publish_skill_moves_a_conflicting_real_directory_aside_instead_of_deleting_it() {
    let source_root = TempDir::new().unwrap();
    let skill_root = TempDir::new().unwrap();
    let skill_dir = write_skill(source_root.path(), "github");
    let target = skill_root.path().join("github");
    ...
    let backup = skill_root.path().join("github.pre-publish-backup");
...
#[test]
fn publish_skill_numbers_the_backup_when_one_already_exists() {
    let source_root = TempDir::new().unwrap();
    let skill_root = TempDir::new().unwrap();
    let skill_dir = write_skill(source_root.path(), "github");
    let target = skill_root.path().join("github");
    ...
    fs::create_dir_all(skill_root.path().join("github.pre-publish-backup")).unwrap();

    publish_skill(skill_root.path(), "github", &skill_dir).unwrap();

    assert!(skill_root.path().join("github.pre-publish-backup").is_dir());
    assert!(
        skill_root
            .path()
            .join("github.pre-publish-backup-2")
            .is_dir()
    );
}
...
fn publish_skill_dirs_prefers_most_specific_directory_on_name_collision() {
    ...
    let published = publish_skill_dirs(skill_root.path(), &[specific_dir, general_dir]);
...
#[test]
fn publish_skill_dirs_overrides_an_existing_environment_skill_and_preserves_it() {
    let root = TempDir::new().unwrap();
    ...
    let published = publish_skill_dirs(skill_root.path(), &[source_dir]);
...
fn publish_skill_dirs_is_a_noop_for_empty_source_dirs() {
    ...
    assert_eq!(publish_skill_dirs(&skill_root, &[]), 0);
...
fn publish_skill_dirs_recovers_from_missing_source_directory() {
    ...
    let published = publish_skill_dirs(skill_root.path(), &[missing_dir, present_dir]);
