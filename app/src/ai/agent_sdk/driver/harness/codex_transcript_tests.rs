...
// --- rehydrate_codex_transcript_from_reader ---

#[test]
#[serial_test::serial]
fn rehydrate_codex_transcript_from_reader_does_not_patch_session_meta_cwd() {
    let tmp = TempDir::new().unwrap();
    let prev = std::env::var(CODEX_HOME_ENV).ok();
    std::env::set_var(CODEX_HOME_ENV, tmp.path());

    let uuid = Uuid::new_v4();
    let remote_cwd = "/remote/cloud/workspace";
    let timestamp = "2026-04-30T01:54:20.000Z";
    let meta_line = session_meta_line(uuid, remote_cwd, timestamp, "0.55.0");
    let entries: Vec<serde_json::Value> = vec![
        serde_json::from_str(&meta_line).unwrap(),
        serde_json::json!({"type": "event_msg", "payload": {"x": 1}}),
    ];
    let envelope = CodexTranscriptEnvelope {
        cwd: remote_cwd.into(),
    ...
    let result = rehydrate_codex_transcript_from_reader(json.as_slice()).unwrap();
    ...
    // Resume command references the original session id.
    assert!(
        result.command.contains(&uuid.to_string()),
        "expected session id in command: {}",
        result.command
    );

    // Transcript was written to disk and is findable.
    let sessions_root = tmp.path().join(CODEX_SESSIONS_SUBDIR);
    assert!(
        find_session_file(&sessions_root, uuid).is_some(),
        "session file not found"
    );

    // The session_meta entry's cwd was NOT patched to a local path.
    let on_disk = read_jsonl(&result.transcript_path).unwrap();
    let meta_payload = on_disk[0]["payload"].clone();
    assert_eq!(
        meta_payload["cwd"],
        serde_json::Value::String(remote_cwd.to_string()),
        "session_meta cwd should not be patched"
    );
}
