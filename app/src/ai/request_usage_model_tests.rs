...
use ai::api_keys::{
    ...
    CustomEndpointDefinitions, CustomEndpointId, CustomEndpointModel, CustomEndpointSchema,
    GrokTokens,
};
use chrono::Duration;
...
#[test]
fn test_out_of_credits_refined_by_shared_custom_endpoint() {
    App::test((), |mut app| async move {
        let (_uid, mut workspace) = create_test_workspace();
        ...
        let endpoint_id = CustomEndpointId::generated();
            ...
            .insert(
                ...
                CustomEndpointDefinition {
                    ...
                    models: vec![CustomEndpointModel {
                        ...
                        alias: None,
                        config_key: "test-model-config-key".to_string(),
                    }],
                },
            )
            .unwrap();
        ApiKeyManager::handle(&app).update(&mut app, |manager, ctx| {
            ...
            manager
                .persist_custom_endpoint_key(
                    endpoint_id,
                    Some("test-endpoint-key".to_string()),
                    ctx,
                )
                .unwrap();
            assert!(
                manager.keys().custom_endpoints.is_empty(),
                "the shared endpoint should not be present in legacy storage",
            );
            assert_eq!(manager.custom_endpoints().len(), 1);
        });
        ...
        request_usage_model.update(&mut app, |model, ctx| {
            ...
            assert!(
                model.has_any_ai_remaining(&TeamlessScopeForTest, ctx),
                "out of credits with a shared custom endpoint should permit AI",
            );
        });
    });
}
