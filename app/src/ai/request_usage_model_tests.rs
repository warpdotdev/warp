...
mod tui_usage_snapshot_tests {
    ...
    fn payg_entry(
        credits_used: i32,
        attributed_team_uid: Option<String>,
        ...
        BillingCycleUsageEntry {
            ...
            credits_used,
            cost_cents: credits_used,
            attributed_team_uid,
        }
    }

    fn billing_cycle_usage(entries: Vec<BillingCycleUsageEntry>) -> BillingCycleUsageData {
        let now = Utc::now();
    ...
    fn two_workspaces_with_owner(
        ...
        let user_uid = crate::auth::UserUid::new(crate::auth::user::TEST_USER_UID);
        let member = TeamMember {
            uid: user_uid,
            email: crate::auth::user::TEST_USER_EMAIL.to_owned(),
            role: MembershipRole::Owner,
        };
        let mut current = Workspace::from_local_cache(
            WorkspaceUid::from(crate::server::ids::ServerId::from(1_i64)),
            "Current Workspace".to_owned(),
            Some(vec![Team::from_local_cache(
                crate::server::ids::ServerId::from(1_i64),
                "Current Team".to_owned(),
                None,
                None,
                Some(vec![member.clone()]),
            )]),
        );
        current.settings.usage_based_pricing_settings.enabled = true;
        current_extra(&mut current);
        let mut other = Workspace::from_local_cache(
            WorkspaceUid::from(crate::server::ids::ServerId::from(2_i64)),
        ...
        other.settings.usage_based_pricing_settings.enabled = true;
        other_extra(&mut other);
        (current, other)
    }

    #[test]
    fn addon_credits_exclude_grants_scoped_to_a_different_workspace() {
        App::test((), |mut app| async move {
            ...
            request_usage_model.update(&mut app, |model, _ctx| {
                ...
                model.bonus_grants = vec![
                    make(BonusGrantScope::Workspace(current_uid), 100),
                    make(BonusGrantScope::Workspace(other_uid), 900),
                ];
            });
            ...
            app.read(|ctx| {
                ...
                let addon = snapshot
                    .addon_credits
                    .expect("add-on section should render for the current workspace's grant");
                assert_eq!(
                    addon.limit, 100,
    ...
    #[test]
    fn pay_as_you_go_excludes_spend_attributed_to_a_different_team() {
        App::test((), |mut app| async move {
            let (mut current, _other) = two_workspaces_with_owner(
                |workspace| {
                    workspace.billing_cycle_usage = Some(billing_cycle_usage(vec![
                        payg_entry(500, Some("test_uid00000000000001".to_owned())),
                        payg_entry(9000, Some("test_uid00000000000002".to_owned())),
                    ]));
                },
                |_| {},
            );
            ...
            current.teams.push(extra_team);
            add_user_workspaces_with_workspace(&mut app, current);
            ...
            app.read(|ctx| {
                ...
                let payg = snapshot
                    .pay_as_you_go
                    .expect("pay-as-you-go section should render");
                assert_eq!(
                    payg.credits_used, 500,
                    "only the first team's attributed spend should count"
                );
            });
        });
    }
    ...
    #[test]
    fn pay_as_you_go_omitted_when_visibility_only_exposes_aggregates() {
        App::test((), |mut app| async move {
            let (current, _other) = two_workspaces_with_owner(
                |workspace| {
                    workspace.billing_metadata.tier.usage_visibility_policy =
                        Some(UsageVisibilityPolicy {
                            admin_granularity: UsageVisibilityGranularity::TeamAggregate,
                            max_prior_cycles: Default::default(),
                        });
                    workspace.billing_cycle_usage = Some(billing_cycle_usage(vec![payg_entry(
                        12345,
                        Some("test_uid00000000000001".to_owned()),
                    )]));
                },
                |_| {},
            );
            ...
            app.read(|ctx| {
                let snapshot = compute_tui_usage_snapshot(ctx);
                assert!(
                    snapshot.pay_as_you_go.is_none(),
                    "a TeamAggregate-only visibility can't back a pay-as-you-go-specific figure, so it must be omitted rather than misreported"
                );
            });
        });
    }
