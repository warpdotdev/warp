...
fn kimi_models_show_the_kimi_logo() {
    // Kimi ships through Fireworks, which the server reports as `Unknown`, so
    // these rows would otherwise land on the agent glyph.
    for id in [
        "kimi-k25-fireworks",
        "kimi-k26-fireworks",
        ...
        assert_eq!(
            model_leading_icon(&llm, ModelIconFlags::default()),
            Icon::KimiLogo,
            "{id} should show the Kimi logo"
        );
    }

    // The display name alone is enough when the id doesn't carry the brand.
    let mut llm = server_llm("moonshotai/kimi-k2-instruct", None);
    llm.display_name = "Kimi K2.5".to_string();
...
#[test]
fn other_fireworks_hosted_models_keep_the_agent_glyph() {
    // The Kimi match is anchored to a leading `kimi` token precisely so the
    // other Fireworks-hosted families don't inherit its branding.
    for id in [
        "glm-46-fireworks",
        "minimax-m2-fireworks",
        ...
        assert_eq!(
            model_leading_icon(&llm, ModelIconFlags::default()),
            Icon::Agent,
            "{id} should keep the agent glyph"
        );
    }
}

#[test]
fn kimi_models_still_yield_to_auto_and_host_badges() {
