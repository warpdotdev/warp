...
pub fn init(app: &mut AppContext) {
    ...
    app.register_editable_bindings([EditableBinding::new(
        ...
        i18n::t("ai.requested_command.edit_requested_command"),
...
impl RequestedCommandView {
    ...
    fn toggle_accept_split_button_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.is_accept_split_button_menu_open {
            ...
            let accept_item =
                MenuItemFields::new_with_label(i18n::t("common.run"), accept_keystroke)
    ...
    fn maybe_render_footer(&self, app: &AppContext) -> Option<Box<dyn Element>> {
            ...
            || self.block_model.status(app).is_streaming()
        {
            ...
            .map(|citation| (i18n::t("ai.requested_command.copied_from"), citation))
        } else {
            ...
            .map(|citations| (i18n::t("ai.requested_command.derived_from"), citations))
        };
            ...
            (
                ...
                *shown.lock() = true;
                Some(render_autonomy_checkbox_setting_speedbump_footer(
                    i18n::t("ai.output.always_allow_oz_read_only_commands"),
                    *checked,
    ...
    fn render_profile_autoexecution_info_footer(
        ...
        Container::new(
            Flex::row()
                ...
                .with_child(
                    Text::new(
                        i18n::t("ai.requested_command.always_ask_permission"),
                        appearance.ui_font_family(),
                        font_size,
                    )
                    .with_color(blended_colors::text_sub(theme, theme.surface_1()))
                    ...
                    Expanded::new(
                        ...
                        Align::new(
                            appearance
                                .ui_builder()
                                .link(
                                    i18n::t("ai.requested_command.manage_command_execution_setting")
                                        .into(),
                                    None,
            ...
            Some(AIActionStatus::Blocked) => {
                title = match &self.action_type {
                    RequestedActionViewType::Command => {
                        i18n::t("ai.requested_command.ok_run_command").into()
                    }
                    RequestedActionViewType::McpTool => {
                        i18n::t("ai.requested_command.ok_call_mcp_tool").into()
                    }
                };
            }
                            ...
                            .and_then(|block| block.long_running_control_state())
                        {
                            match long_running_command_control_state {
                                LongRunningCommandControlState::Agent { is_blocked, .. } => {
                                    ...
                                    if is_errored {
                                        i18n::t("ai.requested_command.agent_error_take_over").into()
                                    } else if *is_blocked {
                                        i18n::t("ai.requested_command.agent_needs_input").into()
                                    } else {
                                        i18n::t("ai.requested_command.agent_monitoring_command")
                                            .into()
                                    }
                                }
                            ...
                            i18n::t("ai.requested_command.viewing_command_detail").into()
                        }
                    }
                    RequestedActionViewType::McpTool => {
                        i18n::t("ai.requested_command.viewing_mcp_tool_detail").into()
                    }
                };
            }
            None => {
                if self.block_model.status(app).is_streaming() {
                    title = i18n::t("ai.requested_command.generating_command").into();
                    ...
                    title = if title_str.trim().is_empty() {
                        i18n::t("ai.requested_command.generating_command").into()
                    } else {
                        title_str.into()
                    };
...
pub(crate) fn header_message_for_user_take_over_reason(reason: &UserTakeOverReason) -> String {
    match reason {
        UserTakeOverReason::Manual => i18n::t("ai.requested_command.user_in_control"),
        UserTakeOverReason::Stop => i18n::t("ai.requested_command.paused_agent_user_in_control"),
        UserTakeOverReason::TransferFromAgent { .. } => {
            i18n::t("ai.requested_command.user_in_control_short")
        }
    }
}
                    ...
                    .and_then(|status| status.finished_result().map(|result| &result.result))
            {
                ...
                let result_text = match result {
                    CallMCPToolResult::Success { result } => serde_json::to_string_pretty(result)
                        .unwrap_or_else(|_| i18n::t("ai.requested_command.error_formatting_json")),
                    CallMCPToolResult::Error(error) => {
                        i18n::t("ai.requested_command.error_prefix").replace("{error}", error)
                    }
                    CallMCPToolResult::Cancelled => {
                        i18n::t("ai.requested_command.tool_call_cancelled")
                    }
                };
                i18n::t("ai.requested_command.response_with_result")
                    .replace("{command}", command_text)
