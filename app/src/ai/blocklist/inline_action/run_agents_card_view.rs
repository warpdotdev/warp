...
impl RunAgentsCardView {
    ...
    fn refresh_accept_button_state(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        accept.set_tooltip(reason.or_else(|| Some(i18n::t("common.accept"))), ctx);
    ...
    fn toggle_accept_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.is_accept_menu_open {
            let item = MenuItemFields::new_with_label(
                i18n::t("ai.run_agents.accept_without_orchestration"),
                String::new(),
            )
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if self.block_model.is_restored() {
            return render_status_only_card(
                i18n::t("ai.run_agents.cancelled"),
                appearance,
        ...
        if !matches!(status, Some(AIActionStatus::Blocked)) {
            return render_status_only_card(
                i18n::t("ai.run_agents.configuring"),
                appearance,
                StatusKind::Spawning,
                app,
            );
...
fn render_header(handles: &RunAgentsCardHandles, app: &AppContext) -> Box<dyn Element> {
    let appearance = Appearance::as_ref(app);
    let mut config = HeaderConfig::new(i18n::t("ai.run_agents_card.title"), app)
...
fn render_agents_section(state: &RunAgentsEditState, app: &AppContext) -> Box<dyn Element> {
    ...
    let theme = appearance.theme();
    let label = Text::new(
        i18n::t("ai.run_agents.agents_count")
            .replace("{count}", &state.agent_run_configs.len().to_string()),
        appearance.ui_font_family(),
    ...
    match result {
        RunAgentsResult::Launched { agents, .. } => {
            ...
            let label = if launched == total {
                if total == 1 {
                    i18n::t("ai.run_agents.spawned_one")
                } else {
                    i18n::t("ai.run_agents.spawned_other").replace("{count}", &total.to_string())
                }
            } else {
                i18n::t("ai.run_agents.spawned_partial")
        ...
        RunAgentsResult::Denied { reason } => {
            let body = if reason.is_empty() {
                i18n::t("ai.run_agents.orchestration_disabled")
            } else {
                i18n::t("ai.run_agents.orchestration_disabled_reason").replace("{reason}", reason)
            };
            (body, StatusKind::Cancelled)
        }
        RunAgentsResult::Failure { error } => {
            let label = if error.is_empty() {
                i18n::t("ai.run_agents.failed_to_start_orchestration")
            } else {
                i18n::t("ai.run_agents.failed_to_start_orchestration_with_error")
                    .replace("{error}", error)
            };
            (label, StatusKind::Failure)
        }
        RunAgentsResult::Cancelled => (i18n::t("ai.run_agents.cancelled"), StatusKind::Cancelled),
    }
}
...
fn render_spawning_card(
    ...
    let label = if total == 1 {
        i18n::t("ai.run_agents.spawning_one")
    } else {
        i18n::t("ai.run_agents.spawning_other").replace("{count}", &total.to_string())
    };
