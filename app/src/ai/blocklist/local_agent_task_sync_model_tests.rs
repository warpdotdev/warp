...
fn cli_task_mapping_survives_cli_session_end() {
    App::test((), |mut app| async move {
        ...
        let mut mock = MockAIClient::new();
        mock.expect_update_agent_task()
            .returning(move |_, task_state, _, _, _, _, _, _| {
...
fn shared_session_link_uses_correct_argument_order() {
    App::test((), |mut app| async move {
        ...
        mock.expect_update_agent_task()
            .withf(
                move |arg_task_id, task_state, arg_session_id, conv_id, status_msg, _, _, _| {
                    *arg_task_id == task_id
                        && task_state.is_none()
...
fn preserve_terminal_setup_failure_conversation_reports_token_without_task_state() {
    App::test((), |mut app| async move {
        ...
        mock.expect_update_agent_task()
            .withf(
                move |arg_task_id, task_state, _, conv_id, status_msg, _, _, _| {
                    *arg_task_id == task_id
                        && task_state.is_none()
                        && conv_id.is_some()
                        && status_msg.is_none()
