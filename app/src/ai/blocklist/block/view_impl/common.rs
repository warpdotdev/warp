...
pub fn render_warping_indicator<V: View>(
    ...
    let message = if let Some(summarization_type) = summarization_type {
        ...
        let base_message = match summarization_type {
            SummarizationType::ConversationSummary => {
                i18n::t("ai.loading.summarizing_conversation")
            }
            SummarizationType::ToolCallResultSummary => {
                i18n::t("ai.loading.summarizing_command_output")
            }
        };
            ...
            .is_none_or(|output| output.get().messages.is_empty())
    {
        ...
        i18n::t("ai.loading.adjusting_tasks")
    } else {
        match props
            .action_model
        ...
        {
            Some(AIAgentActionType::SearchCodebase(..)) => i18n::t("ai.loading.searching_codebase"),
            Some(AIAgentActionType::Grep { .. }) => i18n::t("ai.loading.grepping"),
            Some(AIAgentActionType::CallMCPTool { name, .. }) => {
                i18n::t("ai.loading.calling_mcp_tool").replace("{name}", name)
            }
            Some(AIAgentActionType::ReadMCPResource { name, .. }) => {
                i18n::t("ai.loading.reading_mcp_resource").replace("{name}", name)
            }
            Some(AIAgentActionType::FileGlob { .. })
            | Some(AIAgentActionType::FileGlobV2 { .. }) => i18n::t("ai.loading.finding_files"),
            Some(AIAgentActionType::WriteToLongRunningShellCommand { .. }) => {
                i18n::t("ai.loading.writing_command_input")
            }
            action => {
                    ...
                    && active_block.agent_interaction_metadata().is_some()
                {
                    if action.is_none() {
                        ...
                        i18n::t("ai.common.agent_waiting_for_instructions")
                    } else {
                        ...
                        let base = match action {
                            Some(AIAgentActionType::ReadShellCommandOutput {
                                ...
                                ..
                            }) => i18n::t("ai.loading.waiting_for_command_exit"),
                            _ => i18n::t("ai.loading.executing_command"),
                        };
                        ...
                        if let Some(remaining) = next_check_remaining {
                            ...
                            let suffix = format!(
                                " · {}",
                                i18n::t("ai.loading.next_check_in").replace("{time}", &formatted)
                            );
    ...
    if let Some(take_over_button_props) = props.take_over_lrc_control_button {
        ...
        buttons_row.add_child(render_switch_control_to_user_button(
            ...
            i18n::t("ai.common.take_over_command_tooltip"),
...
pub fn format_elapsed_seconds(elapsed: std::time::Duration) -> String {
    ...
    if total_seconds == 1 {
        i18n::t("ai.common.elapsed_second_one")
    } else {
        i18n::t("ai.common.elapsed_second_other").replace("{seconds}", &total_seconds.to_string())
    }
}
...
fn render_hide_responses_button(
    ...
    let button_text = if should_hide_responses {
        ...
        i18n::t("ai.common.hide_responses")
    };
    ...
    let tooltip_text = if should_hide_responses {
        i18n::t("ai.common.show_agent_responses")
    } else {
        i18n::t("ai.common.hide_agent_responses")
    };
...
fn render_stop_button(props: ButtonProps, appearance: &Appearance) -> Box<dyn Element> {
    ...
    render_warping_indicator_button(
        ...
        stop_icon,
        props.keystroke,
        i18n::t("ai.common.stop_agent_task_tooltip"),
        props.is_active,
        false,
...
fn render_queue_next_prompt_button(
    ...
    let tooltip_text = if props.is_active {
        i18n::t("ai.common.auto_queue_on_tooltip")
    } else {
        i18n::t("ai.common.auto_queue_next_prompt_tooltip")
    };
...
fn render_auto_approve_button(
    ...
    let tooltip_text = if props.is_locked {
        i18n::t("ai.common.fast_forward_cloud_always_enabled")
    } else if is_active {
        i18n::t("ai.common.turn_off_auto_approve_actions")
    } else {
        i18n::t("ai.common.auto_approve_actions_for_task")
    };
...
fn render_force_refresh_inline(
    ...
    Hoverable::new(props.button_handle.clone(), move |state| {
        ...
        let text = Text::new(i18n::t("ai.output.check_now"), font_family, font_size)
            .with_color(color)
            .with_style(Properties::default())
            .with_clip(ClipConfig::end())
            .with_selectable(false)
        ...
        if state.is_hovered() {
            let tool_tip = ui_builder
                .tool_tip(i18n::t("ai.output.check_now_tooltip"))
                .build()
                .finish();
...
fn render_mermaid_diagram_section<A: Action>(
    ...
    render_visual_card(
        i18n::t("ai.common.mermaid_diagram"),
        Icon::Dataflow,
...
pub fn render_failed_output(props: FailedOutputProps, app: &AppContext) -> Box<dyn Element> {
    let appearance = Appearance::as_ref(app);
    let error_apology = i18n::t("ai.common.error_apology");
    ...
    let error_text = match props.error {
        RenderableAIError::QuotaLimit {
            ...
            if let Some(message) = user_display_message {
                ...
                format!(
                    "{}\n\n{}",
                    error_apology,
                    i18n::t("ai.common.credit_limit_reset")
                        .replace("{date}", &formatted_next_refresh_time)
                )
            }
        }
        RenderableAIError::ServerOverloaded => i18n::t("ai.common.server_overloaded"),
        RenderableAIError::InternalWarpError => {
            format!(
                "{}\n\n{}",
                error_apology,
                i18n::t("ai.common.internal_warp_error")
            )
        }
            ...
            if *will_attempt_resume {
                if *waiting_for_network {
                    format!(
                        "{}\n\n{}",
                        error_message,
                        i18n::t("ai.common.resume_when_network_restored")
                    )
                } else {
                    format!(
                        "{}\n\n{}",
                        error_message,
                        i18n::t("ai.common.attempting_resume_conversation")
                    )
                }
            } else {
                format!("{error_apology}\n\n{error_message}")
        ...
        RenderableAIError::AwsBedrockCredentialsExpiredOrInvalid { model_name } => {
            ...
            format!(
                "{}\n\n{}",
                error_apology,
                i18n::t("ai.common.aws_credentials_expired_or_missing")
...
fn render_invalid_api_key_error(
    ...
    let alert_text = Text::new(
        i18n::t("ai.settings.invalid_api_key"),
        appearance.ui_font_family(),
        14.,
    )
    ...
    let settings_button = appearance
        ...
        .with_text_label(i18n::t("ai.settings.edit_api_keys"))
...
pub(crate) fn render_debug_footer<V: View>(
    ...
    let submit_button = if !is_dogfood && !is_enterprise_user {
        ...
        Some(
            appearance
                ...
                .with_centered_text_label(i18n::t("common.send_feedback"))
    ...
    let debug_text = Text::new(
        i18n::t("ai.common.debug_information").replace("{debug_info}", &debug_info),
        appearance.ui_font_family(),
        appearance.monospace_font_size(),
    )
    .with_color(
    ...
    let copy_button_with_tooltip = appearance.ui_builder().tool_tip_on_element(
        i18n::t("ai.common.copy_debug_id"),
