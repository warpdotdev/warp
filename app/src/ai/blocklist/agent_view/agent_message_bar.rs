        ...
        match self.figma_button_status(app) {
            ...
            Some(FigmaMcpStatus::Installed) => {
                message.items.push(figma_chip(
                    ...
                    i18n::t("ai.agent.figma.enable_mcp"),
            ...
            Some(FigmaMcpStatus::Enabling) => {
                message.items.push(
                    figma_chip(
                        self.mouse_states.figma_enable_button.clone(),
                        i18n::t("ai.agent.figma.enabling"),
                        None,
                    )
...
impl MessageProvider<AgentMessageArgs<'_>> for BootstrappingMessageProducer {
    fn produce_message(&self, args: AgentMessageArgs<'_>) -> Option<Message> {
            ...
            || args.terminal_model.is_shared_ambient_agent_session()
        {
            ...
            Some(Message::from_text(i18n::t(
                "terminal.agent_message_bar.starting_shell",
            )))
        }
    }
}
        ...
        if show_resume {
            ...
            items.push(MessageItem::clickable(
                vec![
                    ...
                    MessageItem::text(i18n::t("terminal.message_bar.to_resume_conversation")),
                ],
                |ctx| {
                ...
                keybinding_name_to_keystroke(commands::CONVERSATIONS.name, app)
            {
                items.push(MessageItem::clickable(
                    vec![
                        ...
                        MessageItem::text(i18n::t("terminal.message_bar.open_conversation")),
                    ],
                    |ctx| {
            ...
            && *TabSettings::as_ref(app).show_code_review_button
        {
            ...
            items.push(MessageItem::clickable(
                vec![
                    ...
                    MessageItem::text(i18n::t("terminal.message_bar.for_code_review")),
                ],
                |ctx| {
            ...
            items.push(MessageItem::clickable(
                vec![
                    ...
                    MessageItem::text(if is_plan_for_this_conversation_open {
                        i18n::t("terminal.message_bar.to_hide_plan")
                    } else if plan_count > 1 {
                        i18n::t("terminal.message_bar.to_view_plans")
                    } else {
                        i18n::t("terminal.message_bar.to_view_plan")
                    }),
                ],
                |ctx| {
            ...
            .is_some()
        {
            ...
            items.push(MessageItem::clickable(
                vec![
                    ...
                    MessageItem::text(i18n::t("terminal.message_bar.to_fork_and_continue")),
                ],
                |ctx| {
    ...
    fn produce_message(&self, args: AgentMessageArgs<'_>) -> Option<Message> {
        ...
        Some(Message::new(vec![MessageItem::clickable(
            vec![
                ...
                MessageItem::text(i18n::t("terminal.message_bar.to_hide_help")),
            ],
            |ctx| {
                ctx.dispatch_typed_action(InputAction::ToggleAgentViewShortcuts);
            },
        ...
        let message = match keybinding_name_to_keystroke(SET_INPUT_MODE_AGENT_ACTION_NAME, app) {
            Some(keystroke) => Message::new(vec![
                MessageItem::text(i18n::t(
                    "terminal.agent_message_bar.autodetected_shell_command_prefix",
                )),
                MessageItem::keystroke(keystroke),
                MessageItem::text(i18n::t("terminal.message_bar.to_override")),
            ])
            .with_text_color(appearance.theme().ansi_fg_blue()),
            None => Message::from_text(i18n::t(
                "terminal.agent_message_bar.autodetected_shell_command",
            )),
        };
    ...
    fn produce_message(&self, args: AgentMessageArgs<'_>) -> Option<Message> {
        ...
        Some(
            Message::new(vec![
                ...
                MessageItem::text(i18n::t("terminal.agent_message_bar.to_exit_shell_mode")),
            ])
