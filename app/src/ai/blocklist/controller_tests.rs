...
#[test]
fn failed_shared_session_wake_requeues_only_its_cross_task_event_batch() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
        let server_token = ServerConversationToken::new();
        let query_task_id = TaskId::new("active-subagent-task".to_string());

        let (conversation_id, root_task_id) = terminal.update(&mut app, |view, ctx| {
            let terminal_surface_id = view.id();
            let stream_id = ResponseStreamId::new_for_test();
            let (conversation_id, root_task_id) =
                BlocklistAIHistoryModel::handle(ctx).update(ctx, |history, ctx| {
                    let conversation_id =
                        history.start_new_conversation(view.id(), false, false, false, ctx);
                    history.set_server_conversation_token_for_conversation(
                    ...
                    let root_task_id = history
                        .conversation(&conversation_id)
                        .expect("target conversation should exist")
                        .get_root_task_id()
                        .clone();
                    history
                        .update_conversation_for_new_request_input(
                            RequestInput {
                                conversation_id,
                                input_messages: HashMap::from([(root_task_id.clone(), vec![])]),
                                working_directory: None,
                                model_id: LLMId::from("test-model"),
                                coding_model_id: LLMId::from("test-coding-model"),
                                cli_agent_model_id: LLMId::from("test-cli-agent-model"),
                        ...
                        .expect("target request should start");
            ...
            view.ai_controller().update(ctx, |controller, ctx| {
                {
                    let mut terminal_model = controller.terminal_model.lock();
            ...
            OrchestrationEventService::handle(ctx).update(ctx, |service, ctx| {
                service.enqueue_event_batch(
                    conversation_id,
                    vec![PendingEvent {
                        event_id: "event-x".to_string(),
                        source_agent_id: "parent-agent".to_string(),
                        attempt_count: 0,
                        detail: PendingEventDetail::Message {
                            message_id: "message-x".to_string(),
                            addresses: vec!["child-agent".to_string()],
                            subject: "Earlier request".to_string(),
                            message_body: "Already in flight.".to_string(),
                        },
                    }],
                    ctx,
                );
                service
                    .drain_events_for_request(conversation_id, ctx)
                    .expect("event X should move to awaiting delivery");
                service.enqueue_event_batch(
                    conversation_id,
                    vec![PendingEvent {
                        event_id: "event-y".to_string(),
                        source_agent_id: "parent-agent".to_string(),
                        attempt_count: 0,
                        detail: PendingEventDetail::Message {
                            message_id: "message-y".to_string(),
                            addresses: vec!["child-agent".to_string()],
                            subject: "Wake request".to_string(),
                            message_body: "Retry only this batch.".to_string(),
                        },
                    }],
                    ctx,
                );
        ...
        assert_ne!(query_task_id, root_task_id);
        OrchestrationEventService::handle(&app).read(&app, |service, _| {
            assert_eq!(
                service.awaiting_event_ids_for_test(conversation_id),
                vec!["event-x"]
            );
            assert_eq!(
                service.pending_event_attempts_for_test(conversation_id),
                vec![("event-y".to_string(), 1)]
            );
        });
    });
}
