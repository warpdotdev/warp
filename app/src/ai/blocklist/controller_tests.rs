...
/// When a parent-completion cascade recorded this local descendant's own run as `SUCCEEDED`
/// server-side while it was still in progress, `stop_conversation_for_parent_completed_success`
/// must finalize the conversation as `Success` (not `Cancelled`), and a subsequent
/// `ManuallyCancelled` (as fired by the pane-close path) must not overwrite that outcome.
#[test]
fn stop_conversation_for_parent_completed_success_reports_success_and_survives_manual_cancel() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
        ...
        let conversation_id = terminal.update(&mut app, |view, ctx| {
            ...
            view.ai_controller().update(ctx, |controller, ctx| {
                controller.register_mock_stream_for_test(stream_id, conversation_id, stream, ctx);
                controller.stop_conversation_for_parent_completed_success(conversation_id, ctx);
            });
            conversation_id
        });

        // The in-flight request is finalized as Success, not Cancelled.
        BlocklistAIHistoryModel::handle(&app).read(&app, |history, _| {
            assert_eq!(
                history.conversation(&conversation_id).map(|c| c.status()),
                Some(&crate::ai::agent::conversation::ConversationStatus::Success)
