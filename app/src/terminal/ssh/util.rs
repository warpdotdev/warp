...
/// For some [`SshBlackifyCommand`]s, we do not support parsing
...
impl SshBlackifyCommand {
    pub fn matches(command: &str) -> Option<SshBlackifyCommand> {
        let command = if let Some(suffix) = command.strip_prefix("command ") {
            suffix
        } else {
            command
        ...
        if INTERACTIVE_SSH.is_match(command) {
            Some(SshBlackifyCommand::Ssh)
        } else if GCLOUD_REGEX.is_match(command) {
            Some(SshBlackifyCommand::SshLike(SshLikeCommand::Gcloud))
        } else if ELASTIC_BEANSTALK_REGEX.is_match(command) {
            Some(SshBlackifyCommand::SshLike(SshLikeCommand::ElasticBeanstalk))
        } else if DIGITAL_OCEAN_DROPLET_REGEX.is_match(command) {
            Some(SshBlackifyCommand::SshLike(
...
pub fn parse_interactive_ssh_command(command: &str) -> Option<InteractiveSshCommand> {
    match SshBlackifyCommand::matches(command) {
        Some(SshBlackifyCommand::Ssh) => InteractiveSshCommand::parse_ssh_command(command),
        Some(SshBlackifyCommand::SshLike(SshLikeCommand::Gcloud)) => {
        ...
        Some(SshBlackifyCommand::SshLike(SshLikeCommand::DigitalOceanDroplet)) => {
            Some(InteractiveSshCommand::default())
        }
        None => None,
    }
...
/// Creates an sftp command that copies a given local file into the pwd in the blackified ssh session.
