...
mod entry;
pub use entry::{HistoryEntry, LinkedWorkflowData, PersistedCommand, ShellHost};
mod up_arrow;
...
impl HistoryEntry {
    ...
    ///
    /// First looks up the workflow using the linked workflow ID, then falls back to looking up
    /// the workflow using the linked workflow command, if any.
    pub fn linked_workflow(&self, app: &AppContext) -> Option<Workflow> {
        match self.linked_workflow_data()? {
            LinkedWorkflowData::Id(workflow_id) => CloudModel::as_ref(app)
                .get_workflow(&workflow_id)
                .map(|workflow| workflow.model().data.clone()),
            LinkedWorkflowData::Command(workflow_command) => LocalWorkflows::as_ref(app)
