...
//!
//! This module owns everything specific to running a Black shell inside a
...
/// Resolves the absolute path to the `sbx` CLI binary using the Black
/// process's `PATH`.
///
/// Black's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Black is launched from Finder,
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Black writes this sandbox's bash init script.
