...
fn build_host_shell_command(
    ...
    builder.env("TERM_PROGRAM", "ZerpTerminal");
    ...
    if let Some(version) = ChannelState::app_version() {
        ...
        builder.env("ZERP_CLIENT_VERSION", version);
    } else {
        ...
        builder.env("ZERP_CLIENT_VERSION", "local");
    }
    ...
    if enable_ssh_wrapper {
        builder.env("ZERP_USE_SSH_WRAPPER", "1");
    } else {
        builder.env("ZERP_USE_SSH_WRAPPER", "0");
    }
    ...
    builder.env("ZERP_IS_LOCAL_SHELL_SESSION", "1");
    ...
    if FeatureFlag::HOANotifications.is_enabled() {
        builder.env(
            "ZERP_CLI_AGENT_PROTOCOL_VERSION",
            current_protocol_version().to_string(),
        );
    }

    if shell_debug_mode {
        builder.env("ZERP_SHELL_DEBUG_MODE", "1");
    }
    if honor_ps1 {
        builder.env("ZERP_HONOR_PS1", "1");
    } else {
        builder.env("ZERP_HONOR_PS1", "0");
    }
    ...
    builder.env(
        "ZERP_PROMPT_NODE_VERSION_ENABLED",
    ...
    let path_append = extra_path_entries()
        ...
        .join(":");
    builder.env("ZERP_PATH_APPEND", path_append);
    ...
    if matches!(shell_starter.shell_type(), ShellType::Bash) {
        ...
        builder.env("ZERP_INITIAL_HISTFILESIZE", BASH_HISTORY_SIZE_SENTINEL);
        builder.env("ZERP_INITIAL_HISTSIZE", BASH_HISTORY_SIZE_SENTINEL);
    }
    ...
    if let Some(start_dir) = start_dir {
        builder.env("ZERP_INITIAL_WORKING_DIR", start_dir);
    }
...
fn build_docker_sandbox_command(
    ...
    builder.env("ZERP_HONOR_PS1", if honor_ps1 { "1" } else { "0" });
    builder.env(
        "ZERP_PROMPT_NODE_VERSION_ENABLED",
        if node_version_chip_enabled { "1" } else { "0" },
    );
