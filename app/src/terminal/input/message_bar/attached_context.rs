...
impl<Args: AttachedContextArgs + Copy> MessageProvider<Args> for AttachedBlocksMessageProducer {
    fn produce_message(&self, args: Args) -> Option<Message> {
        ...
        let message_text = if context_block_ids.len() == 1 {
            i18n::t("terminal.input.message_bar.attached_context.single")
                .replace("{command}", &block_command)
        } else if context_block_ids.len() == 2 {
            i18n::t("terminal.input.message_bar.attached_context.one_other_command")
                .replace("{command}", &block_command)
        } else {
            i18n::t("terminal.input.message_bar.attached_context.other_commands")
                .replace("{command}", &block_command)
                .replace(
                    "{count}",
        ...
        if args.agent_view_controller().is_active() {
            ...
            items.push(MessageItem::clickable(
                vec![
                    ...
                    MessageItem::text(i18n::t("terminal.message_bar.to_remove")),
                ],
                |ctx| {
    ...
    for AttachedTextSelectionMessageProducer
{
    fn produce_message(&self, args: Args) -> Option<Message> {
        ...
        let mut items = vec![MessageItem::text(i18n::t(
            "terminal.input.message_bar.attached_context.selected_text",
        ))];
