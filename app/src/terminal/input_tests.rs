...
/// The ctrl-r/ctrl-t helpers are Warp-internal. Fish does not honor histignorespace in
/// `Shell::should_add_command_to_history`, so a space prefix would still record them unless
/// `should_add_command_to_history` is false on the execute event.
#[cfg_attr(windows, ignore = "TODO(CORE-3626)")]
#[test]
fn fzf_ctrl_r_helper_is_not_added_to_warp_history() {
    assert_fzf_helper_is_not_added_to_warp_history(|input, ctx| {
        input.trigger_external_ctrl_r_history_search("warp_run_external_ctrl_r_widget", ctx)
    });
}

#[cfg_attr(windows, ignore = "TODO(CORE-3626)")]
#[test]
fn fzf_ctrl_t_helper_is_not_added_to_warp_history() {
    assert_fzf_helper_is_not_added_to_warp_history(|input, ctx| {
        input.trigger_external_ctrl_t_file_search(
            "warp_run_external_ctrl_t_widget",
            CtrlTApplyMode::Splice,
...
fn assert_fzf_helper_is_not_added_to_warp_history(
    trigger: impl Fn(&mut Input, &mut ViewContext<Input>) -> bool + 'static,
) {
    let session_id: SessionId = 1.into();
    let session_info = SessionInfo::new_for_test()
        .with_id(session_id)
    ...
    App::test((), |mut app| async move {
        ...
        let executed = Rc::new(RefCell::new(Vec::<ExecuteCommandEvent>::new()));
        let executed_for_subscription = executed.clone();
        app.update(|ctx| {
            ctx.subscribe_to_view(&input, move |_, event: &super::Event, _| {
                if let super::Event::ExecuteCommand(event) = event {
                    executed_for_subscription
                        .borrow_mut()
                        .push((**event).clone());
                }
            });
        ...
        let started = input.update(&mut app, |input, ctx| trigger(input, ctx));
        assert!(started);

        let events = executed.borrow().clone();
        assert_eq!(events.len(), 1);
        assert!(
            !events[0].should_add_command_to_history,
            "helper {} must not request Warp history",
            events[0].command
        );
        assert!(
            events[0].command.contains("warp_run_external_ctrl_"),
            "{}",
            events[0].command
        );
        ...
        History::handle(&app).read(&app, |history, _ctx| {
            assert!(
                history.commands(session_id).unwrap().is_empty(),
                "fzf helpers must not land in Warp history"
            );
        });
    });
}
