...
fn precmd_with_completion_metadata_recovers_in_band_completion_and_reuses_cached_prompt() {
    ...
    // The recovered in-band block is hidden by default, so it's removed from the block list
    // once its completion event fires; the recovery is only observable through those events.
    let events: Vec<_> = std::iter::from_fn(|| event_rx.try_recv().ok()).collect();
