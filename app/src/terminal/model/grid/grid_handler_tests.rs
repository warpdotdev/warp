...
mod bounds_to_markdown_string_tests {
    use warp_terminal::model::ansi::Hyperlink;
    ...
    fn write(grid: &mut GridHandler, s: &str) {
        for c in s.chars() {
            grid.input(c);
        }
    }

    fn full_bounds(grid: &GridHandler) -> (Point, Point) {
        let cols = grid.columns().saturating_sub(1);
        let last_row = grid.total_rows().saturating_sub(1);
        (Point::new(0, 0), Point::new(last_row, cols))
    }

    #[test]
    fn allowed_scheme_emits_markdown_link() {
        let mut grid = GridHandler::new_for_test(2, 20);
        grid.set_hyperlink(Some(link("https://example.com")));
        write(&mut grid, "Click me");
        grid.set_hyperlink(None);

        let (start, end) = full_bounds(&grid);
        let md = grid.bounds_to_markdown_string(start, end, RespectObfuscatedSecrets::No);
        assert!(
            md.contains("[Click me](https://example.com)"),
            "got: {md:?}"
        );
    }

    #[test]
    fn disallowed_scheme_renders_as_plain_text() {
        // Per product invariant 13: spans whose URI fails the OSC 8 scheme
        // allow-list emit as plain visible text — no `[`, no `(`, no URI.
        let mut grid = GridHandler::new_for_test(2, 30);
        grid.set_hyperlink(Some(link("javascript:alert(1)")));
        write(&mut grid, "Click me");
        grid.set_hyperlink(None);

        let (start, end) = full_bounds(&grid);
        let md = grid.bounds_to_markdown_string(start, end, RespectObfuscatedSecrets::No);
        assert!(md.contains("Click me"), "got: {md:?}");
        assert!(!md.contains('['), "should not contain `[`, got: {md:?}");
        assert!(
            !md.contains("javascript"),
            "URI must not leak into output, got: {md:?}"
        );
    }

    #[test]
    fn visible_text_inside_anchor_is_markdown_escaped() {
        // Anti-regression: characters that would break out of `[...]` or be
        // re-interpreted as markdown formatting must be backslash-escaped.
        let mut grid = GridHandler::new_for_test(2, 30);
        grid.set_hyperlink(Some(link("https://example.com")));
        write(&mut grid, "a]b*c_d");
        grid.set_hyperlink(None);

        let (start, end) = full_bounds(&grid);
        let md = grid.bounds_to_markdown_string(start, end, RespectObfuscatedSecrets::No);
        assert!(
            md.contains(r"[a\]b\*c\_d](https://example.com)"),
            "got: {md:?}"
        );
    }

    #[test]
    fn uri_special_chars_are_url_encoded() {
        // `)` and space would terminate or mis-parse the `(...)` target;
        // url_encode_for_markdown must percent-encode them.
        let mut grid = GridHandler::new_for_test(2, 40);
        grid.set_hyperlink(Some(link("https://example.com/a b)c")));
        write(&mut grid, "x");
        grid.set_hyperlink(None);
        ...
        let md = grid.bounds_to_markdown_string(start, end, RespectObfuscatedSecrets::No);
        assert!(
            md.contains("[x](https://example.com/a%20b%29c)"),
            "got: {md:?}"
        );
    }

    #[test]
    fn span_across_rows_emits_one_link_per_row() {
        // A span that wraps across rows in the grid emits as one markdown
        // link per row — same URI on each row, never a single link straddling
        // the newline (markdown anchors can't contain raw newlines).
        let mut grid = GridHandler::new_for_test(3, 5);
        grid.set_hyperlink(Some(link("https://example.com")));
        // 8 chars across a 5-column grid -> wraps onto row 1.
        write(&mut grid, "abcdefgh");
        grid.set_hyperlink(None);

        let (start, end) = full_bounds(&grid);
        let md = grid.bounds_to_markdown_string(start, end, RespectObfuscatedSecrets::No);
        let occurrences = md.matches("](https://example.com)").count();
        assert_eq!(occurrences, 2, "expected 2 link emissions, got: {md:?}");
    }

    #[test]
    fn plain_cells_outside_span_are_escaped_but_not_linked() {
        let mut grid = GridHandler::new_for_test(2, 30);
        write(&mut grid, "before*");
        grid.set_hyperlink(Some(link("https://example.com")));
        write(&mut grid, "mid");
        grid.set_hyperlink(None);
        write(&mut grid, "after");

        let (start, end) = full_bounds(&grid);
        let md = grid.bounds_to_markdown_string(start, end, RespectObfuscatedSecrets::No);
        assert!(
            md.contains(r"before\*[mid](https://example.com)after"),
            "got: {md:?}"
        );
    }
}
