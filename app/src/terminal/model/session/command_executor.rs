    ...
    match &session_info.session_type {
        BootstrapSessionType::Local if !force_use_in_band_generators => {
            ...
            match &session_info.launch_data {
                ...
                // Docker sandbox and Dev Container sessions should already
                // be routed to the in-band executor via
                // `launch_data_needs_in_band_executor` above, so this arm is
                // only reached if that routing drifts (e.g. a feature
                // flag/debug setting disables it). Rather than panic in
                // user-facing code, log loudly and fall back to a no-op
                // executor so the session still runs; generators won't work
                // but the PTY stays healthy.
                Some(
                    ShellLaunchData::DockerSandbox { .. } | ShellLaunchData::DevContainer { .. },
                ) => {
                    debug_assert!(
                        false,
                        "Docker sandbox / Dev Container sessions should be routed through the in-band executor"
                    );
                    report_error!(
                        "Docker sandbox / Dev Container session reached the local-executor branch; \
