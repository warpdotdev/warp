...
fn file_title_text(openable_path: &OpenablePath) -> String {
    match openable_path.file_type {
        OpenableFileType::Markdown => i18n::t("terminal.open_in_warp.markdown_title"),
            ...
            cfg_if::cfg_if! {
                if #[cfg(not(target_family = "wasm"))] {
                    ...
                    match language.as_ref().map(|language| language.display_name()) {
                        Some(display_name) => {
                            i18n::t("terminal.open_in_warp.code_language_title")
                                .replace("{language}", display_name)
                        }
                        None => i18n::t("terminal.open_in_warp.code_title"),
                    }
                } else {
                    // The `languages` crate is not available on WASM, so use a fallback message.
                    i18n::t("terminal.open_in_warp.code_title")
                }
            }
        }
    }
...
pub fn render_open_in_warp_banner(
    ...
    let button_text = match state.target.file_type {
        OpenableFileType::Markdown => i18n::t("terminal.open_in_warp.view_in_warp"),
        OpenableFileType::Code | OpenableFileType::Text => {
            i18n::t("terminal.open_in_warp.edit_in_warp")
        }
    };
