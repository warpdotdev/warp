...
/// REMOTE-2208: a query sent into a live ambient session refreshes the run's post-failure
/// session-retention window, so an environment retained for debugging is not torn down
/// mid-conversation.
#[test]
fn session_keepalive_targets_ambient_run_for_executor_viewer() {
    let task_id = ambient_task_id(1);
    assert_eq!(
        session_keepalive_run_id(&AIQueryRouting::LiveRemoteVm {
            is_executor: true,
            ambient_agent_task_id: Some(task_id),
        }),
        Some(task_id)
...
/// The keepalive must not depend on the client's (possibly stale) view of the run state: a run
/// the client still believes is in progress may already have failed into retention server-side.
#[test]
fn session_keepalive_targets_run_regardless_of_client_side_run_state() {
    App::test((), |mut app| async move {
        let TestHandles {
            terminal_view_id,
            task_id,
        ...
        // A run the client still has cached as an ordinary in-progress execution.
        AgentConversationsModel::handle(&app).update(&mut app, |model, _| {
            model.insert_task_for_test(active_ambient_agent_task(task_id));
        });
        let model = ambient_pane_model(task_id, SharedSessionStatus::executor());
        app.update(|ctx| {
            let routing = resolve_ai_query_routing(terminal_view_id, None, &model, ctx);
            assert_eq!(session_keepalive_run_id(&routing), Some(task_id));
        });
    });
}

/// A retained failed run attached as an executor viewer is the case the feature exists for.
#[test]
fn session_keepalive_targets_retained_failed_run() {
    App::test((), |mut app| async move {
        let TestHandles {
            terminal_view_id,
            task_id,
...
#[test]
fn session_keepalive_skips_non_live_and_read_only_targets() {
    let task_id = ambient_task_id(1);
    // A read-only viewer's prompt is rejected, so it is not session activity.
    assert_eq!(
        session_keepalive_run_id(&AIQueryRouting::LiveRemoteVm {
            is_executor: false,
            ambient_agent_task_id: Some(task_id),
    ...
    // A shared *local* session has no ambient run whose environment could be retained.
    assert_eq!(
        session_keepalive_run_id(&AIQueryRouting::LiveRemoteVm {
            is_executor: true,
            ambient_agent_task_id: None,
    ...
    // A cloud-to-cloud follow-up starts a brand new VM; there is no retained session to refresh.
    assert_eq!(
        session_keepalive_run_id(&AIQueryRouting::NewCloudVm { task_id }),
        None
    );
    assert_eq!(
        session_keepalive_run_id(&AIQueryRouting::UnconnectedReadOnly),
        None
    );
    assert_eq!(session_keepalive_run_id(&AIQueryRouting::Local), None);
}
