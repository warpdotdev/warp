...
impl ModelSelector {
    ...
    fn build_oz_menu_items(
        ...
        let items: Vec<MenuItem<ModelSelectorAction>> = auto_choices
            ...
            .map(|llm| {
                let display_name = llm.menu_display_name();
                let leading_icon = oz_menu_item_leading_icon(llm);
...
/// The leading icon for a model row in the Oz Agent Mode model list.
/// Kept as a small free function so it can be unit tested without spinning
/// up a full `ModelSelector` view.
fn oz_menu_item_leading_icon(llm: &LLMInfo) -> Icon {
    model_leading_icon(
        llm,
        ModelIconFlags {
