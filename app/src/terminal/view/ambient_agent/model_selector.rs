...
impl ModelSelector {
    ...
    fn build_oz_menu_items(
        ...
        let items: Vec<MenuItem<ModelSelectorAction>> = auto_choices
            ...
            .map(|llm| {
                let display_name = llm.menu_display_name();
                let leading_icon = oz_menu_item_leading_icon(
                    llm,
                    should_show_bedrock_icon_for_model(llm, ctx),
                    should_show_gemini_enterprise_agent_platform_icon_for_model(llm, ctx),
                );
...
/// The leading icon for an Oz Agent Mode menu row. Delegates to the shared
/// [`model_leading_icon`] so this menu can't drift from the model picker's
/// icon precedence (custom router / auto / host / provider or brand logo).
fn oz_menu_item_leading_icon(
    llm: &LLMInfo,
