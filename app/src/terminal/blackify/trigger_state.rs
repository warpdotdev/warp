...
use black_ui::{AppContext, EntityId, SingletonEntity as _, ViewContext, ViewHandle};

use super::success_block::BlackifySuccessBlock;
...
pub enum SshBlockState {
    Blackifying {
        handle: ViewHandle<SshBlackifyBlock>,
    },
    BlackifySuccess {
        handle: ViewHandle<BlackifySuccessBlock>,
    },
    InstallTmux {
...
impl SshBlockState {
    ...
    pub fn get_block_view_id(&self) -> EntityId {
        match self {
            SshBlockState::Blackifying { handle, .. } => handle.id(),
            SshBlockState::BlackifySuccess { handle, .. } => handle.id(),
    ...
    pub fn collapse_script(&mut self, ctx: &mut ViewContext<TerminalView>) -> bool {
        match self {
            ...
            SshBlockState::Blackifying { .. } => false,
            SshBlockState::BlackifySuccess { .. } => false,
    ...
    pub fn focus(&mut self, ctx: &mut ViewContext<TerminalView>) {
        match self {
            SshBlockState::Blackifying { handle } => {
                handle.update(ctx, |block, ctx| block.focus(ctx));
            }
            SshBlockState::BlackifySuccess { .. } => {}
            SshBlockState::InstallTmux { handle } => {
                handle.update(ctx, |block, ctx| block.focus(ctx));
            }
    ...
    pub fn get_system_details(&self, app: &AppContext) -> Option<SystemDetails> {
        match self {
            ...
            SshBlockState::Blackifying { .. } => None,
            SshBlockState::BlackifySuccess { .. } => None,
    ...
    pub fn on_warpified_session_complete(
        ...
        match self {
            SshBlockState::InstallTmux { .. } | SshBlockState::Blackifying { .. } => {
                let block_id = self.get_block_view_id();
                return Some(block_id);
            }
            SshBlockState::BlackifySuccess { handle } => {
...
/// Temporary state used to trigger Blackification.
#[derive(Default)]
struct BlackifyTriggerState {
    block_id: Option<BlockId>,

    /// Lets us abort an attempt to auto blackify if the subshell command
    /// hasn't completed.
    auto_blackify_abort_handle: Option<SpawnedFutureHandle>,
    ...
    /// The command which may trigger ssh Blackification
    pending_command: Option<String>,
    /// The Host which may trigger ssh Blackification
    pending_blackify_ssh_host: Option<String>,

    /// Which, if any, SSH block is currently added to the blocklist.
    ssh_block_state: Option<SshBlockState>,

    ssh_blackify_timeout_handle: Option<SpawnedFutureHandle>,
...
#[derive(Default)]
pub struct BlackifyState {
    session_id: Option<SessionId>,

    pending_state: Option<BlackifyTriggerState>,
...
impl BlackifyState {
    ...
    pub fn add_auto_blackify_abort_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.auto_blackify_abort_handle = Some(spawned_future_handle);
    }

    pub fn abort_auto_blackify(&mut self) {
        if let Some(abort_handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.auto_blackify_abort_handle.take())
        {
            abort_handle.abort();
        };
    }

    pub fn add_ssh_blackify_timeout_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.ssh_blackify_timeout_handle = Some(spawned_future_handle);
    }

    pub fn abort_ssh_blackify_timeout(&mut self) {
        self.replace_timeout_id();
        if let Some(handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.ssh_blackify_timeout_handle.take())
        {
            handle.abort();
        };
    }
    ...
    pub fn get_pending_ssh_host(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &BlackifyTriggerState| state.pending_blackify_ssh_host.clone())
    }

    pub fn get_pending_ssh_command(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &BlackifyTriggerState| state.pending_command.clone())
    }

    pub fn take_pending_ssh_host(&mut self) -> Option<String> {
        self.pending_state
            .as_mut()
            .and_then(|state: &mut BlackifyTriggerState| state.pending_blackify_ssh_host.take())
    }

    pub fn clear_pending_ssh_host(&mut self) {
        if let Some(ref mut pending_state) = self.pending_state.as_mut() {
            pending_state.pending_blackify_ssh_host = None;
        }
    }
    ...
    pub fn set_pending_ssh_host(&mut self, command: String, ssh_host: Option<String>) {
        ...
        pending_state.pending_blackify_ssh_host = ssh_host;
    }
    ...
    /// and we have a Blackify Success block.
    fn on_warpified_session_complete(
        &mut self,
        state: BlackifyTriggerState,
        ctx: &mut ViewContext<TerminalView>,
    ) -> Option<EntityId> {
