...
impl ProfileModelSelector {
    pub fn new(
        ...
        let profile_button = ctx.add_typed_action_view(|ctx| {
            ...
            .with_tooltip(i18n::t("terminal.profile_model_selector.profile_tooltip"))
        ...
        let model_button = ctx.add_typed_action_view(|ctx| {
            ...
            .with_tooltip(i18n::t("terminal.profile_model_selector.model_tooltip"))
            .with_size(ButtonSize::UDIButton)
        });
            ...
            FeaturePopup::new_feature(NewFeaturePopupLabel::FromCallable(Box::new(|ctx| {
                ...
                if let Some(new_choices) = new_choices {
                    ...
                    let mut label = display_names
                        ...
                        .map(|name| {
                            if *name == "auto" {
                                i18n::t("terminal.profile_model_selector.auto_select_best_model")
                            } else {
                                name.to_string()
                            }
                        })
                    ...
                    i18n::t("terminal.profile_model_selector.new_models_available")
                }
            })))
        });
        ...
        let manage_api_key_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(i18n::t("common.manage"), SecondaryTheme)
                .with_tooltip(i18n::t("terminal.profile_model_selector.manage_api_keys"))
    ...
    fn refresh_state(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let model_tooltip: Option<String> = if self.is_locked_for_cloud_followup(ctx) {
            Some(i18n::t(
                "terminal.profile_model_selector.model_locked_followup_tooltip",
            ))
        } else if self.is_locked_for_non_oz_run(ctx) {
            None
        } else {
            Some(i18n::t("terminal.profile_model_selector.model_tooltip"))
        };
    ...
    fn refresh_profile_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let mut menu_items = vec![
            MenuItem::Header {
                fields: MenuItemFields::new(i18n::t("terminal.profile_selector.profiles"))
        ...
        menu_items.push(MenuItem::Item(
            MenuItemFields::new(i18n::t("terminal.profile_selector.manage_profiles"))
                .with_icon(Icon::Gear)
    ...
    fn harness_model_display_name(&self, app: &AppContext) -> String {
        self.active_harness_model_info(app)
            ...
            .unwrap_or_else(|| i18n::t("common.default"))
    }
    ...
    fn refresh_harness_model_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let mut default_fields =
            MenuItemFields::new(i18n::t("common.default")).with_on_select_action(default_action);
        if default_selected {
    ...
    fn refresh_model_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if !custom_choices.is_empty() {
            ...
            items.push(MenuItem::Header {
                fields: MenuItemFields::new(i18n::t("terminal.profile_selector.custom_models"))
                    .with_override_text_color(
                        appearance
                            .theme()
                            .sub_text_color(appearance.theme().background())
        ...
        Hoverable::new(self.profile_mouse_state.clone(), move |state| {
            if state.is_hovered() {
                ...
                let tooltip = appearance
                    .ui_builder()
                    .tool_tip(i18n::t("terminal.profile_model_selector.profile_tooltip"));
    ...
    fn render_model_section(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let hoverable = Hoverable::new(self.model_mouse_state.clone(), move |state| {
            if state.is_hovered() && can_interact {
                ...
                let tooltip = appearance
                    .ui_builder()
                    .tool_tip(i18n::t("terminal.profile_model_selector.model_tooltip"));
                let mut stack = Stack::new();
                stack.add_child(button_with_hover);
                stack.add_positioned_overlay_child(
                    tooltip.build().finish(),
                ...
                let tooltip_text: Option<String> = if is_locked_for_followup {
                    ...
                    Some(i18n::t(
                        "terminal.profile_model_selector.model_requires_edit_access_tooltip",
                    ))
                };
    ...
    fn render_model_spec_api_key(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        Container::new(
            Flex::row()
                ...
                .with_cross_axis_alignment(CrossAxisAlignment::Center)
                .with_child(
                    self.render_model_spec_value_label(i18n::t("terminal.model_specs.cost"), app),
                )
                .with_child(
                    Expanded::new(
                        1.,
                        Flex::row()
                            ...
                            .with_child(
                                Container::new(
                                    Text::new(
                                        i18n::t("terminal.profile_selector.billed_to_api"),
                                        appearance.ui_font_family(),
                                        14.,
                                    )
                                    .with_color(theme.disabled_ui_text_color().into())
    ...
    fn render_sidecar_spec_panel(
        ...
        let (title, description) = match kind {
            ModelSpecSidecarKind::Auto => (
                i18n::t("terminal.profile_model_selector.auto_mode_title"),
                i18n::t("terminal.profile_model_selector.auto_mode_description"),
            ),
            ModelSpecSidecarKind::Reasoning => (
                i18n::t("terminal.profile_model_selector.reasoning_level_title"),
                i18n::t("terminal.profile_model_selector.reasoning_level_description"),
            ),
        };
