    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ThemeCreatorImageState::Empty => {
                write!(f, "{}", i18n::t("themes.creator.select_image"))
            }
            ThemeCreatorImageState::Uploading => {
                write!(f, "{}", i18n::t("themes.creator.selecting_image"))
            }
    ...
    pub fn create_theme(&mut self, ctx: &mut ViewContext<Self>) {
        if let Some(theme_options) = self.theme_options.as_mut() {
            ...
            let Some(image_extension) = image_extension else {
                self.send_error_toast(i18n::t("themes.creator.error_process_image"), ctx);
                return;
            };
            ...
            if errored {
                self.send_error_toast(i18n::t("common.something_went_wrong"), ctx);
            }
        }
    }
            ...
            move |theme_creator_body, theme_options, ctx| match theme_options {
                ...
                Err(e) => {
                    theme_creator_body.send_error_toast(
                        i18n::t("themes.creator.error_process_image_with_error")
...
impl View for ThemeCreatorBody {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let mut create_button = appearance
            ...
            .with_centered_text_label(i18n::t("themes.creator.create_theme"));
        ...
        let mut flex: Flex = Flex::column()
            ...
            .with_child(
                Container::new(
                    Text::new_inline(
                        i18n::t("themes.creator.modal_subheader"),
        ...
        if let Some(theme_options) = &self.theme_options {
            flex.add_child(
                Container::new(
                    Text::new_inline(
                        i18n::t("themes.creator.theme_name"),
                        appearance.ui_font_family(),
                        14.,
                    )
                    .with_color(appearance.theme().active_ui_text_color().into())
            ...
            flex.add_child(
                Container::new(
                    Text::new_inline(
                        i18n::t("themes.creator.background_color"),
                        appearance.ui_font_family(),
                        14.,
                    )
                    .with_color(appearance.theme().active_ui_text_color().into())
