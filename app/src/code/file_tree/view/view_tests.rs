...
fn std_path(path: &std::path::Path) -> black_util::standardized_path::StandardizedPath {
    black_util::standardized_path::StandardizedPath::try_from_local(path).unwrap()
}
...
fn repo_transition_unregisters_lazy_loaded_path() {
    VirtualFS::test("file_tree_repo_transition", |dirs, mut vfs| {
        ...
        let canonical_repo_root =
            black_util::standardized_path::StandardizedPath::from_local_canonicalized(&repo_root)
                .unwrap();
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.read(&app, |view, _ctx| {
                assert!(view.registered_lazy_loaded_paths.contains(
                    &black_util::standardized_path::StandardizedPath::try_from_local(
                        &displayed_root
                    )
                    .unwrap()
                ));
                let displayed_std =
                    black_util::standardized_path::StandardizedPath::try_from_local(&displayed_root)
                        .unwrap();
                assert_eq!(
            ...
            file_tree_view.read(&app, |view, _ctx| {
                let displayed_std =
                    black_util::standardized_path::StandardizedPath::try_from_local(&displayed_root)
                        .unwrap();
                let repo_std =
                    black_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap();
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &black_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &black_util::standardized_path::StandardizedPath::try_from_local(&src_dir)
                        .unwrap(),
                    ctx,
                );
            });
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &black_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &black_util::standardized_path::StandardizedPath::try_from_local(&nested_dir)
                        .unwrap(),
                    ctx,
                );
            });
