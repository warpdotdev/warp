...
impl FileTreeView {
    ...
    fn show_exceeded_file_limit_toast(ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            let toast = DismissibleToast::error(i18n::t("code.file_tree.too_many_files"))
    ...
    fn context_menu_items(
        ...
        if is_remote {
            ...
            match item {
                FileTreeItem::File { .. } => {
                    ...
                    if !is_file_content_binary(&path_local) {
                        items.extend([
                            MenuItemFields::new(i18n::t("code.file_tree.open_in_new_pane"))
                                .with_on_select_action(FileTreeAction::OpenInNewPane {
                                    id: id.clone(),
                                })
                                .into_item(),
                            MenuItemFields::new(i18n::t("code.file_tree.open_in_new_tab"))
                        ...
                        items.push(
                            MenuItemFields::new(i18n::t("common.open_file"))
                ...
                FileTreeItem::DirectoryHeader { .. } => {
                    items.push(
                        MenuItemFields::new(i18n::t("code.file_tree.new_file"))
                    ...
                    if self.has_terminal_session {
                        items.push(
                            MenuItemFields::new(i18n::t("code.file_tree.cd_to_directory"))
            ...
            let open_text = if cfg!(target_os = "macos") {
                i18n::t("code.file_tree.reveal_in_finder")
            } else if cfg!(target_os = "windows") {
                i18n::t("code.file_tree.reveal_in_explorer")
            } else {
                i18n::t("code.file_tree.reveal_in_file_manager")
            };
            items.push(
            ...
            if !is_repo_root_dir {
                ...
                items.push(
                    MenuItemFields::new(i18n::t("common.delete"))
        ...
        if self.has_terminal_session {
            ...
            items.push(
                MenuItemFields::new(i18n::t("code.file_tree.attach_as_context"))
        ...
        items.extend([
            MenuItemFields::new(i18n::t("common.copy_path"))
                ...
                .into_item(),
            MenuItemFields::new(i18n::t("common.copy_relative_path"))
    ...
    fn render_error_state(&self, text: String, app: &AppContext) -> Box<dyn Element> {
        ...
        let main_column = Flex::column()
            ...
            .with_child(
                Text::new(
                    i18n::t("code.file_tree.project_explorer_unavailable"),
                    appearance.ui_font_family(),
...
impl View for FileTreeView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        self.render_error_state(i18n::t("code.file_tree.remote_unavailable"), app)
    }

    #[cfg(feature = "local_fs")]
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        if matches!(self.enablement, CodingPanelEnablementState::Disabled) {
            return self.render_error_state(i18n::t("code.file_tree.disabled_unavailable"), app);
        }
        ...
        if self.displayed_directories.is_empty() {
            ...
            if matches!(
                ...
                return self.render_error_state(i18n::t("code.file_tree.wsl_unavailable"), app);
            }
