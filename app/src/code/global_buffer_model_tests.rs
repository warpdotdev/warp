...
use crate::code::buffer_location::SyncClock;
...
impl GlobalBufferModel {
    ...
    /// Seeds a `ServerLocal` buffer with the given content and sync clock,
    /// bypassing the async `FileModel` load path.
    fn seed_server_local_buffer_for_test(
        &mut self,
        content: &str,
        server_version: u64,
        client_version: u64,
        ...
        self.buffers.insert(
            ...
            InternalBufferState {
                ...
                source: BufferSource::ServerLocal {
                    sync_clock: SyncClock::from_wire(server_version, client_version),
                    base_content_version: Some(version),
                    initial_content_version: Some(version),
                },
            },
...
#[test]
fn auto_reload_over_size_limit_broadcasts_for_server_local_buffer() {
    App::test((), |mut app| async move {
        init_app(&mut app);
        app.add_singleton_model(GlobalBufferModel::new);
        let _incremental_auto_reload = FeatureFlag::IncrementalAutoReload.override_enabled(true);
        ...
        let buffer_state = gbm(&app).update(&mut app, |gbm, ctx| {
            gbm.seed_server_local_buffer_for_test("hello", 1, 0, ctx)
        });
        let file_id = buffer_state.file_id;
        let base_version = app.read(|ctx| buffer_state.buffer.as_ref(ctx).version());
        let original_server_version = {
            let handle = gbm(&app);
            app.read(|ctx| {
                handle
                    .as_ref(ctx)
        ...
        // One byte over the (test-mode) size limit, so the synchronous fallback applies.
        let oversized_content = "a".repeat(MAX_INCREMENTAL_DIFF_CONTENT_BYTES + 1);
        gbm(&app).update(&mut app, |gbm, ctx| {
            gbm.populate_buffer_with_read_content(
                file_id,
        ...
        // The fallback must still bump the sync clock's server version, the same
        // way the background-diff path's ServerLocal branch does. Without this,
        // `ServerModel` never learns the content changed (it ignores a successful
        // `BufferUpdatedFromFileEvent`), so connected clients hold stale content.
        let handle = gbm(&app);
        app.read(|ctx| {
            let clock = handle
                .as_ref(ctx)
                .sync_clock_for_server_local(file_id)
                .unwrap();
            assert_ne!(clock.server_version, original_server_version);
        });
    })
}

#[test]
fn auto_reload_over_size_limit_aborts_stale_pending_diff() {
    App::test((), |mut app| async move {
        init_app(&mut app);
        app.add_singleton_model(GlobalBufferModel::new);
        let _incremental_auto_reload = FeatureFlag::IncrementalAutoReload.override_enabled(true);
        ...
        // An under-limit update leaves a background diff pending.
        gbm(&app).update(&mut app, |gbm, ctx| {
            gbm.populate_buffer_with_read_content(
                file_id,
                "hello world",
        ...
        let handle = gbm(&app);
        app.read(|ctx| {
            assert!(handle.as_ref(ctx).has_pending_diff_parse_for_test(file_id));
        });

        // A second, over-limit update (e.g. a rapid follow-up file-watcher event)
        // must abort that stale diff rather than leaving it to fail its
        // base-version check later and broadcast a spurious conflict.
        let oversized_content = "a".repeat(MAX_INCREMENTAL_DIFF_CONTENT_BYTES + 1);
        gbm(&app).update(&mut app, |gbm, ctx| {
            gbm.populate_buffer_with_read_content(
                file_id,
        ...
        app.read(|ctx| {
            assert!(
                !handle.as_ref(ctx).has_pending_diff_parse_for_test(file_id),
                "the stale under-limit diff should be aborted, not left pending"
            );
        });
        assert_eq!(content(&app, file_id), oversized_content);
    })
