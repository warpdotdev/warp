...
impl CodeEditorFind {
    pub fn new(searcher: ModelHandle<Searcher>, ctx: &mut ViewContext<Self>) -> Self {
        let find_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(i18n::t("code.editor.find.placeholder"), ctx);
            editor
        });
        ...
        let replace_editor = ctx.add_typed_action_view(|ctx| {
            ...
            replace_editor
                .set_placeholder_text(i18n::t("code.editor.find.replace_placeholder"), ctx);
            replace_editor
        });
        ...
        let select_all_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(i18n::t("code.editor.find.select_all"), SecondaryTheme)
                .on_click(|ctx| {
        ...
        let replace_all_button = ctx.add_typed_action_view(|ctx| {
            let mut button =
                ActionButton::new(i18n::t("code.editor.find.replace_all"), SecondaryTheme)
                    .on_click(|ctx| {
    ...
    pub fn emit_replace_a11y_content(&mut self, ctx: &mut ViewContext<Self>) {
        let content = if let Some(match_index) = self.searcher.as_ref(ctx).selected_match() {
            ...
            AccessibilityContent::new(
                i18n::t("code.editor.find.a11y.replaced_match")
                    .replace("{current}", &match_index.to_string())
                    .replace("{total}", &remaining_matches.to_string()),
                i18n::t("code.editor.find.a11y.replace_more_help"),
                WarpA11yRole::UserAction,
            )
        } else {
            AccessibilityContent::new_without_help(
                i18n::t("code.editor.find.a11y.replaced_last_match"),
                WarpA11yRole::UserAction,
            )
        };
        ctx.emit_a11y_content(content);
    ...
    fn render_find_row(
        ...
        let regex_icon = self.render_hoverable_icon_in_editor(
            ...
            editor_height,
            Some(i18n::t("code.editor.find.regex_tooltip")),
            ICON_PADDING,
        );
        let case_sensitive_icon = Container::new(
            SavePosition::new(
                self.render_hoverable_icon_in_editor(
                    ...
                    editor_height,
                    Some(i18n::t("code.editor.find.case_sensitive_tooltip")),
                    ICON_PADDING,
                ),
    ...
    fn render_replace_row(&self, appearance: &Appearance, editor_height: f32) -> Box<dyn Element> {
        ...
        let preserve_case_icon = self.render_hoverable_icon_in_editor(
            ...
            editor_height,
            Some(i18n::t("code.editor.find.preserve_case_tooltip")),
            ICON_PADDING,
        );
...
impl View for CodeEditorFind {
    ...
    fn accessibility_contents(&self, app: &AppContext) -> Option<AccessibilityContent> {
        ...
        let description = match (match_count, selected_match) {
            (0, _) | (_, None) => i18n::t("code.editor.find.a11y.find_bar"),
            (count, Some(current)) => i18n::t("code.editor.find.a11y.find_bar_with_matches")
        ...
        let help_text = if is_replace_focused {
            i18n::t("code.editor.find.a11y.replace_focused")
        } else {
            i18n::t("code.editor.find.a11y.find_focused")
        };
