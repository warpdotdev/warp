...
/// Helper to check whether the find bar's query input can currently be edited.
...
/// Text used by the find-bar click-to-reactivate tests. "hello" is the word under the cursor
/// at the buffer start, so Vim's `*` picks it up.
const FIND_BAR_TEST_TEXT: &str = "hello world\nhello beta\ntheta again";

#[test]
fn test_click_on_find_input_after_vim_enter_reactivates_it() {
    let _feature_flag_guard = FeatureFlag::VimCodeEditor.override_enabled(true);
    ...
    App::test((), |mut app| async move {
        ...
        // In Vim mode, Enter hands the caret back to the main editor, leaving the find input
        // disabled and unfocused: this reproduces the reported bug (before the fix, a
        // subsequent click on the input does nothing).
        let find_editor = find_bar.read(&app, |find_bar, _| find_bar.find_editor_for_test());
        ...
        // Typing must now be routed to the find input rather than to Vim: the query is
        // replaced (it's selected on reactivation) while the buffer and Vim mode are
        // untouched. Had Vim received these keystrokes instead, `a` would have switched it to
        // Insert mode.
        render_views(
            &mut app,
            &presenter,
...
#[test]
fn test_click_on_find_input_after_vim_search_word_reactivates_it() {
    let _feature_flag_guard = FeatureFlag::VimCodeEditor.override_enabled(true);
