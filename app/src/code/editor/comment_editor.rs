...
impl CommentEditor {
    ...
    fn create_buttons(
        ...
        let save_button = ctx.add_typed_action_view(|ctx| {
            ActionButton::new(i18n::t("code.editor.comment.comment"), PrimaryTheme)
                .with_keybinding(
        ...
        let remove_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(i18n::t("common.remove"), DangerNakedTheme)
                .on_click(|ctx| {
    ...
    pub fn reopen_saved_comment(
        ...
        self.save_button.update(ctx, |button, ctx| {
            button.set_label(i18n::t("common.update"), ctx);
        });
        ctx.notify();
    ...
    fn reset(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        self.save_button.update(ctx, |button, ctx| {
            button.set_label(i18n::t("code.editor.comment.comment"), ctx);
        });
        ctx.notify();
    ...
    fn render_github_import_indicator(
        ...
        let label = Text::new(
            i18n::t("code.editor.comment.imported_from_github"),
