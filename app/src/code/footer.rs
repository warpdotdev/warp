...
impl CodeFooterView {
    ...
    pub fn new(path: PathBuf, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let enable_lsp_button = server_type.map(|st| {
            let label =
                i18n::t("code.footer.enable_server").replace("{server_name}", st.binary_name());
            ctx.add_typed_action_view(|_ctx| {
        ...
        match statuses.len() {
            ...
            _ => {
                ...
                if any_needs_install {
                    Some(i18n::t("code.footer.install_servers"))
                } else {
                    Some(i18n::t("code.footer.enable_servers"))
                }
            }
        }
    }
    ...
    fn render_single_server_menu(
        ...
        let title = PersistedWorkspace::as_ref(app)
            ...
            .unwrap_or_else(|| i18n::t("code.footer.unknown_workspace"));
    ...
    fn render_restart_server_menu_item(
        ...
        Self::render_menu_item(
            ...
            i18n::t("code.footer.restart_server"),
    ...
    fn render_stop_server_menu_item(
        ...
        Self::render_menu_item(
            ...
            i18n::t("code.footer.stop_server"),
    ...
    fn render_start_server_menu_item(
        ...
        Self::render_menu_item(
            ...
            i18n::t("code.footer.start_server"),
    ...
    fn render_remove_server_menu_item(
        ...
        Self::render_menu_item(
            ...
            i18n::t("code.footer.remove_server"),
    ...
    fn render_restart_all_servers_menu_item(
        ...
        Self::render_menu_item(
            ...
            if is_plural {
                i18n::t("code.footer.restart_all_servers")
            } else {
                i18n::t("code.footer.restart_server")
            },
    ...
    fn render_stop_all_servers_menu_item(
        ...
        Self::render_menu_item(
            ...
            if is_plural {
                i18n::t("code.footer.stop_all_servers")
            } else {
                i18n::t("code.footer.stop_server")
            },
    ...
    fn render_start_all_servers_menu_item(
        ...
        Self::render_menu_item(
            ...
            if !is_plural {
                ...
                i18n::t("code.footer.start_all_stopped_servers")
            } else {
                i18n::t("code.footer.start_all_servers")
            },
    ...
    fn render_manage_servers_menu_item(
        ...
        Self::render_menu_item(
            ...
            i18n::t("code.footer.manage_servers"),
    ...
    fn server_status_message(server: &LspServerModel) -> Option<String> {
        match server.state() {
            LspModelState::Available { .. } | LspModelState::Starting => server
                ...
                .map(|msg| {
                    i18n::t("code.footer.server_message")
                        .replace("{server_name}", &server.server_name())
                        .replace("{message}", &msg)
                }),
            LspModelState::Stopped { .. } | LspModelState::Stopping { .. } => Some(
                i18n::t("code.footer.server_stopped")
                    .replace("{server_name}", &server.server_name()),
            ),
            LspModelState::Failed { .. } => Some(
                i18n::t("code.footer.server_error").replace("{server_name}", &server.server_name()),
            ),
        }
    }
    ...
    fn workspace_cta_message(&self) -> Option<(Option<String>, bool)> {
        ...
        if has_cta {
            let root_name = root_path
                ...
                .and_then(|s| s.to_str())
                .map(str::to_owned)
                .unwrap_or_else(|| i18n::t("code.footer.this_workspace"));
            Some((
                Some(
            ...
            for server in &live {
                ...
                if let LspModelState::Failed { error } = server_ref.state() {
                    return (
                        Some(
                            i18n::t("code.footer.server_message")
                                .replace("{server_name}", &server_ref.server_name())
        ...
        match &self.mode {
            ...
            FooterMode::SingleFile {
                ...
                LSPEnablementResultForFile::UnsupportedLanguage => (
                    Some(i18n::t(
                        "code.footer.language_support_unavailable_file_type",
                    )),
                    false,
                ),
                LSPEnablementResultForFile::LSPNotEnabled { root_name } => match lsp_repo_status {
                    LspRepoStatus::CheckingForInstallation => (
                        Some(i18n::t("code.footer.language_support_not_enabled").replace(
                            "{root_name}",
                            &root_name.unwrap_or_else(|| i18n::t("code.footer.this_codebase")),
                        )),
                        false,
                    ),
                    LspRepoStatus::Ready | LspRepoStatus::Enabled => (
                        Some(i18n::t("code.footer.language_server_unavailable_codebase")),
                        false,
                    ),
                    LspRepoStatus::DisabledAndNotInstalled { .. }
                    | LspRepoStatus::DisabledAndInstalled { .. } => (
                    ...
                    LspRepoStatus::Installing { server_type } => (
                        Some(
                            i18n::t("code.footer.installing_server")
                                .replace("{server_name}", server_type.binary_name()),
                        ),
                        false,
                    ),
            ...
            FooterMode::Workspace {
                ...
                (
                    Some(
                        i18n::t("code.footer.language_support_unavailable")
                            .replace("{root_name}", &root_name),
                    ),
                    false,
                )
            ...
            footer_content.add_child(
                Shrinkable::new(
                    ...
                    Self::render_status_text(
                        theme,
                        appearance,
                        i18n::t("code.footer.use_oz_to_update_config"),
                    ),
                )
                .finish(),
            );
