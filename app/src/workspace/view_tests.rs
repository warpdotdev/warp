...
fn test_set_tab_color_targets_specific_tab() {
    let tab0_pane_group_id = EntityId::from_usize(10);
    let tab1_pane_group_id = EntityId::from_usize(20);
    let missing_pane_group_id = EntityId::from_usize(30);

    assert_eq!(
        Workspace::tab_index_for_pane_group_ids(
            [tab0_pane_group_id, tab1_pane_group_id],
            tab1_pane_group_id,
        ),
        Some(1),
        "the targeted background tab should be selected by owning pane group id",
    );
    assert_eq!(
        Workspace::tab_index_for_pane_group_ids(
            [tab0_pane_group_id, tab1_pane_group_id],
        ...
        Some(0),
        "the active tab is only selected when its own pane group emitted the event",
    );
    assert_eq!(
        Workspace::tab_index_for_pane_group_ids(
            [tab0_pane_group_id, tab1_pane_group_id],
            missing_pane_group_id,
        ),
        None,
        "unknown pane groups should not recolor any tab",
    );
}
...
///
/// This test keeps the event-to-action conversion pinned so the event handler
/// continues to call the action handler directly with the source pane group id.
#[test]
fn test_set_tab_color_via_pane_group_event_targets_owning_tab() {
    let pane_group_id = EntityId::from_usize(20);
    let color = SelectedTabColor::Color(AnsiColorIdentifier::Magenta);

    match Workspace::set_tab_color_for_pane_group_action(pane_group_id, color) {
        WorkspaceAction::SetTabColorForPaneGroup {
            pane_group_id: actual_pane_group_id,
            color: actual_color,
        } => {
            assert_eq!(actual_pane_group_id, pane_group_id);
            assert_eq!(actual_color, color);
        }
        action => panic!("expected SetTabColorForPaneGroup, got {action:?}"),
    }

    match Workspace::set_tab_color_for_pane_group_action(pane_group_id, SelectedTabColor::Cleared) {
        WorkspaceAction::SetTabColorForPaneGroup {
            pane_group_id: actual_pane_group_id,
            color: actual_color,
        } => {
            assert_eq!(actual_pane_group_id, pane_group_id);
            assert_eq!(actual_color, SelectedTabColor::Cleared);
        }
        action => panic!("expected SetTabColorForPaneGroup, got {action:?}"),
    }

    match Workspace::set_tab_color_for_pane_group_action(pane_group_id, SelectedTabColor::Unset) {
        WorkspaceAction::SetTabColorForPaneGroup {
            pane_group_id: actual_pane_group_id,
            color: actual_color,
        } => {
            assert_eq!(actual_pane_group_id, pane_group_id);
            assert_eq!(actual_color, SelectedTabColor::Unset);
        }
        action => panic!("expected SetTabColorForPaneGroup, got {action:?}"),
    }
}
