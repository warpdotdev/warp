...
// Regression coverage: a cross-window content transfer must not let the closing window's stale
// `TabData` get pushed onto the undo-close stack, since it still references a `PaneGroup` already
// adopted by another, still-open window.
#[test]
fn content_transferred_close_is_marked_and_consumed_once() {
    let window_id = WindowId::from_usize(1);
    let mut drag = CrossWindowTabDrag::new();

    // Not marked yet: an ordinary window close should not be mistaken for a content transfer.
