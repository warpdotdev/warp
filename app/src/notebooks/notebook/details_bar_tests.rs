...
fn test_editor_display_name() {
    App::test((), |mut app| async move {
        ...
        app.read(|ctx| {
            ...
            assert_eq!(
                &editor_display_name(Some("unknown.io"), ctx),
                "unknown.io"
            );

            // If the profile is missing a display name, default to the email.
            assert_eq!(
                &editor_display_name(Some("anon.io"), ctx),
                "anon.io"
            );

            // If there's a display name available, use that.
            assert_eq!(
                &editor_display_name(Some("editor.io"), ctx),
                "The Editor"
            );
        });
    })
