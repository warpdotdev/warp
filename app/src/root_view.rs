...
pub struct RootView {
    ...
    /// BYOK "Add API key" modal, hosted here so the onboarding crate (which can't
    /// reference the app-crate editor/`ApiKeyManager`) can trigger it via an event.
...
impl RootView {
    ...
    fn create_agent_onboarding_view(
        ...
        // Gate the AI-access slide's "Next" on having a BYOK key/endpoint: seed
        // from the current `ApiKeyManager` state and keep it in sync as the user
        // adds keys/endpoints via the onboarding BYOK modals.
        let byok_configured = ApiKeyManager::as_ref(ctx).keys().has_any_key();
        onboarding_view.update(ctx, |view, ctx| {
            view.set_byok_configured(byok_configured, ctx);
        });
        ...
        ctx.subscribe_to_model(
            ...
            move |_, api_key_manager, event, ctx| {
                if matches!(event, ApiKeyManagerEvent::KeysUpdated) {
                    let byok_configured = api_key_manager.as_ref(ctx).keys().has_any_key();
    ...
    fn handle_agent_onboarding_event(
        ...
        match event {
            ...
            AgentOnboardingEvent::AddApiKeyRequested => {
                let modal = ctx.add_typed_action_view(ProviderKeysModalView::new);
                ctx.subscribe_to_view(&modal, |me, _, event, ctx| match event {
                    ...
                    ProviderKeysModalEvent::Save {
                        ...
                        ApiKeyManager::handle(ctx).update(ctx, |manager, ctx| {
                            if let Some(key) = openai {
                                manager.set_openai_key(Some(key), ctx);
                            }
                            if let Some(key) = anthropic {
                                manager.set_anthropic_key(Some(key), ctx);
                            }
                            if let Some(key) = google {
                                manager.set_google_key(Some(key), ctx);
                            }
                        });
                        me.add_api_key_modal = None;
                        me.focus(ctx);
            ...
            AgentOnboardingEvent::AddCustomEndpointRequested => {
                ...
                ctx.subscribe_to_view(&body, |me, _, event, ctx| match event {
                    ...
                    | CustomEndpointModalEvent::RemoveEndpoint { .. } => {}
                });

                let body_for_modal = body.clone();
                let modal = ctx.add_typed_action_view(move |ctx| {
                    Modal::new(Some("Add custom endpoint".to_string()), body_for_modal, ctx)
