...
use warpui::elements::{
    ...
    MainAxisAlignment, MainAxisSize, ParentElement as _, Point, Radius, Text,
};
...
/// The conversation transcript: a column of styled entries anchored to the
/// bottom (newest just above the input). Must be placed in a bounded slot.
...
fn keydown_to_key(
    ...
    if !candidate.is_empty() && candidate.chars().all(|c| !c.is_control()) {
        Some(KeyPress::Char(candidate.to_owned()))
    } else {
        None
    }
}
