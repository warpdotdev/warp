...
impl TuiAgentBridge {
    ...
    fn render_message(&mut self, message: &api::Message, append: bool) {
        match message.message.as_ref() {
            ...
            Some(MessageBody::ToolCall(tool_call)) => {
                self.upsert_tool_call_entry(message, &tool_call.tool_call_id);
            }
            Some(MessageBody::ToolCallResult(result)) => {
                self.mark_tool_call_succeeded(&result.tool_call_id);
            }
            _ => {}
        }
    }
