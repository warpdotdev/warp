...
/// runnable only through [`Self::confirm_install`], after the frontend has
/// collected values and presented its final confirmation.
...
impl TuiMcpManager {
    ...
    pub fn prepare_install(
        ...
        let (server, instructions, source) = match id {
            TuiMcpServerId::SyncedTemplate(template_uuid) => {
                let server = TemplatableMCPServerManager::as_ref(ctx)
                    .get_templatable_mcp_server(template_uuid)
                    .cloned()
                    .ok_or_else(|| "The synced MCP template is no longer available".to_owned())?;
                (server, None, TuiMcpServerSource::SyncedTemplate)
            }
            TuiMcpServerId::Gallery(gallery_uuid) => {
                let gallery = MCPGalleryManager::as_ref(ctx)
                    .get_gallery_item(gallery_uuid)
                    .ok_or_else(|| "The gallery MCP is no longer available".to_owned())?;
                let instructions = gallery.instructions_in_markdown().cloned();
                let server = MCPGalleryManager::as_ref(ctx)
                    .get_templatable_mcp_server(gallery_uuid)
                    .cloned()
                    .ok_or_else(|| "The gallery MCP template is no longer available".to_owned())?;
                (server, instructions, TuiMcpServerSource::Gallery)
            }
    ...
    /// Installs and starts an available template after the frontend's final
    /// confirmation. Catalog refresh and selection never call this method.
