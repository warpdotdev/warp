...
impl ManagedSecretsClient for ServerApi {
    ...
    async fn list_harness_auth_secrets(
        ...
        let Some(harness_input) = Option::<
            black_graphql::queries::list_harness_auth_secrets::AgentHarnessInput,
        >::from(harness) else {
            return Ok(vec![]);
        };
        ...
        match response.harness_auth_secrets {
            black_graphql::queries::list_harness_auth_secrets::HarnessAuthSecretsResult::HarnessAuthSecretsOutput(output) => {
                Ok(output.managed_secrets)
            }
            black_graphql::queries::list_harness_auth_secrets::HarnessAuthSecretsResult::UserFacingError(error) => {
                Err(anyhow!(get_user_facing_error_message(error)))
            }
            black_graphql::queries::list_harness_auth_secrets::HarnessAuthSecretsResult::Unknown => {
    ...
    async fn issue_task_identity_token(
        &self,
        options: black_managed_secrets::client::IdentityTokenOptions,
