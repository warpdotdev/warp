...
/// The client used to take `workspaces[0]` unconditionally, disregarding which team the
/// caller actually asked about. Selecting by `uid` fixes that regardless of whether the
/// server's entries currently carry distinct content per team.
#[test]
...
/// A team the response doesn't include must fail the request rather than fall back to
/// another entry positionally — the fallback this replaces was the original `workspaces[0]`
/// defect wearing a fix's clothing.
#[test]
