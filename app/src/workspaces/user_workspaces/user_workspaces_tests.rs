...
#[test]
fn test_fetch_discovery_options_rejection_preserves_empty_model_and_emits_event() {
    App::test((), |mut app| async move {
        let mut team_client = MockTeamClient::new();
        team_client
            .expect_get_discovery_options()
            .times(1)
            .return_once(|| Err(anyhow::anyhow!("discovery unavailable")));
        app.add_singleton_model(|ctx| {
            UserWorkspaces::mock(
                Arc::new(team_client),
                Arc::new(MockWorkspaceClient::new()),
        ...
        app.update(|ctx| {
            ctx.subscribe_to_model(&user_workspaces, move |_, event, _| {
                if let UserWorkspacesEvent::FetchDiscoveryOptionsRejected(err) = event {
                    let _ = sender.try_send(err.to_string());
                }
            });
        });
        ...
        assert_eq!(
            receiver
                .recv()
                .await
                .expect("expected discovery-options rejection event"),
...
#[test]
fn test_join_workspace_from_discovery_without_team_forwards_target_and_updates_workspace() {
    App::test((), |mut app| async move {
        ...
        team_client
            ...
            .withf(move |actual_workspace_uid, actual_team_uid| {
                *actual_workspace_uid == workspace_uid && actual_team_uid.is_none()
            })
            .times(1)
        ...
        user_workspaces.update(&mut app, |user_workspaces, ctx| {
            user_workspaces.join_workspace_from_discovery(workspace_uid, None, ctx);
        });
...
#[test]
fn test_join_workspace_from_discovery_rejection_preserves_empty_workspace_and_emits_event() {
    App::test((), |mut app| async move {
        let workspace_uid: WorkspaceUid = ServerId::from(10).into();
        let mut team_client = MockTeamClient::new();
        team_client
            ...
            .times(1)
            .return_once(|_, _| Err(anyhow::anyhow!("workspace join rejected")));
        app.add_singleton_model(PrivacySettings::mock);
        app.add_singleton_model(|ctx| {
            UserWorkspaces::mock(
                Arc::new(team_client),
        ...
        app.update(|ctx| {
            ctx.subscribe_to_model(&user_workspaces, move |_, event, _| {
                if let UserWorkspacesEvent::JoinWorkspaceFromDiscoveryRejected(err) = event {
                    let _ = sender.try_send(err.to_string());
                }
            });
        });
        ...
        assert_eq!(
            receiver
                .recv()
                .await
                .expect("expected workspace discovery join rejection event"),
