...
impl MemberUsageRow {
    ...
    /// Iterates the given member list so zero-usage members still get a
    /// row. Service accounts and other non-member subjects surface as
    /// extra rows at the bottom, sorted by total credits desc.
    fn for_each_member(
        entries: &[BillingCycleUsageEntry],
...
/// `members` is the roster the per-member rows are built from: the members of
/// the team currently in view, not every member of the workspace.
