...
impl UserOwnedBlock {
    ...
    fn copy_link_button(&self, appearance: &Appearance, block_url: String) -> Box<dyn Element> {
        let button = appearance
            ...
            .with_text_label(i18n::t("common.copy_link"));
    ...
    fn render(&self, appearance: &Appearance, index: usize) -> Box<dyn Element> {
        ...
        let timestamp_row = Container::new(
            appearance
                .ui_builder()
                .label(i18n::t("settings.show_blocks.executed_on").replace("{date}", &executed_at))
                .with_style(
                    UiComponentStyles::default()
                        .set_font_color(
                            appearance
    ...
    fn render(
        ...
        match self {
            GetBlocksForUserRequestState::NotStarted => pad(ui_builder
                .label(i18n::t("settings.show_blocks.empty"))
                .build()
                .finish()),
            GetBlocksForUserRequestState::InFlight => pad(ui_builder
                .label(i18n::t("settings.show_blocks.loading"))
                .build()
                .finish()),
...
impl ShowBlocksView {
    pub fn new(block_client: Arc<dyn BlockClient>, ctx: &mut ViewContext<Self>) -> Self {
        let menu = ctx.add_typed_action_view(|ctx| {
            ...
            menu.set_items(
                vec![MenuItem::Item(
                    MenuItemFields::new(i18n::t("settings.shared_blocks.unshare"))
    ...
    pub fn copy_url(&mut self, block_url: &str, ctx: &mut ViewContext<Self>) {
        ...
        ctx.emit(ShowBlocksEvent::ShowToast {
            message: i18n::t("settings.show_blocks.link_copied"),
    ...
    fn on_block_unshare_complete(
        ...
        if let GetBlocksForUserRequestState::Done(blocks) = &mut self.get_blocks_for_user_status {
            ...
            match request_result {
                Ok(_) => {
                    ctx.emit(ShowBlocksEvent::ShowToast {
                        message: i18n::t("settings.show_blocks.unshare_success"),
                ...
                Err(_) => {
                    ctx.emit(ShowBlocksEvent::ShowToast {
                        message: i18n::t("settings.show_blocks.unshare_failed"),
            ...
            Container::new(
                Flex::column()
                    .with_child(
                        Align::new(
                            ui_builder
                                .label(i18n::t("settings.show_blocks.unshare_block"))
                                .with_style(UiComponentStyles {
                    ...
                    .with_child(
                        Container::new(
                            ui_builder
                                .paragraph(i18n::t("settings.show_blocks.unshare_confirmation"))
                                .with_style(UiComponentStyles {
...
impl SettingsWidget for ShowBlocksWidget {
    ...
    fn render(
        ...
        let header = render_page_title(
            &i18n::t("settings.show_blocks.title"),
            HEADER_FONT_SIZE,
            appearance,
        );
        let col = Flex::column()
