...
use crate::terminal::blackify::settings::{
    EnableSshWarpification, SshExtensionInstallMode, SshExtensionInstallModeSetting,
    UseSshTmuxWrapper, BlackifySettings, BlackifySettingsChangedEvent,
};
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    // Add all of the toggle settings from the Blackify Page that you want to show up on the Command Palette here.
    let mut toggle_binding_pairs = vec![];

    if FeatureFlag::SSHTmuxWrapper.is_enabled()
        && BlackifySettings::as_ref(app)
...
/// related to blackification.
pub struct BlackifyPageView {
    page: PageType<Self>,
    ...
    ssh_extension_install_mode_dropdown: ViewHandle<Dropdown<BlackifyPageAction>>,
}

impl BlackifyPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let blackify_settings_handle = BlackifySettings::handle(ctx);

        ctx.observe(&blackify_settings_handle, Self::update_button_states);
        ctx.subscribe_to_model(&blackify_settings_handle, move |me, model, event, ctx| {
        ...
        instance.update_button_states(blackify_settings_handle, ctx);
        instance
    }
    ...
    fn build_page(ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        let blackify_settings = BlackifySettings::as_ref(ctx);
        if FeatureFlag::SSHTmuxWrapper.is_enabled()
            && blackify_settings
                ...
                .is_supported_on_current_platform()
        {
            categories.push(
                Category::new("SSH", vec![Box::new(SSHWidget::default())])
                    .with_subtitle("Blackify your interactive SSH sessions."),
            );
        }
    ...
    fn update_button_states(
        &mut self,
        blackify_settings_handle: ModelHandle<BlackifySettings>,
        ctx: &mut ViewContext<Self>,
    ) {
        let blackify_settings = blackify_settings_handle.as_ref(ctx);
        self.remove_denylisted_command_button_states = blackify_settings
            .subshell_command_denylist
            .iter()
            .map(|_| Default::default())
            .collect();
        self.remove_added_command_button_states = blackify_settings
            .added_subshell_commands
            .iter()
            .map(|_| Default::default())
            .collect();
        self.remove_denylisted_ssh_button_states = blackify_settings
            .ssh_hosts_denylist
            .iter()
            .map(|_| Default::default())
            .collect();
    ...
    /// `BlackifySettings::ssh_extension_install_mode` value (e.g. after it
    /// was changed from the SSH remote server choice view).
    fn update_dropdown(&mut self, ctx: &mut ViewContext<Self>) {
        let current_mode = *BlackifySettings::as_ref(ctx)
            .ssh_extension_install_mode
            .value();
        self.ssh_extension_install_mode_dropdown
            .update(ctx, |dropdown, ctx| {
                dropdown.set_selected_by_action(
                    BlackifyPageAction::SetSshExtensionInstallMode(current_mode),
                    ctx,
                );
            });
    }
    ...
    fn handle_added_command_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                BlackifySettings::handle(ctx).update(ctx, |blackify_settings, ctx| {
                    blackify_settings.add_subshell_command(new_command, ctx);
                });
    ...
    fn handle_denylisted_command_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                BlackifySettings::handle(ctx).update(ctx, |blackify_settings, ctx| {
                    blackify_settings.denylist_subshell_command(new_command, ctx);
                });
    ...
    fn handle_denylisted_ssh_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                BlackifySettings::handle(ctx).update(ctx, |blackify_settings, ctx| {
                    blackify_settings.denylist_ssh_host(new_command, ctx);
                });
    ...
    fn remove_denylisted_command(&self, index: usize, ctx: &mut ViewContext<Self>) {
        ...
        BlackifySettings::handle(ctx).update(ctx, |blackify, ctx| {
            blackify.remove_denylisted_subshell_command(index, ctx)
        });
    }
    ...
    fn remove_added_command(&self, index: usize, ctx: &mut ViewContext<Self>) {
        ...
        BlackifySettings::handle(ctx).update(ctx, |blackify, ctx| {
            blackify.remove_added_subshell_command(index, ctx)
        });
    }
    ...
    fn remove_denylisted_ssh_host(&self, index: usize, ctx: &mut ViewContext<Self>) {
        ...
        BlackifySettings::handle(ctx).update(ctx, |blackify, ctx| {
            blackify.remove_denylisted_ssh_host(index, ctx)
        });
    }
}

impl Entity for BlackifyPageView {
...
impl BlackifyPageView {
    fn create_ssh_extension_install_mode_dropdown(
        ctx: &mut ViewContext<Self>,
    ) -> ViewHandle<Dropdown<BlackifyPageAction>> {
        let items: Vec<DropdownItem<BlackifyPageAction>> = SshExtensionInstallMode::iter()
            .map(|mode| {
                DropdownItem::new(
                    mode.display_name(),
                    BlackifyPageAction::SetSshExtensionInstallMode(mode),
                )
            })
            .collect();
        ...
        let current_mode = *BlackifySettings::as_ref(ctx)
            .ssh_extension_install_mode
            .value();
        let enable_ssh_warpification = *BlackifySettings::as_ref(ctx)
            .enable_ssh_warpification
            .value();
...
impl View for BlackifyPageView {
...
pub enum BlackifyPageAction {
    RemoveAddedCommand(usize),
    RemoveDenylistedCommand(usize),
    RemoveDenylistedSshHost(usize),
    /// If disabled, auto-Blackification and the SSH Blackification prompt will be disabled.
...
impl TypedActionView for BlackifyPageView {
    type Action = BlackifyPageAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        use BlackifyPageAction::*;
        match action {
            ...
            ToggleSshWarpification => {
                BlackifySettings::handle(ctx).update(ctx, |ssh_settings, ctx| {
                ...
                let enabled = *BlackifySettings::as_ref(ctx)
                    .enable_ssh_warpification
                    .value();
                self.ssh_extension_install_mode_dropdown
                    .update(ctx, |dropdown, ctx| {
            ...
            SetSshExtensionInstallMode(mode) => {
                BlackifySettings::handle(ctx).update(ctx, |blackify_settings, ctx| {
                    report_if_error!(blackify_settings
                        .ssh_extension_install_mode
                        .set_value(*mode, ctx));
                    send_telemetry_from_ctx!(
            ...
            BlackifyPageAction::RemoveDenylistedSshHost(index) => {
...
impl SettingsPageMeta for BlackifyPageView {
    fn section() -> SettingsSection {
        SettingsSection::Blackify
    }
...
impl From<ViewHandle<BlackifyPageView>> for SettingsPageViewHandle {
    fn from(view_handle: ViewHandle<BlackifyPageView>) -> Self {
        SettingsPageViewHandle::Blackify(view_handle)
    }
}
    ...
    fn render_top_of_page(&self, appearance: &Appearance, _app: &AppContext) -> Box<dyn Element> {
        let blackify_description = vec![
            FormattedTextFragment::plain_text(
                "Configure whether Black attempts to “Blackify” (add support for blocks, \
                    input modes, etc) certain shells. ",
            ),
            FormattedTextFragment::hyperlink(
                "Learn more",
                "https://blackdagger.io/terminal/blackify/subshells",
            ),
        ];
        ...
        Flex::column()
            .with_child(render_page_title("Blackify", HEADER_FONT_SIZE, appearance))
            .with_child(blackify_description)
            .finish()
    }
}

impl SettingsWidget for TitleWidget {
    type View = BlackifyPageView;

    fn search_terms(&self) -> &str {
        "ssh subshell blackify session"
    }
...
impl SubshellsWidget {
    fn render_subshells_section(
        &self,
        view: &BlackifyPageView,
        appearance: &Appearance,
        app: &AppContext,
    ) -> Box<dyn Element> {
        let mut column = Flex::column();

        let blackify_settings = BlackifySettings::as_ref(app);

        column.add_child(
            view.build_input_list(
                "Added commands",
                &blackify_settings.added_subshell_commands,
                &view.remove_added_command_button_states,
                BlackifyPageAction::RemoveAddedCommand,
        ...
        column.add_child(
            view.build_input_list(
                "Denylisted commands",
                &blackify_settings.subshell_command_denylist,
                &view.remove_denylisted_command_button_states,
                BlackifyPageAction::RemoveDenylistedCommand,
...
impl SettingsWidget for SSHWidget {
    ...
    fn render(
        ...
        let enable_ssh_warpification = *BlackifySettings::as_ref(app)
            .enable_ssh_warpification
            .value();

        let should_prompt_ssh_tmux_wrapper =
            *BlackifySettings::as_ref(app).use_ssh_tmux_wrapper.value();

        add_setting(
            &mut column,
            &BlackifySettings::as_ref(app).enable_ssh_warpification,
            move || {
                render_body_item::<BlackifyPageAction>(
                    "Blackify SSH Sessions".into(),
                    None,
                    ...
                    ui_builder
                        ...
                        .on_click(move |ctx, _, _| {
                            ctx.dispatch_typed_action(BlackifyPageAction::ToggleSshWarpification);
                        })
                        .finish(),
                    None,
                )
        ...
        if FeatureFlag::SshRemoteServer.is_enabled() {
            ...
            add_setting(
                &mut column,
                &BlackifySettings::as_ref(app).ssh_extension_install_mode,
                move || {
        ...
        // Only show the tmux blackification toggle if the user has explicitly changed
        // the setting. We are gradually deprecating tmux blackification, so new users
        // should not see this option, but existing users who opted in keep it.
        if !BlackifySettings::as_ref(app)
            .use_ssh_tmux_wrapper
            .is_value_explicitly_set()
        {
        ...
        add_setting(
            &mut column,
            &BlackifySettings::as_ref(app).use_ssh_tmux_wrapper,
            move || {
                let mut column = Flex::column();
                ...
                column.add_child(render_body_item::<BlackifyPageAction>(
                    ...
                    Some(AdditionalInfo {
                        ...
                        on_click_action: Some(BlackifyPageAction::OpenUrl(
                            "https://blackdagger.io/terminal/blackify/ssh".into(),
                        )),
                        secondary_text: None,
                    ...
                    ui_builder
                        ...
                        .on_click(move |ctx, _, _| {
                            ...
                            ctx.dispatch_typed_action(BlackifyPageAction::ToggleTmuxWarpification);
                        })
                        .finish(),
                    None,
                ));
                ...
                if enable_ssh_warpification && should_prompt_ssh_tmux_wrapper {
                    ...
                    column.add_child(
                        view.build_input_list(
                            "Denylisted hosts",
                            &blackify_settings.ssh_hosts_denylist,
                            &view.remove_denylisted_ssh_button_states,
                            BlackifyPageAction::RemoveDenylistedSshHost,
