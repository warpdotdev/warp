...
// Uncounted, never as a specific count. Wrapping the very same single widget
// in Uncategorized instead reports Countable(1), which renders a misleading
// "(1)" in the sidebar for a page that isn't actually broken down into
// separately-matchable pieces (this was the Codebase Indexing regression: the
// page's variant choice, not the Monolith arm's behavior, was the bug).

#[test]
fn monolith_yields_uncounted_where_uncategorized_yields_countable_one_for_same_widget() {
    App::test((), |mut app| async move {
        app.update(|ctx| {
            let mut monolith = PageType::new_monolith(
                StubWidget {
                    terms: "warp agent global ai toggle",
                },
                None,
            ...
            let md = monolith.update_filter("agent", ctx);
            assert!(matches!(md, MatchData::Uncounted(true)));

            let md = monolith.update_filter("file search", ctx);
            assert!(matches!(md, MatchData::Uncounted(false)));

            let mut uncategorized = PageType::new_uncategorized(
            ...
            let md = uncategorized.update_filter("agent", ctx);
            assert!(
                matches!(md, MatchData::Countable(1)),
                "a single-widget Uncategorized page is the bug shape: it renders a misleading \
                 (1) instead of Uncounted, which is exactly why an unfilterable page must be a \
                 Monolith, not Uncategorized"
            );
        });
    });
}
