...
fn button_retry() -> &'static str {
    crate::menu_label("settings.environments_page.button_retry", "Retry")
}
fn button_authorize() -> &'static str {
...
impl EnvironmentsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let search_editor = Self::create_single_line_editor(search_placeholder(), ctx);
    ...
    fn handle_update_manager_event(
            ...
            (&result.operation, &result.success_type)
        {
            ...
            if should_handle {
                ...
                self.show_success_toast(toast_updated_success().to_string(), ctx);
            ...
            && let Some(pending_client_id) = self.pending_create_client_id.take()
        {
            ...
            {
                self.show_success_toast(toast_created_success().to_string(), ctx);
            }
        }
            ...
            && let Some(pending_env_id) = self.pending_delete_env_id.take()
        {
            ...
            {
                self.show_success_toast(toast_deleted_success().to_string(), ctx);
            }
        }
        ...
        if matches!(&result.operation, ObjectOperation::MoveToDrive) {
            ...
            if matches!(result.success_type, OperationSuccessType::Success) {
                self.show_success_toast(toast_shared_success().to_string(), ctx);
            } else {
                self.show_error_toast(toast_share_failed().to_string(), ctx);
            }
...
impl EnvironmentsPageWidget {
    ...
    fn render_empty_state(
        ...
        let (github_button_label, github_button_enabled) = if dropdown_state.is_loading {
            ...
            (button_authorize(), true)
        } else {
            (button_get_started(), true)
        };
        ...
        let github_row = Self::render_empty_state_row(
            ...
            EmptyStateRowConfig {
                ...
                badge: Some(quick_setup_badge()),
                subtitle: quick_setup_subtitle(),
        ...
        let local_repos_row = Self::render_empty_state_row(
            ...
            EmptyStateRowConfig {
                icon: Icon::Terminal,
                title: use_agent_title(),
                badge: None,
                subtitle: use_agent_subtitle(),
        ...
        Hoverable::new(card_hover_mouse_state, move |state| {
            ...
            if should_render_share_button {
                ...
                let share_button_element = if is_card_hovered {
                    ...
                    .with_tooltip(move || {
                        share_ui_builder
                            .tool_tip(tooltip_share().to_string())
                            .build()
                            .finish()
                    })
                    .build()
            ...
            if is_card_hovered {
                edit_button = edit_button.with_tooltip(move || {
                    edit_ui_builder
                        .tool_tip(tooltip_edit().to_string())
                        .build()
                        .finish()
                });
            }
