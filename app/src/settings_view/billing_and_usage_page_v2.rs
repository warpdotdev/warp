...
impl GrantBucket {
    ...
    fn expiry_label(&self) -> String {
            ...
            .all(|e| e.date_naive() == first.date_naive())
        {
            ...
            format!(
                "{} {}",
                i18n::t("settings.billing.balance.expires_prefix"),
                local.format("%b %d, %Y")
            )
        } else {
            String::new()
...
impl BillingAndUsagePageV2View {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let addon_credit_modal_view = ctx.add_typed_action_view(|ctx| {
            Modal::new(
                Some(i18n::t("settings.billing.spending_limit_modal.title")),
                addon_credit_modal,
                ctx,
            )
        ...
        let load_more_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(
                i18n::t("settings.billing.usage_history.load_more"),
                SecondaryTheme,
            )
            .on_click(|ctx| {
    ...
    fn handle_workspaces_event(
        ...
        match event {
            ...
            UserWorkspacesEvent::UpdateWorkspaceSettingsRejected(_err) => {
                ...
                let msg = i18n::t("settings.billing.toast.update_settings_failed");
                self.show_toast(&msg, ToastFlavor::Error, ctx);
            }
            ...
            UserWorkspacesEvent::PurchaseAddonCreditsSuccess => {
                ...
                let msg = i18n::t("settings.billing.toast.purchase_success");
                self.show_toast(&msg, ToastFlavor::Success, ctx);
    ...
    fn render_plan_section(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        plan_header.add_child(
            Text::new_inline(
                i18n::t("settings.billing.plan.header"),
        ...
        if let Some(team) = workspaces.current_team() {
            ...
            right_side.add_child(
                Container::new(render_customer_type_badge(
                    appearance,
                    i18n::t("settings.billing.plan.free_badge"),
                ))
                .with_margin_right(8.)
                .finish(),
            );
            ...
            cards_row.add_child(
                Expanded::new(
                    ...
                    render_balance_card(
                        ...
                        &i18n::t("settings.billing.balance.base_credits"),
                        &reset_str,
                        base_remaining,
                        outline_color,
                    ),
            ...
            cards_row.add_child(
                Expanded::new(
                    ...
                    render_balance_card(
                        ...
                        &i18n::t("settings.billing.balance.personal_credits"),
            ...
            cards_row.add_child(
                Expanded::new(
                    ...
                    render_balance_card(
                        appearance,
                        BONUS_CREDITS_DOT_COLOR,
                        &i18n::t("settings.billing.balance.team_credits"),
        ...
        Some(
            Flex::column()
                .with_child(
                    Container::new(
                        Text::new_inline(
                            i18n::t("settings.billing.balance.header"),
                            appearance.ui_font_family(),
                            HEADER_FONT_SIZE,
                        )
                        .with_style(Properties::default().weight(Weight::Bold))
    ...
    fn render_ambient_agent_trial_widget(
        ...
        let title = Text::new_inline(
            i18n::t("settings.billing.ambient_trial.title"),
            appearance.ui_font_family(),
            14.,
        )
        .with_color(theme.active_ui_text_color().into())
        ...
        let credits_text = if credits_remaining == 1 {
            i18n::t("settings.billing.ambient_trial.credits_remaining_one")
        } else {
            format!(
                "{} {}",
                credits_remaining.separate_with_commas(),
                i18n::t("settings.billing.ambient_trial.credits_remaining_suffix")
            )
        };
        ...
        if credits_remaining >= AMBIENT_AGENT_TRIAL_CREDIT_THRESHOLD {
            let new_agent_button = ui_builder
                ...
                .with_text_label(i18n::t("settings.billing.ambient_trial.new_agent_button"))
                .with_style(UiComponentStyles {
                    font_color: Some(bg),
        ...
        if !is_on_paid_plan {
            ...
            let buy_more_button = ui_builder
                ...
                .with_text_label(i18n::t("settings.billing.ambient_trial.buy_more_button"))
                .with_style(UiComponentStyles {
    ...
    fn addon_credits_panel_state(
            ...
            if !has_admin_permissions {
                ...
                return AddonCreditsPanelState::IneligiblePlan(
                    AddonCreditsRestriction::UpgradeToBuild {
                        link_text: i18n::t("settings.billing.addon.upgrade_to_build_link"),
        ...
        let description_text = if team_count > 1 {
            format!(
                "{} {}",
                i18n::t("settings.billing.addon.description"),
                i18n::t("settings.billing.addon.description_team_suffix")
            )
        } else {
            i18n::t("settings.billing.addon.description")
        };
        ...
        let price_label = selected_credit_option
            .map(|opt| {
                ...
                i18n::t("settings.billing.addon.price_label")
        ...
        let auto_reload_credit_amount = selected_credit_option
            .map(|o| {
                format!(
                    "{} {}",
                    o.credits.separate_with_commas(),
                    i18n::t("settings.billing.addon.credits_unit")
                )
            })
            .unwrap_or_else(|| i18n::t("settings.billing.addon.selected_credit_amount_fallback"));
        let auto_reload_tooltip_text = i18n::t("settings.billing.addon.auto_reload_tooltip")
            .replace("{amount}", &auto_reload_credit_amount);
        let warning_text: Option<String> = if delinquent && has_admin_permissions {
            Some(i18n::t("settings.billing.addon.warning_delinquent_admin"))
        } else if delinquent {
            Some(i18n::t(
                "settings.billing.addon.warning_delinquent_non_admin",
            ))
        } else if workspace
            .billing_metadata
            .has_failed_addon_credit_auto_reload_status()
        {
            Some(if has_admin_permissions {
                i18n::t("settings.billing.addon.warning_failed_reload_admin")
            } else {
                i18n::t("settings.billing.addon.warning_failed_reload_non_admin")
            })
        } else if would_exceed {
            Some(match (auto_reload_enabled, has_admin_permissions) {
                (true, true) => i18n::t("settings.billing.addon.warning_auto_reload_paused_admin"),
                (true, false) => {
                    i18n::t("settings.billing.addon.warning_auto_reload_paused_non_admin")
                }
                (false, true) => i18n::t("settings.billing.addon.warning_purchase_exceeds_admin"),
                (false, false) => {
                    i18n::t("settings.billing.addon.warning_purchase_exceeds_non_admin")
                }
            })
        } else {
            None
        ...
        if !has_admin_permissions && auto_reload_enabled {
            ...
            let description_text = match configured_auto_reload_option {
                Some(option) => {
                    ...
                    i18n::t("settings.billing.addon.non_admin_auto_reload_description_with_amount")
                        .replace("{credits}", &credits)
                        .replace("{price}", &price)
                }
                None => i18n::t("settings.billing.addon.non_admin_auto_reload_description"),
            };
    ...
    fn render_addon_credits_ineligible_plan_card(
        ...
        let explanation = match restriction {
                ...
                FormattedTextElement::new(
                    FormattedText::new([FormattedTextLine::Line(vec![
                        ...
                        FormattedTextFragment::plain_text(i18n::t(
                            "settings.billing.addon.upgrade_to_build_suffix",
                        )),
                    ])]),
            ...
            AddonCreditsRestriction::ContactAccountExecutive => appearance
                .ui_builder()
                .paragraph(i18n::t("settings.billing.addon.contact_account_executive"))
                .with_style(UiComponentStyles {
            ...
            AddonCreditsRestriction::ContactTeamAdmin => appearance
                .ui_builder()
                .paragraph(i18n::t("settings.billing.addon.contact_team_admin"))
                .with_style(UiComponentStyles {
                    font_color: Some(theme.sub_text_color(bg).into()),
                    ..Default::default()
                })
        ...
        let header = Text::new_inline(
            i18n::t("settings.billing.addon.buy_credits_header"),
            appearance.ui_font_family(),
            HEADER_FONT_SIZE,
        )
    ...
    fn render_addon_credits_non_admin_auto_reload_card(
        ...
        let auto_reload_header = Text::new_inline(
            i18n::t("settings.billing.addon.auto_reload_enabled_header"),
            appearance.ui_font_family(),
            HEADER_FONT_SIZE,
        )
        .with_color(theme.foreground().into())
        ...
        if state.has_admin_permissions {
            let info_icon = render_info_icon(
                ...
                AdditionalInfo::<BillingAndUsagePageAction> {
                    ...
                    secondary_text: None,
                    tooltip_override_text: Some(i18n::t(
                        "settings.billing.addon.monthly_spend_limit_tooltip",
                    )),
                },
            );
            ...
            let spend_row = Flex::row()
                ...
                .with_children([
                    ui_builder
                        .span(i18n::t("settings.billing.addon.monthly_spend_limit_label"))
                        .build()
                        .finish(),
    ...
    fn render_purchased_this_month_row(
        ...
        let label = Text::new_inline(
            i18n::t("settings.billing.addon.purchased_this_month"),
            appearance.ui_font_family(),
            12.,
        )
        .with_color(theme.active_ui_text_color().into())
        ...
        let credits_text = if credits_purchased == 1 {
            i18n::t("settings.billing.addon.credits_count_one")
        } else {
            format!(
                "{} {}",
                credits_purchased.separate_with_commas(),
                i18n::t("settings.billing.addon.credits_unit")
            )
        };
    ...
    fn render_addon_credits_lower_section(
        ...
        let purchase_button_label = if self.addon_credits.purchase_loading {
            i18n::t("settings.billing.addon.purchase_button_loading")
        } else {
            i18n::t("settings.billing.addon.purchase_button")
        };
        ...
        if state.has_admin_permissions {
            ...
            right_group.add_children([
                Text::new_inline(
                    i18n::t("settings.billing.addon.auto_reload_label"),
                    appearance.ui_font_family(),
                    14.,
                )
                .with_color(fg.into())
    ...
    fn render_usage_history_tab(
        ...
        let mut content = Flex::column()
            ...
            .with_child(
                Container::new(
                    Text::new_inline(
                        i18n::t("settings.billing.usage_history.last_30_days"),
                        appearance.ui_font_family(),
                        14.,
                    )
        ...
        if loading {
            ...
            let zero = Flex::column()
                ...
                .with_child(
                    Container::new(
                        Text::new(
                            i18n::t("settings.billing.usage_history.empty_title"),
                            appearance.ui_font_family(),
                            14.,
                        )
                        .with_color(blended_colors::text_sub(
            ...
            BillingAndUsagePageAction::UpdateAutoReloadEnabled { team_uid, enabled } => {
                let auto_reload_denomination_credits = if *enabled {
                    let Some(option) = self
                        ...
                        let msg = i18n::t("settings.billing.toast.auto_reload_pricing_loading");
                        self.show_toast(&msg, ToastFlavor::Error, ctx);
                        return;
                    };
                    Some(option.credits)
                ...
                self.pending_auto_reload_toast = Some(if *enabled {
                    let credits = auto_reload_denomination_credits
                        ...
                        .unwrap_or_else(|| {
                            i18n::t("settings.billing.toast.your_selected_fallback")
                        });
                    i18n::t("settings.billing.toast.auto_reload_enabled")
                        .replace("{credits}", &credits)
                } else {
                    i18n::t("settings.billing.toast.auto_reload_disabled")
                });
                UserWorkspaces::handle(ctx).update(ctx, |ws, ctx| {
                    ws.update_addon_credits_settings(
                        *team_uid,
...
fn render_balance_card(
    ...
    let remaining_label = Text::new_inline(
        i18n::t("settings.billing.balance.remaining"),
        appearance.ui_font_family(),
        14.,
    )
    .with_color(sub_color)
