...
impl MCPServersListPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        search_editor.update(ctx, |editor, ctx| {
            ...
            editor.set_placeholder_text(i18n::t("settings.mcp.search.placeholder"), ctx);
        });
        let search_bar = ctx.add_typed_action_view(|_| SearchBar::new(search_editor.clone()));

        let add_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(i18n::t("settings.mcp.add_button"), NakedTheme)
                .with_icon(Icon::Plus)
    ...
    fn create_template_server_card(
        ...
        let server_card = ServerCardView::new(
            ...
            template
                .description
                .clone()
                .or_else(|| Some(i18n::t("settings.mcp.status.available_to_install"))),
        ...
        match update {
            MCPServerUpdate::CloudTemplate { .. } => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast =
                        DismissibleToast::success(i18n::t("settings.mcp.toast.server_updated"));
    ...
    fn render_file_based_mcp_section(
        ...
        let label = render_body_item_label::<MCPServersListPageViewAction>(
            i18n::t("settings.mcp.auto_spawn.label"),
            None,
            None,
        ...
        > = std::sync::LazyLock::new(|| {
            vec![
                FormattedTextFragment::plain_text(i18n::t("settings.mcp.auto_spawn.description")),
                FormattedTextFragment::hyperlink(
                    i18n::t("settings.mcp.auto_spawn.see_providers_link"),
    ...
    fn render_page_body(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let description_fragments = vec![
            FormattedTextFragment::plain_text(i18n::t("settings.mcp.page.description")),
            FormattedTextFragment::hyperlink(
                i18n::t("settings.mcp.page.learn_more_link"),
        ...
        if !has_any_content {
                ...
                && filtered_file_based_cards.is_empty()
            {
                ...
                if !owned_server_cards.is_empty() {
                    let my_mcps_header = i18n::t("settings.mcp.section.my_mcps");
                ...
                if !shared_server_cards.is_empty() {
                    ...
                    let shared_by_text = match team_name {
                        Some(name) => i18n::t("settings.mcp.section.shared_by_warp_and_team")
                            .replace("{name}", &name),
                        None => i18n::t("settings.mcp.section.shared_by_warp_and_devices"),
                    };
                    ...
                    let shared_from_warp_header = i18n::t("settings.mcp.section.shared_from_warp");
                    page.add_child(self.render_server_cards_section(
                        &shared_from_warp_header,
                ...
                for (provider, cards) in &filtered_file_based_cards {
                    let section_title = i18n::t("settings.mcp.section.detected_from")
                ...
                Align::new(
                    Flex::column()
                        ...
                        .with_child(
                            appearance
                                .ui_builder()
                                .wrappable_text(i18n::t("settings.mcp.no_search_results"), true)
                                .with_style(style::description_text(appearance))
                                .build()
                                .finish(),
                        )
        ...
        for provider in MCPProvider::iter() {
            ...
            let server_card = ServerCardView::new(
                ...
                installation
                    ...
                    .description
                    .clone()
                    .or_else(|| Some(i18n::t("settings.mcp.status.detected_from_config"))),
                ...
                if is_shared {
                    match creator {
                        Some(creator) => Some(TitleChip::text(
                            i18n::t("settings.mcp.chip.shared_by").replace("{creator}", &creator),
                        )),
                        None => Some(TitleChip::text(i18n::t(
                            "settings.mcp.chip.shared_by_team_member",
                        ))),
                    }
                } else if matches!(item_id, ServerCardItemId::TemplatableMCP(_)) {
                    Some(TitleChip::text(i18n::t(
                        "settings.mcp.chip.from_another_device",
                    )))
                } else {
                    None
                }
