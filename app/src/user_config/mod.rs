...
pub(crate) fn materialize_default_worktree_config(
    ...
    // the `{{autogenerated_branch_name}}` handlebars placeholder. The prefix
    // is the part that comes from the user's filesystem and may contain
    // shell metacharacters — spaces, `$`, backticks, etc. — that would
    // otherwise be re-parsed by the user's shell when `git worktree add` /
    // `cd` runs. The placeholder itself is left untouched so handlebars can
    // still resolve it at use time; autogenerated branch names are
    // shell-safe by construction.
    //
    // Without this escape, a repo path like `~/Developer/My Project` ends
    // up in the rendered command as a bare `/Users/.../My Project/<branch>`
    // and the shell splits it into separate positional args, breaking
    // `git worktree add` (#11144).
    const BRANCH_PLACEHOLDER: &str = "{{autogenerated_branch_name}}";
    let raw_worktree_path = crate::tab_configs::tab_config::generated_worktree_path_string(
        Path::new(repo_path),
        BRANCH_PLACEHOLDER,
    );
...
/// Shell-escape every byte of `worktree_path` that precedes
/// `branch_placeholder`, leaving the placeholder (and everything after it)
/// untouched.
///
/// `worktree_path` always has the shape
/// `<repo_dir>/<branch_placeholder>` because it's produced by
/// [`generated_worktree_path_string`]. The `<repo_dir>` portion comes from
/// the user's filesystem and may contain shell metacharacters; the
/// placeholder is a `{{handlebars}}` token that must survive to be
/// resolved by the tab-config render pass. Splitting at the placeholder
