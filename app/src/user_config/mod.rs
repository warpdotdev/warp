...
pub(crate) fn materialize_default_worktree_config(
    ...
    // Shell-escape the repo-derived portion of the worktree path. Without
    // this, a repo path like `~/Developer/My Project` ends up in the
    // rendered command as a bare `/Users/.../My Project/<branch>` and the
    // shell splits it into separate positional args, breaking
    // `git worktree add` (#11144). See [`build_shell_safe_worktree_path`]
    // for the precondition on `shell_family`.
    const BRANCH_PLACEHOLDER: &str = "{{autogenerated_branch_name}}";
    let worktree_path = crate::tab_configs::tab_config::build_shell_safe_worktree_path(
        Path::new(repo_path),
        BRANCH_PLACEHOLDER,
        shell_family,
    );
