...
pub(crate) fn materialize_default_worktree_config(
    ...
    // arguments (#11144). Only the literal prefix is quoted; the trailing branch-name
    // placeholder is resolved (and independently shell-quoted) later by `render_tab_config`,
    // so it must stay outside our quotes — adjacent shell quotes concatenate into one argument.
    const BRANCH_PLACEHOLDER: &str = "{{autogenerated_branch_name}}";
    let worktree_path = match worktree_path.strip_suffix(BRANCH_PLACEHOLDER) {
        Some(prefix) => shell_words::quote(prefix).into_owned() + BRANCH_PLACEHOLDER,
        None => shell_words::quote(&worktree_path).into_owned(),
    };
