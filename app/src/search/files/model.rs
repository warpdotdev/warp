...
impl FileSearchModel {
    ...
    /// Gets repository files for the current working directory.
    ///
    /// This is intended for search surfaces that cannot display directories.
    /// Keeping it separate from [`Self::get_repo_contents`] preserves directory
    /// results for callers like the AI context menu.
    #[cfg(feature = "local_fs")]
        ...
        F: for<'a> Fn(&repo_metadata::RepoContent<'a>) -> Option<String> + Send + Sync + 'static,
    {
        ...
        let args = GetContentsArgs::default();
        let args = if include_folders {
            args
        } else {
            args.exclude_folders()
        };
        args.with_filter(move |content| {
