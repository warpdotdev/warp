...
impl CommandSearchView {
    pub fn new(ai_client: Arc<dyn AIClient>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let search_bar = ctx.add_typed_action_view(|ctx| {
            SearchBar::new(
                ...
                i18n::t("search.command_search.placeholder"),
                |result_index, result| {
    ...
    fn handle_result_selected(
    ...
    ) {
        {
            ...
            let (a11y_content, a11y_help_content) = if was_immediately_executed {
                (
                    i18n::t("search.command_search.a11y.result_executed"),
                    i18n::t("search.command_search.a11y.result_executed_help"),
                )
            } else {
                (
                    i18n::t("search.command_search.a11y.result_accepted"),
                    i18n::t("search.command_search.a11y.result_accepted_help"),
                )
            };
    ...
    fn render_loading_state(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let text = appearance
            .ui_builder()
            .span(i18n::t("search.command_search.loading"))
                ...
                if team.billing_metadata.can_upgrade_to_higher_tier_plan() {
                    if has_admin_permissions {
                        ...
                        self.render_error_header_text(
                            i18n::t("search.command_search.out_of_credits_contact_admin"),
                            appearance,
                        )
                    }
                } else {
    ...
    fn render_error_header_with_upgrade_link(
            ...
            .is_anonymous_or_logged_out()
        {
            appearance
                .ui_builder()
                .link(
                    i18n::t("search.command_search.upgrade"),
                    None,
        ...
        row.add_child(
            appearance
                .ui_builder()
                .span(i18n::t("search.command_search.out_of_credits_prefix"))
                .with_style(UiComponentStyles {
                    font_size: Some(appearance.monospace_font_size()),
                    font_family_id: Some(appearance.ui_font_family()),
                    font_color: Some(appearance.theme().nonactive_ui_text_color().into()),
        ...
        row.add_child(
            appearance
                .ui_builder()
                .span(i18n::t("search.command_search.out_of_credits_suffix"))
                .with_style(UiComponentStyles {
                    font_size: Some(appearance.monospace_font_size()),
                    font_family_id: Some(appearance.ui_font_family()),
                    font_color: Some(appearance.theme().nonactive_ui_text_color().into()),
    ...
    fn render_results(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        match (&query_result_renderers, selected_index) {
            (Some(query_result_renderers), _) if query_result_renderers.is_empty() => {
                ...
                let text = appearance
                    .ui_builder()
                    .span(i18n::t("search.command_search.no_results"))
                    .with_style(UiComponentStyles {
                        font_size: Some(appearance.monospace_font_size()),
                        font_family_id: Some(appearance.ui_font_family()),
                        font_color: Some(appearance.theme().nonactive_ui_text_color().into()),
...
impl View for CommandSearchView {
    ...
    fn accessibility_contents(&self, _ctx: &AppContext) -> Option<AccessibilityContent> {
        Some(AccessibilityContent::new(
            i18n::t("search.command_search.a11y.title"),
            i18n::t("search.command_search.a11y.help"),
