...
pub fn build_tab_config(
    ...
    if enable_worktree {
        if autogenerate_worktree_branch_name {
            ...
            // Double-quote the worktree path so a repo/worktree directory whose
            // name contains spaces is not word-split by the shell. Pragmatic
            // 80-20 fix for APP-4982 / GH#11144 — not a full shell-agnostic
            // escaping solution (branch names cannot contain spaces).
