    ...
    F: Fn(&mut EventContext, Vector2F) + 'static,
{
    ...
    let dir_display =
        black_util::path::user_friendly_path(&raw_path, home_dir.as_ref().and_then(|h| h.to_str()))
            .into_owned();
...
where
    F: Fn(&mut black_ui::EventContext, black_ui::geometry::vector::Vector2F) + 'static,
{
