...
/// Renders `config` and asserts both generated worktree commands parse with the spaced
/// worktree path as exactly one shell argument (#11144).
fn assert_worktree_commands_shell_quote_spaced_path(
    config: &TabConfig,
    params: &HashMap<String, String>,
    autogenerated_branch: Option<&str>,
    branch: &str,
) {
    let (_, template) = render_tab_config(config, params, autogenerated_branch);
    let PaneTemplateType::PaneTemplate { commands, .. } = template else {
        panic!("Expected PaneTemplate");
    };
    let expected_path = generated_worktree_path_string("/Users/me/my project", branch);
    assert!(
        expected_path.contains(' '),
        "test path must contain a space"
    );
    ...
    assert_eq!(
        ...
        "unexpected split for: {}",
        commands[0].exec
    );
    let cd_args = shell_words::split(&commands[1].exec).expect("cd command must parse as shell");
    assert_eq!(cd_args, vec!["cd".to_string(), expected_path]);
}

#[test]
fn test_build_worktree_toml_autogenerate_shell_quotes_spaced_repo_path() {
    let toml_str =
        build_worktree_config_toml("Worktree: my project", "/Users/me/my project", "main", None);
    let config: TabConfig = toml::from_str(&toml_str).expect("Generated TOML should parse");

    assert_worktree_commands_shell_quote_spaced_path(
        &config,
        &HashMap::new(),
        Some("obsidian-hawk"),
        "obsidian-hawk",
...
#[test]
fn test_build_worktree_toml_manual_shell_quotes_spaced_repo_path() {
    let toml_str = build_worktree_config_toml(
        "Worktree: my project",
        "/Users/me/my project",
        "main",
    ...
    let mut params = HashMap::new();
    params.insert("worktree_branch_name".to_string(), "my-feature".to_string());
    assert_worktree_commands_shell_quote_spaced_path(&config, &params, None, "my-feature");
}
