...
/// Delegates to the canonical [`generated_worktree_path`] under the hood.
...
fn test_worktree_template_substitution() {
    let worktree_path = expected_worktree_path("/Users/me/repo", "{{autogenerated_branch_name}}");
    ...
    if let PaneTemplateType::PaneTemplate { commands, .. } = template {
        ...
        assert_eq!(
            ...
            format!(
                ...
                expected_worktree_path("/Users/me/repo", "mesa-coyote")
            )
        );
        assert_eq!(
            commands[1].exec,
    ...
    if let PaneTemplateType::PaneTemplate { commands, .. } = template {
        ...
        assert_eq!(
            ...
            format!(
                ...
                expected_worktree_path("/Users/me/repo", "obsidian-hawk")
            )
        );
        assert_eq!(
            commands[1].exec,
    ...
    if let PaneTemplateType::PaneTemplate { commands, .. } = template {
        ...
        assert_eq!(
            ...
            format!(
                ...
                expected_worktree_path("/Users/me/repo", "my-feature")
            )
        );
        assert_eq!(
            commands[1].exec,
...
/// The New Worktree modal (`+ > New worktree`) submits through
/// `build_worktree_config_toml`. Before #11392 the static repo prefix in the
/// rendered `git worktree add` / `cd` commands was not shell-escaped, so a
/// repo name with spaces or metacharacters got split into separate positional
/// args by the user's shell. These tests pin the escape on both branches of
/// the function (manual branch name + autogenerated branch name).
mod build_worktree_config_toml_path_quoting {
    use warp_util::path::ShellFamily;

    use super::*;

    /// Renders the TOML through `render_tab_config` and returns the rendered
    /// `git worktree add` command for a given branch name.
    fn rendered_git_worktree_add(toml_str: &str, branch_name: &str) -> String {
        let config: TabConfig = toml::from_str(toml_str).expect("Generated TOML should parse");
        let mut params = HashMap::new();
        if !config.params.is_empty() {
            params.insert("worktree_branch_name".to_string(), branch_name.to_string());
        }
        let (_, template) = render_tab_config(&config, &params, Some(branch_name));
        if let PaneTemplateType::PaneTemplate { commands, .. } = template {
            commands[0].exec.clone()
        } else {
            panic!("Expected PaneTemplate");
        }
    }
    ...
    #[test]
    fn autogenerate_branch_escapes_space_in_repo_name() {
        let toml_str = build_worktree_config_toml(
            "Worktree: My Project",
            "/Users/me/My Project",
            "main",
        ...
        let cmd = rendered_git_worktree_add(&toml_str, "obsidian-hawk");
        // The space in "My Project" must be backslash-escaped so the shell
        // parses the whole path as one positional arg to `git worktree add`.
        assert!(
            cmd.contains(r"My\ Project"),
            "expected escaped space in rendered command, got: {cmd}"
        );
        assert!(
            !cmd.contains("My Project"),
            "raw space leaked into command, got: {cmd}"
        );
    }

    #[test]
    fn manual_branch_escapes_space_in_repo_name() {
        let toml_str = build_worktree_config_toml(
            "Worktree: My Project",
            "/Users/me/My Project",
            "main",
        ...
        let cmd = rendered_git_worktree_add(&toml_str, "my-feature");
        assert!(
            cmd.contains(r"My\ Project"),
            "expected escaped space in rendered command, got: {cmd}"
        );
    ...
    fn dollar_in_repo_name_is_shell_escaped() {
        ...
        let cmd = rendered_git_worktree_add(&toml_str, "branch1");
        // `$` must not survive unescaped — otherwise the shell tries to expand
        // it as a variable reference.
        for (i, c) in cmd.char_indices() {
            if c == '$' {
                assert!(
                    i > 0 && cmd.as_bytes()[i - 1] == b'\\',
                    "unescaped $ at byte {i}: {cmd}"
                );
            }
        }
    }
    ...
    fn plain_repo_name_round_trips_unchanged() {
        ...
        // acquire backslashes. This guards against accidentally escaping
        // characters that don't need it.
        let toml_str = build_worktree_config_toml(
            "Worktree: repo",
            "/Users/me/repo",
            "main",
        ...
        assert!(
            !cmd.contains('\\'),
            "expected no escapes for a plain path, got: {cmd}"
        );
    }

    // ── PowerShell ──
    //
    // The first round of #11144 hardcoded `ShellFamily::Posix`, which used
    // backslash escapes that PowerShell parses as literal `\` characters
    // rather than as escapes. With the `shell_family` parameter wired through,
    // PowerShell sessions now get backtick escapes — the only shell-correct
    // form for that family.

    #[test]
    fn powershell_repo_name_with_space_uses_backtick_escape() {
        let toml_str = build_worktree_config_toml(
            "Worktree: My Project",
            "/Users/me/My Project",
            "main",
        ...
        let cmd = rendered_git_worktree_add(&toml_str, "branch1");
        // Backtick-escape (the PowerShell form) for the space inside the path.
        assert!(
            cmd.contains("My` Project"),
            "expected backtick-escaped space, got: {cmd}"
        );
        // And the POSIX backslash form must NOT appear — that would be
        // parsed as a literal `\` by PowerShell, breaking the command.
        assert!(
            !cmd.contains(r"My\ Project"),
            "POSIX backslash leaked into PowerShell command: {cmd}"
        );
    }

    #[test]
    fn powershell_env_var_in_repo_name_is_backtick_escaped() {
        // PowerShell expands `$env:VAR` inside the unquoted command text;
        // backtick-escape must precede every `$env:` occurrence.
        let toml_str = build_worktree_config_toml(
            "Worktree: env",
            "/Users/me/$env:USERPROFILE/repo",
            "main",
            None,
            ShellFamily::PowerShell,
        );
