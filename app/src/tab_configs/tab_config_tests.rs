...
fn build_worktree_config_toml(
    config_name: &str,
    repo: &str,
    base_branch: &str,
    worktree_branch_name: Option<&str>,
) -> String {
    super::build_worktree_config_toml(
        config_name,
        repo,
        base_branch,
...
#[test]
fn test_build_worktree_toml_autogenerate_shell_quotes_worktree_path() {
    let repo_path = "/Users/me/my project";
    ...
    if let PaneTemplateType::PaneTemplate { cwd, commands, .. } = template {
        let expected_worktree_path = expected_worktree_path(repo_path, "obsidian-hawk");
...
#[test]
fn test_build_worktree_toml_manual_shell_quotes_worktree_path() {
    let repo_path = "/Users/me/my project";
    let toml_str = super::build_worktree_config_toml(
        "Worktree: my project",
        repo_path,
    ...
    if let PaneTemplateType::PaneTemplate { cwd, commands, .. } = template {
        let expected_worktree_path = expected_worktree_path(repo_path, "my-feature");
        assert_eq!(cwd, Path::new(repo_path));
        assert_eq!(
            shell_words::split(&commands[0].exec).expect("worktree command should be valid shell"),
            vec![
...
#[test]
fn test_build_worktree_toml_shell_quotes_base_branch() {
    for base_branch in ["main;echo-pwned", "main&echo-pwned", "main'quoted"] {
        let toml_str = super::build_worktree_config_toml(
            "Worktree: my project",
            "/Users/me/my project",
            base_branch,
        ...
        if let PaneTemplateType::PaneTemplate { commands, .. } = template {
            let args = shell_words::split(&commands[0].exec)
                .expect("worktree command should be valid shell");
            assert_eq!(args.last().map(String::as_str), Some(base_branch));
            assert_eq!(args.len(), 7, "base branch must remain one shell argument");
        } else {
            panic!("Expected PaneTemplate");
        }

        let powershell_toml = super::build_worktree_config_toml(
            "Worktree: my project",
        ...
        let powershell_config: TabConfig =
            toml::from_str(&powershell_toml).expect("Generated TOML should parse");
        let powershell_command = powershell_config.panes[0]
            .commands
            .as_ref()
            .expect("worktree commands should exist")[0]
            .as_str();
...
#[test]
fn test_generated_worktree_command_path_uses_shell_family() {
    let branch_placeholder = "{{autogenerated_branch_name}}";
    let repo_path = Path::new("/Users/me/my project");

    let posix_path =
        generated_worktree_command_path(repo_path, branch_placeholder, ShellFamily::Posix);
    let powershell_path =
        generated_worktree_command_path(repo_path, branch_placeholder, ShellFamily::PowerShell);

    assert!(posix_path.contains("my\\ project"));
    assert!(powershell_path.contains("my` project"));
    assert!(posix_path.ends_with(branch_placeholder));
    assert!(powershell_path.ends_with(branch_placeholder));
}
