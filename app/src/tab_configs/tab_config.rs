...
///
/// The path always has the shape `<repo_dir>/<branch_placeholder>` because
/// it's joined by [`generated_worktree_path_string`]. The `<repo_dir>`
/// portion comes from the user's filesystem and may contain spaces, `$`,
/// backticks, or other shell metacharacters; the `branch_placeholder` is a
/// `{{handlebars}}` token that must survive to be resolved by the
/// tab-config render pass. We shell-escape only the prefix.
///
/// The escape strategy depends on the shell that will eventually execute the
/// rendered command: POSIX shells use backslash escapes, PowerShell uses
/// backtick. Pick the right [`warp_util::path::ShellFamily`] at the call
/// site (e.g. from the active session's shell or the user's preferred new-
/// session shell).
///
