...
/// Like [`generated_worktree_path_string`], but shell-quotes the literal path prefix so the
/// generated `git worktree add` / `cd` commands survive shell parsing when the repo or data
/// directory name contains spaces (#11144). Only the literal prefix is quoted: the trailing
/// `worktree_name` is typically a handlebars placeholder that is resolved (and independently
/// shell-quoted) at render time, so it must stay outside the quotes — adjacent shell quotes
/// concatenate into a single argument.
pub(crate) fn shell_quoted_generated_worktree_path_string(
    repo_path: &Path,
    worktree_name: &str,
) -> String {
    let path = generated_worktree_path_string(repo_path, worktree_name);
    match path.strip_suffix(worktree_name) {
        Some(prefix) => shell_words::quote(prefix).into_owned() + worktree_name,
        None => shell_words::quote(&path).into_owned(),
    }
}
...
pub(crate) fn build_worktree_config_toml(
    ...
    if worktree_branch_name.is_some() {
        ...
        let worktree_path =
            shell_quoted_generated_worktree_path_string(Path::new(repo), &worktree_branch_name);
        ...
        let worktree_path = shell_quoted_generated_worktree_path_string(
            Path::new(repo),
            &autogenerated_branch_name,
        );
        pane.insert(
