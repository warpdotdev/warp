...
/// Returns the generated worktree path with its literal prefix shell-quoted, so the
/// generated `git worktree add` / `cd` commands survive shell parsing when the repo or
/// data directory name contains spaces. Only the literal prefix is quoted: the trailing
/// `worktree_name` is typically a handlebars placeholder that is resolved (and
/// independently shell-quoted) at render time, so it must stay outside the quotes —
/// adjacent shell quotes concatenate into a single argument.
pub(crate) fn generated_worktree_path_string(repo_path: &Path, worktree_name: &str) -> String {
    let path = generated_worktree_path(repo_path, worktree_name)
        .display()
        .to_string();
