...
pub(crate) fn build_shell_safe_worktree_path(repo_path: &Path, branch_placeholder: &str) -> String {
...
pub(crate) fn build_worktree_config_toml(
    ...
    if worktree_branch_name.is_some() {
        ...
        let worktree_path = build_shell_safe_worktree_path(Path::new(repo), &worktree_branch_name);
        ...
        let worktree_path =
            build_shell_safe_worktree_path(Path::new(repo), &autogenerated_branch_name);
        pane.insert(
            "commands".into(),
            Value::Array(vec![
                Value::String(format!(
