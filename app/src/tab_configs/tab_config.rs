...
/// Render `<repo_dir>/<worktree_name>` as a `String`. Internal to the
/// `tab_configs` module: external callers must go through
/// [`build_shell_safe_worktree_path`] so a raw, unescaped worktree path
/// can't accidentally leak into a shell command.
pub(super) fn generated_worktree_path_string(repo_path: &Path, worktree_name: &str) -> String {
    generated_worktree_path(repo_path, worktree_name)
        .display()
        .to_string()
}

/// Builds the worktree path that gets embedded into `git worktree add` and
/// `cd` commands, with the static prefix shell-escaped so the user's shell
/// parses the rendered command as a single word.
///
/// The path always has the shape `<repo_dir>/<branch_placeholder>`. The
/// `<repo_dir>` portion comes from the user's filesystem and may contain
/// spaces, `$`, backticks, or other shell metacharacters; the
/// `branch_placeholder` is a `{{handlebars}}` token that must survive to be
/// resolved by the tab-config render pass. We shell-escape only the prefix.
///
/// # `shell_family` precondition
///
/// `shell_family` MUST match the shell that the pane this command lands in
/// will actually run under. The generated `TabConfig` panes leave
/// `shell: None`, which means the runtime falls back to the user's
/// preferred shell via
/// [`AvailableShells::user_preferred_shell_family`][user_pref] — *not*
/// `OperatingSystem::default_shell_family()`. Callers that get this wrong
/// will produce commands escaped for the wrong shell, reintroducing
/// #11144. Always derive `shell_family` from `AvailableShells` at the call
/// site.
///
/// Without this escape, a repo path like `~/Developer/My Project` ends up
...
pub(crate) fn build_shell_safe_worktree_path(
    ...
    let raw = generated_worktree_path_string(repo_path, branch_placeholder);
    match raw.rsplit_once(branch_placeholder) {
        ...
        // `generated_worktree_path_string` is guaranteed to embed the
        // placeholder, but if a future refactor breaks that invariant we
        // still want a safe fallback: escape the whole rendered path.
        None => shell_family.shell_escape(&raw).into_owned(),
    }
}
...
///
/// `shell_family` must match the shell the eventual pane will run under —
/// see [`build_shell_safe_worktree_path`] for the contract. Derive it from
/// [`AvailableShells::user_preferred_shell_family`][user_pref], not from
/// `OperatingSystem::default_shell_family()`.
///
/// [user_pref]: crate::terminal::available_shells::AvailableShells::user_preferred_shell_family
#[cfg(feature = "local_fs")]
pub(crate) fn build_worktree_config_toml(
    ...
    if worktree_branch_name.is_some() {
        ...
        let worktree_path =
            build_shell_safe_worktree_path(Path::new(repo), &worktree_branch_name, shell_family);
        ...
        let worktree_path = build_shell_safe_worktree_path(
            Path::new(repo),
            &autogenerated_branch_name,
            shell_family,
        );
