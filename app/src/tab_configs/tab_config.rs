...
use crate::terminal::available_shells::TAB_CONFIG_SYSTEM_DEFAULT_SHELL;
...
/// Returns the shell-escaped worktree path expression for a generated command.
/// The dynamic Handlebars worktree name remains unescaped for the later render pass.
pub(crate) fn generated_worktree_command_path(
    repo_path: &Path,
    worktree_name: &str,
    shell_family: ShellFamily,
) -> String {
    let mut path_prefix = generated_worktree_repo_dir(repo_path).display().to_string();
    path_prefix.push(std::path::MAIN_SEPARATOR);
    format!("{}{worktree_name}", shell_family.shell_escape(&path_prefix))
}
...
pub fn render_tab_config(
    ...
    render_tab_config_with_shell_family(
        config,
        param_values,
...
/// Renders a tab config using `default_shell_family` for panes that do not
/// specify a shell. Panes with an explicit shell use that shell's family.
pub fn render_tab_config_with_shell_family(
    config: &TabConfig,
    param_values: &HashMap<String, String>,
    worktree_branch_name: Option<&str>,
    default_shell_family: ShellFamily,
) -> (Option<String>, PaneTemplateType) {
    let unquoted_context = build_template_context(param_values, worktree_branch_name);
    ...
    let pane_template =
        match resolve_pane_tree(&config.panes, &unquoted_context, default_shell_family) {
...
fn shell_family_for_pane(shell: Option<&str>, default: ShellFamily) -> ShellFamily {
    let Some(shell) = shell else {
        return default;
    };

    if shell == TAB_CONFIG_SYSTEM_DEFAULT_SHELL {
        return OperatingSystem::get().default_shell_family();
    }

    if shell.starts_with("wsl:") || shell.starts_with("msys2:") {
        return ShellFamily::Posix;
    }

    let normalized_shell = shell.to_ascii_lowercase();
    if normalized_shell == "bash-msys2" {
        return ShellFamily::Posix;
    }

    ShellType::from_name(&normalized_shell)
        .map(Into::into)
        .unwrap_or(default)
}
...
fn resolve_pane_node(
    ...
    if let Some(split_direction) = &node.split {
        ...
        for child_id in children {
            ...
            let (child_template, did_focus) = resolve_pane_node(
                child,
                pane_map,
                unquoted,
                default_shell_family,
                auto_focus_remaining,
            )?;
            if did_focus {
        ...
        let shell_family = shell_family_for_pane(node.shell.as_deref(), default_shell_family);
        let quoted_context: HashMap<String, String> = unquoted
            .iter()
            .map(|(key, value)| (key.clone(), shell_family.escape(value).into_owned()))
            .collect();
            ...
            .map(|cmds| {
                cmds.iter()
                    .map(|cmd| CommandTemplate {
                        exec: handlebars::render_template(cmd, &quoted_context),
                    })
                    .collect()
            })
            .unwrap_or_default();
...
/// generated commands, so reopening the saved config cannot change semantics.
#[cfg(feature = "local_fs")]
pub(crate) fn build_worktree_config_toml(
    ...
    pane.insert("shell".into(), Value::String(shell_name.to_string()));
    ...
    if worktree_branch_name.is_some() {
        ...
        let worktree_path =
            generated_worktree_command_path(Path::new(repo), &worktree_branch_name, shell_family);
        ...
        let worktree_path = generated_worktree_command_path(
            Path::new(repo),
            &autogenerated_branch_name,
            shell_family,
        );
