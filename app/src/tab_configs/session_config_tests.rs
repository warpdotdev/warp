...
#[test]
fn worktree_commands_quote_path_with_spaces_autogenerated() {
    // A repo whose derived worktree path contains spaces must produce
    // double-quoted path args so the shell does not word-split them.
    let config = build_tab_config(
        &SessionType::Terminal,
    ...
    let expected_worktree_path =
        generated_worktree_path_string("/home/user/my project", "{{autogenerated_branch_name}}");
    ...
    // The full space-containing path sits inside double quotes, not bare.
    let quoted = format!("\"{expected_worktree_path}\"");
    assert!(commands[0].contains(&quoted));
    assert!(commands[1].contains(&quoted));
}

#[test]
fn worktree_commands_quote_path_with_spaces_manual() {
    let config = build_tab_config(
        &SessionType::Terminal,
        Path::new("/home/user/my project"),
        true,
    ...
    let expected_worktree_path =
        generated_worktree_path_string("/home/user/my project", "{{worktree_branch_name}}");
    let commands = config.panes[0].commands.as_deref().unwrap();
    assert_eq!(
        commands,
        [
