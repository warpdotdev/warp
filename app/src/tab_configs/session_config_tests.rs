...
// The four worktree builder tests below are POSIX-only: they pass
// `ShellFamily::Posix` and assert the rendered command matches the raw
// `generated_worktree_path` output. That equivalence holds on Unix only —
// on Windows, the platform-supplied AppData prefix contains `\` separators
// that the POSIX shell-escape regex doubles, so the rendered command no
// longer matches the helper's raw `display().to_string()`. PowerShell
// semantics for Windows are covered in `tab_config_tests::build_worktree_config_toml_path_quoting`.
#[cfg(not(windows))]
#[test]
...
mod worktree_path_quoting {
    ...
    // POSIX-only: the assertion checks for `/dollar\$repo/` (forward-slash
    // path separators around the escaped repo name). On Windows the path
    // uses `\` separators, so the substring doesn't match. PowerShell
    // equivalent lives in `tab_config_tests::build_worktree_config_toml_path_quoting`.
    #[cfg(not(windows))]
    #[test]
    ...
    // POSIX-only: the assertion is "no `\` in the rendered command",
    // which is true under POSIX semantics for a plain repo path but
    // tautologically false on Windows where `\` is the native path
    // separator.
    #[cfg(not(windows))]
    #[test]
    fn plain_paths_remain_unchanged() {
        ...
        // No backslash escapes appear when no metacharacters are present.
        for command in commands {
            assert!(
                !command.contains('\\'),
