...
fn terminal_no_worktree() {
    ...
    assert!(config.panes[0].shell.is_none());
...
fn terminal_with_worktree() {
    ...
    assert_eq!(config.panes[0].shell.as_deref(), Some("default"));
...
#[test]
fn render_worktree_shell_quotes_worktree_path() {
    let repo_path = "/home/user/my project";
    let config = super::build_tab_config(
        &SessionType::Terminal,
        Path::new(repo_path),
        true,
        true,
    ...
    let (_, pane_template) =
        super::super::render_tab_config(&config, &HashMap::new(), Some("mesa-coyote"));
    ...
    {
        let expected_worktree_path = expected_worktree_path(repo_path, "mesa-coyote");
