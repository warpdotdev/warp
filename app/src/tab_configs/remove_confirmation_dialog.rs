...
impl RemoveTabConfigConfirmationDialog {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let confirm_button = ctx.add_typed_action_view(|ctx| {
            ActionButton::new(i18n::t("common.remove"), DangerPrimaryTheme)
...
impl View for RemoveTabConfigConfirmationDialog {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let title = i18n::t("tab_configs.remove.title").replace("{name}", &self.config_name);

        let dialog = Dialog::new(
            title,
            Some(i18n::t("tab_configs.remove.description")),
            UiComponentStyles {
