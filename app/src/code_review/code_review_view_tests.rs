...
use crate::code_review::diff_state::{
    DiffHunk, DiffLine, DiffLineType, DiffStateModel, FileDiff, GitFileStatus,
};
...
fn replacement_hunk(
    ...
    DiffHunk {
        old_start_line: line_number,
        old_line_count: 1,
        new_start_line: line_number,
        new_line_count: 1,
        lines: vec![
            DiffLine {
                line_type: DiffLineType::Delete,
...
fn modified_file_diff(hunks: Vec<DiffHunk>) -> FileDiff {
    FileDiff {
...
#[test]
fn test_reconstruct_base_content_from_current_file_and_diff() {
    ...
    assert_eq!(
        CodeReviewView::reconstruct_base_content("one\nnew\nthree\n", &file_diff).as_deref(),
        Some("one\nold\nthree\n")
    );
}

#[test]
fn test_reconstruct_base_content_applies_multiple_hunks_in_reverse() {
    let file_diff = modified_file_diff(vec![
        replacement_hunk(2, "one", "ONE", false),
        replacement_hunk(4, "three", "THREE", false),
    ]);

    assert_eq!(
        CodeReviewView::reconstruct_base_content("zero\nONE\nmiddle\nTHREE\nend\n", &file_diff)
            .as_deref(),
        Some("zero\none\nmiddle\nthree\nend\n")
    );
}
...
#[test]
fn test_reconstruct_base_content_preserves_missing_final_newline() {
    let file_diff = modified_file_diff(vec![replacement_hunk(1, "old", "new", true)]);

    assert_eq!(
        CodeReviewView::reconstruct_base_content("new", &file_diff).as_deref(),
        Some("old")
    );
}
...
#[test]
fn test_jump_to_comment_expands_collapsed_file_and_constructs_editor() {
    App::test((), |mut app| async move {
        let test = TestContext::new(&mut app, "existing.txt", "existing");
        let diff_data = Arc::new(GitDiffWithBaseContent {
            files: vec![code_review_file("generated.rs", "first\nsecond\n", true)],
            total_additions: 0,
            total_deletions: 0,
            files_changed: 1,
        });
...
fn test_deferred_editor_load_keeps_loaded_sibling_visible() {
    App::test((), |mut app| async move {
        ...
        let mut deferred_file = code_review_file("generated.rs", "generated\n", true);
        deferred_file.file_diff.status = GitFileStatus::Modified;
        let diff_data = Arc::new(GitDiffWithBaseContent {
