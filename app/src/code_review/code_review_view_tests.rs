...
/// Builds a `CodeReviewView` under `repo_key`, installs `initial_state` as
/// its loaded state, and forces its diff-state model to `Loaded` so
/// `invalidate_all` can be called directly in tests.
fn setup_invalidate_all_test_view(
    app: &mut App,
    window_id: warpui::WindowId,
    repo_key: LocalOrRemotePath,
    initial_state: LoadedState,
) -> ViewHandle<CodeReviewView> {
    ...
    code_review_view.update(app, |view, ctx| {
        if let Some(repo) = view.active_repo.as_mut() {
...
/// Builds a single-file `LoadedState` for a file with (or without) an
/// editor, so tests can construct the "editor representation doesn't match
/// the file" cases that a plain `create_loaded_state_with_editors` call
/// can't express.
fn single_file_loaded_state(
    app: &mut App,
    window_id: warpui::WindowId,
    file_path: &str,
    is_binary: bool,
    editor: Option<ViewHandle<LocalCodeEditorView>>,
) -> LoadedState {
    let chevron_button = app.add_view(window_id, |_| ActionButton::new("", NakedTheme));
    let open_in_tab_button = app.add_view(window_id, |_| ActionButton::new("", NakedTheme));
    let discard_button = app.add_view(window_id, |_| ActionButton::new("", NakedTheme));
    ...
    let state = FileState {
        ...
        editor_state: editor.map(CodeReviewEditorState::new_loaded),
        is_expanded: true,
        sidebar_mouse_state: MouseStateHandle::default(),
        header_mouse_state: MouseStateHandle::default(),
        chevron_button,
    ...
    LoadedState {
        file_states: [(file_path.to_string(), state)].into_iter().collect(),
        total_additions: 0,
        total_deletions: 0,
        files_changed: 1,
    }
...
fn modified_file_diff_and_content(file_path: &str, content_at_head: &str) -> FileDiffAndContent {
    FileDiffAndContent {
        file_diff: FileDiff {
            file_path: file_path.to_string(),
            status: GitFileStatus::Modified,
            ...
            is_binary: false,
            is_autogenerated: false,
            max_line_number: content_at_head.lines().count(),
            has_hidden_bidi_chars: false,
            size: DiffSize::Normal,
        },
        content_at_head: Some(content_at_head.to_string()),
    }
}
...
fn invalidate_all_reuses_unchanged_file_editor_and_drops_removed_file() {
    App::test((), |mut app| async move {
        ...
        let repo_key = LocalOrRemotePath::Local(PathBuf::from("/repo"));
        let (window_id, _) = app.add_window(WindowStyle::NotStealFocus, |_| TestView);
        ...
        let code_review_view =
            setup_invalidate_all_test_view(&mut app, window_id, repo_key, initial_state);
        ...
        let diff_data = GitDiffWithBaseContent {
            files: vec![modified_file_diff_and_content(
                "unchanged.txt",
                "line 1\nline 2\nline 3",
            )],
            total_additions: 0,
...
/// A reused file's diff base must be refreshed to the new `content_at_head`,
/// not left pointing at the stale base from before the recomputation.
#[test]
fn invalidate_all_refreshes_diff_base_for_reused_text_file() {
    App::test((), |mut app| async move {
        initialize_test_app(&mut app);

        let old_base = "line1\nline2\nline3\n";
        let editor = create_editor_with_diff(&mut app, old_base, old_base);
        ...
        let initial_state = create_loaded_state_with_editors(
            &mut app,
            window_id,
            vec![("changed.txt".to_string(), editor.clone())],
        );
        ...
        let new_base = "line1\nCHANGED\nline3\n";
        let diff_data = GitDiffWithBaseContent {
            files: vec![modified_file_diff_and_content("changed.txt", new_base)],
            total_additions: 0,
            total_deletions: 0,
            files_changed: 1,
        };
        ...
        code_review_view.update(&mut app, |view, ctx| {
            view.invalidate_all(Some(&diff_data), None, ctx);
        });

        code_review_view.update(&mut app, |_view, ctx| {
            let code_editor_view = editor.as_ref(ctx).editor().clone();
            let diff_model = code_editor_view
                .as_ref(ctx)
                .model
                .as_ref(ctx)
                .diff()
            ...
            assert_eq!(
                base.as_deref(),
                Some(new_base),
                "reused editor's diff base should be refreshed to the new content_at_head"
            );
        });
    });
}

/// A file that was previously binary/unrenderable and becomes renderable
/// (non-binary, with base content) must get a fresh editor built for it, not
/// be reused with its old `editor_state: None` representation.
#[test]
fn invalidate_all_builds_editor_when_reused_file_becomes_renderable() {
    App::test((), |mut app| async move {
        initialize_test_app(&mut app);

        let repo_key = LocalOrRemotePath::Local(PathBuf::from("/repo"));
        let (window_id, _) = app.add_window(WindowStyle::NotStealFocus, |_| TestView);
        let initial_state = single_file_loaded_state(
            &mut app,
            window_id,
            "becomes_text.txt",
            true, // previously binary: no editor
            None,
        );
        ...
        code_review_view.update(&mut app, |view, ctx| {
            ...
            let state = loaded
                .file_states
                .get("becomes_text.txt")
                .expect("file should still be present");

            assert!(
                state.editor_state.is_some(),
                "a file that became renderable should get a freshly built editor, \
                 not stay reused with editor_state: None"
            );
        });
    });
}

/// A file with an existing editor that becomes binary must have that editor
/// dropped, not kept alive under a representation that no longer matches
/// the file -- retaining it defeats the memory purpose of this reuse path.
#[test]
fn invalidate_all_drops_editor_when_reused_file_becomes_unrenderable() {
    App::test((), |mut app| async move {
        initialize_test_app(&mut app);

        let editor = create_editor_with_content(&mut app, "some text");
        ...
        let initial_state = create_loaded_state_with_editors(
            &mut app,
            window_id,
            vec![("becomes_binary.txt".to_string(), editor)],
        );
        ...
        let diff_data = GitDiffWithBaseContent {
            files: vec![FileDiffAndContent {
                file_diff: FileDiff {
                    file_path: "becomes_binary.txt".to_string(),
                    status: GitFileStatus::Modified,
                    hunks: Arc::new(vec![]),
                    is_binary: true,
                    is_autogenerated: false,
        ...
        code_review_view.update(&mut app, |view, ctx| {
            ...
            let state = loaded
                .file_states
                .get("becomes_binary.txt")
                .expect("file should still be present");

            assert!(
                state.editor_state.is_none(),
                "a file that became binary/unrenderable should have its old editor \
                 dropped, not kept alive"
            );
        });
    });
}
