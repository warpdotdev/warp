...
use crate::code_review::diff_size_limits::{DiffSize, UnrenderableReason};
use crate::code_review::diff_state::{DiffStateModel, FileDiff, FileDiffAndContent, GitFileStatus};
...
#[test]
fn deleted_file_over_editor_limit_does_not_create_editor() {
    App::test((), |mut app| async move {
        let ctx = TestContext::new(&mut app, "existing.txt", "existing");
        let file = FileDiffAndContent {
            file_diff: FileDiff {
                file_path: "deleted.txt".to_string(),
            ...
            content_at_head: Some("\n".repeat(MAX_EDITOR_BUFFER_NEWLINE_COUNT + 1)),
        };

        ctx.code_review_view.update(&mut app, |view, view_ctx| {
            let file_states =
                view.build_view_state_for_file_diffs(std::slice::from_ref(&file), view_ctx);

            assert_eq!(file_states.len(), 1);
            assert!(file_states[0].editor_state.is_none());
            assert_eq!(
                file_states[0].file_diff.size,
