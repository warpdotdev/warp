...
/// Verifies that review comments attached to removed lines in deleted files
/// are properly relocated without being marked as outdated or triggering fallbacks.
#[test]
fn test_relocate_comments_on_deleted_file() {
    App::test((), |mut app| async move {
        initialize_test_app(&mut app);

        // A deleted file has some base content, but empty current content
        let (diff_tx, diff_rx) = futures::channel::oneshot::channel::<()>();
        ...
        let (_, editor) = app.add_window(WindowStyle::NotStealFocus, |ctx| {
            ...
            let model = code_editor_view.read(ctx, |view, _| view.model.clone());
            ctx.subscribe_to_model(&model, move |_, _, event, _| {
                if let crate::editor::CodeEditorModelEvent::DiffUpdated = event
            ...
            code_editor_view.update(ctx, |editor, ctx| {
                editor.reset(InitialBufferState::plain_text(""), ctx);
                editor.set_base("line 1\nline 2\n", true, ctx);
            });
        ...
        let file_path = "deleted.txt".to_string();
        ...
        let mut file_states = indexmap::IndexMap::new();
        ...
        // Create a comment on the removed line 2 (index 1 of deletion at line 0)
        let repo_location = LocalOrRemotePath::Local(repo_path);
        let comment = AttachedReviewComment {
            id: CommentId::new(),
            content: "My comment".to_string(),
            target: AttachedReviewCommentTarget::Line {
                absolute_file_path: repo_location.join(&file_path),
        ...
        code_review_view.update(&mut app, |_view, view_ctx| {
            let RelocateCommentsResult {
                comments: relocated,
                fallback_count: fallbacks,
            } = CodeReviewView::relocate_comments(vec![comment], &state, &repo_location, view_ctx);

            assert_eq!(relocated.len(), 1);
            assert_eq!(fallbacks, 0);
            assert!(!relocated[0].outdated);
        });
    });
}

/// Verifies that handling a `CommentSaved` event for a deleted file
/// correctly attaches the comment even when the local editor lacks a file path.
#[test]
fn test_save_comment_on_deleted_file() {
    App::test((), |mut app| async move {
        initialize_test_app(&mut app);
        ...
        let code_editor_view = app.add_view(window_id, |ctx| {
            CodeEditorView::new(
                None,
                None,
                CodeEditorRenderOptions::new(VerticalExpansionBehavior::GrowToMaxHeight),
        ...
        // Construct editor without backing file path (mimicking deleted file setup)
        let editor = app.add_view(window_id, |ctx| {
            LocalCodeEditorView::new(code_editor_view, None, false, None, ctx)
        });
        ...
        let code_review_view = app.add_view(window_id, |ctx| {
            CodeReviewView::new(
                Some(repo_location.clone()),
                diff_state_model,
                code_review_comment_batch.clone(),
                None,
                ctx,
            )
        });

        let comment_content = "Test comment on deleted file".to_string();
        let event = super::LocalCodeEditorEvent::CommentSaved {
            comment: crate::code::editor::EditorReviewComment {
                id: CommentId::new(),
                line: EditorLineLocation::Removed {
                    line_number: LineCount::from(0),
                    line_range: LineCount::from(0)..LineCount::from(0),
        ...
        code_review_view.update(&mut app, |view, view_ctx| {
            view.handle_local_code_editor_events(
                editor.clone(),
                &event,
                &repo_location.join(&file_path),
                view_ctx,
            );
        });

        // Verify that the comment was successfully added to the batch
        let comments = code_review_comment_batch
            .expect("should have comment batch")
            .read(&app, |batch, _| batch.comments.clone());
        assert_eq!(comments.len(), 1);
        assert_eq!(comments[0].content, comment_content);
    });
}
