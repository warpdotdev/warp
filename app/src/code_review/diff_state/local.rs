...
use crate::code_review::diff_size_limits::{
    compute_diff_size, compute_diff_size_for_buffer_length, DiffSize,
};
...
impl LocalDiffStateModel {
    ...
    async fn diff_state_against_head(repo_path: &Path) -> Result<GitDiffWithBaseContent> {
        ...
        for (file_path, status) in changed_files {
            ...
            // Never read or ship base content for binary files (can't be
            // inline-rendered and can balloon ~3x after lossy UTF-8 decoding),
            // and skip it for non-`Normal` diffs: loading the full base file of
            // an over-limit/large diff we won't inline-render otherwise buffers
            // entire multi-GB files into memory. See APP-4521.
            let content_at_head = if is_binary || file_diff.size != DiffSize::Normal {
                None
            } else {
    ...
    async fn file_diff_for_path(
        ...
        // Over-limit diffs have empty hunks on purpose (see `get_file_diff`); keep
        // them listed as unrenderable instead of dropping them from the review.
        if !is_binary
        ...
        // Never read or ship base content for binary files (can't be
        // inline-rendered and can balloon ~3x after lossy UTF-8 decoding), and
        // skip it for non-`Normal` diffs so large/unrenderable files don't
        // buffer entire file contents into memory. See APP-4521.
        let content_at_head = if is_binary || file_diff.size != DiffSize::Normal {
            None
        } else {
            match &merge_base {
    ...
    async fn get_file_diff(
        ...
        // Guard against materializing hunks for diffs that are already over the
        // renderable byte limit. `parse_diff_hunks` allocates an owned `DiffLine`
        // (with a copied `String`) for every line, inflating memory to several
        // times the raw diff size, so a single very large changed file (e.g. a
        // generated file, lockfile, or big rewrite vs. the base branch) could
        // otherwise retain multiple GB of parsed hunks. See APP-4521 / APP-4791.
        let mut has_hidden_bidi_chars = false;
        if !matches!(
            compute_diff_size_for_buffer_length(diff_output.len()),
            ...
            // Check for hidden bidi characters (only meaningful for rendered diffs).
            has_hidden_bidi_chars = Self::check_for_hidden_bidi_chars(&diff_output);
        }
