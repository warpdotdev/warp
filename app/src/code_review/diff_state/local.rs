...
use crate::code::global_buffer_model::{MAX_EDITOR_BUFFER_CONTENT_BYTES, editor_buffer_load_error};
...
impl BaseContent {
    fn apply_to(self, file_diff: &mut FileDiff) -> Option<String> {
        match self {
            Self::Loaded(content) => Some(content),
            Self::Unavailable => None,
            Self::ExceedsEditorLimit => {
...
impl LocalDiffStateModel {
    ...
    async fn file_diff_for_path(
        ...
        let content_at_head = if is_binary {
            ...
            let base_content = match &merge_base {
                Some(base) => {
                    match status {
                        GitFileStatus::New | GitFileStatus::Untracked => {
                            ...
                            if repo_path.join(file_path).is_file() {
                                BaseContent::Loaded(String::new())
                            } else {
                                BaseContent::Unavailable
                            }
                        }
            ...
            base_content.apply_to(&mut file_diff)
        };
    ...
    async fn get_file_content_at_head(
        ...
        match status {
            ...
            GitFileStatus::Renamed { old_path } => {
                ...
                Self::get_file_content_at_commit(repo_path, old_path, "HEAD").await
            }
            _ => Self::get_file_content_at_commit(repo_path, file_path, "HEAD").await,
        }
    }
    ...
    async fn get_file_content_at_blob(repo_path: &Path, blob_oid: &str) -> BaseContent {
        ...
        if object_size > MAX_EDITOR_BUFFER_CONTENT_BYTES as u64 {
        ...
        match run_git_command(repo_path, &["cat-file", "blob", blob_oid]).await {
            ...
            Ok(_) => BaseContent::ExceedsEditorLimit,
            Err(_) => BaseContent::Unavailable,
        }
    }
