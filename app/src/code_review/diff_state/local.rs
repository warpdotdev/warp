...
use crate::code_review::diff_size_limits::{
    compute_diff_size, DiffSize, UnrenderableReason, MAX_DIFF_SIZE,
};
...
/// Enforces the per-file base-content budget for locally loaded diffs,
/// mirroring the limit already applied to remote sessions (which withhold
/// over-budget base content on the wire).
///
/// The code review editor loads `content_at_head` (the full file at the diff
/// base) into an in-memory buffer and derives styled blocks from it; for very
/// large files this can consume multiple GB. When the base content exceeds
/// [`MAX_DIFF_SIZE`], drop it and mark the file [`DiffSize::Unrenderable`] so
/// the view shows a "file too large" placeholder instead of constructing an
/// editor. This matches the documented `content_at_head` contract, which
/// already treats over-budget files as having no baseline.
fn enforce_base_content_budget(
    file_diff: &mut FileDiff,
    content_at_head: Option<String>,
) -> Option<String> {
    match content_at_head {
        Some(content) if content.len() > MAX_DIFF_SIZE => {
...
impl LocalDiffStateModel {
    ...
    async fn diff_state_against_head(repo_path: &Path) -> Result<GitDiffWithBaseContent> {
        ...
        for (file_path, status) in changed_files {
            ...
            let content_at_head = enforce_base_content_budget(&mut file_diff, content_at_head);
