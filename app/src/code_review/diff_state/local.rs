...
use warp_util::git::{CappedGitOutput, run_git_command, run_git_command_capped};
...
use crate::code_review::diff_size_limits::{
    DiffSize, MAX_DIFF_SIZE, MAX_DIFF_STDOUT_CAPTURE_BYTES, UnrenderableReason, compute_diff_size,
};
...
impl LocalDiffStateModel {
    ...
    async fn get_file_diff(
        ...
        let diff_output = match run_git_command_capped(
            repo_path,
            &diff_args,
            MAX_DIFF_STDOUT_CAPTURE_BYTES,
        )
        .await
        {
            Ok(CappedGitOutput::Complete(output)) => output,
            // The subprocess's stdout was killed before finishing rather than
            // buffered in full (see APP-5462). This is not an error: it's the
            // same outcome as a completed diff over MAX_DIFF_SIZE below, just
            // detected earlier, so it must not fall into the binary/error
            // fallback below.
            Ok(CappedGitOutput::Exceeded) => {
                return Ok(FileDiff {
                    file_path: file_path.to_owned(),
                    status: status.clone(),
                    hunks: Arc::new(Vec::new()),
