...
use crate::code_review::diff_size_limits::{
    compute_diff_size, compute_diff_size_for_buffer_length,
};
...
impl LocalDiffStateModel {
    ...
    async fn diff_state_against_head(repo_path: &Path) -> Result<GitDiffWithBaseContent> {
        ...
        // Get line-count and binary metadata using git diff --numstat.
        let diff_metadata = Self::get_diff_metadata_using_numstat(repo_path, "HEAD").await?;
        ...
        for (file_path, status) in changed_files {
            let file_metadata = diff_metadata.get(&file_path);
            let is_binary = file_metadata.is_some_and(|metadata| metadata.is_binary_file);
            let mut file_diff =
                Self::get_file_diff(repo_path, &file_path, &status, is_binary, None).await?;
            let content_at_head =
                Self::get_file_content_at_head(repo_path, &file_path, &status, &file_diff).await;

            file_diff.is_autogenerated =
                is_file_autogenerated(&file_path, content_at_head.as_deref());

            total_additions += file_metadata
                .map(|metadata| metadata.lines_added)
                .unwrap_or_else(|| file_diff.additions());
            total_deletions += file_metadata
                .map(|metadata| metadata.lines_removed)
                .unwrap_or_else(|| file_diff.deletions());
            ...
            Some(base) => {
                match status {
                    GitFileStatus::New | GitFileStatus::Untracked => {
                        ...
                        Self::empty_file_content_for_diff(&file_diff)
                    }
                    GitFileStatus::Renamed { old_path } => {
                        // The original content is in the old path of the given base commit.
                        Self::get_file_content_at_commit(repo_path, old_path, base, &file_diff)
                            .await
                    }
                    _ => {
                        Self::get_file_content_at_commit(repo_path, file_path, base, &file_diff)
                            .await
                    }
                }
            }
            None => Self::get_file_content_at_head(repo_path, file_path, status, &file_diff).await,
        };
    ...
    async fn diff_state_against_specific_branch(
        ...
        // Get line-count and binary metadata using git diff --numstat against merge base.
        let diff_metadata = Self::get_diff_metadata_using_numstat(repo_path, &merge_base).await?;
        ...
        for (file_path, status) in changed_files {
            ...
            if let Some(file_diff) = file_diff {
                total_additions += file_metadata
                    .map(|metadata| metadata.lines_added)
                    .unwrap_or_else(|| file_diff.file_diff.additions());
                total_deletions += file_metadata
                    .map(|metadata| metadata.lines_removed)
                    .unwrap_or_else(|| file_diff.file_diff.deletions());
    ...
    async fn get_file_content_at_head(
        ...
        if !Self::should_load_base_content(file_diff) {
            return None;
        }
    ...
    fn should_load_base_content(file_diff: &FileDiff) -> bool {
        !file_diff.is_binary && file_diff.size == DiffSize::Normal
    }

    fn empty_file_content_for_diff(file_diff: &FileDiff) -> Option<String> {
        Self::should_load_base_content(file_diff).then(String::new)
    }
    ...
    async fn get_file_diff(
        ...
        let size = compute_diff_size_for_buffer_length(diff_output.len());
        if size != DiffSize::Unrenderable {
            // Parse the diff output for renderable text files.
    ...
    pub(crate) fn parse_diff_hunks(diff_output: &str) -> Result<Vec<DiffHunk>> {
        ...
        while let Some(line) = lines.next() {
            ...
            if line.starts_with("@@") {
                ...
                while let Some(content_line) = lines.peek().copied() {
