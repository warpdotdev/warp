...
use warp_util::git::{run_git_command, run_git_command_bytes};
...
use super::{
    ...
    GitFileStatus, GitOpResult, ImagePreviewData, ImageSide, MAX_IMAGE_PREVIEW_BYTES,
...
impl LocalDiffStateModel {
    ...
    async fn diff_state_against_head(repo_path: &Path) -> Result<GitDiffWithBaseContent> {
        ...
        for (file_path, status) in changed_files {
            ...
            let image_preview = if is_binary || matches!(status, GitFileStatus::Untracked) {
                Self::load_image_preview(repo_path, &file_path, &status, "HEAD").await
            } else {
                None
            };
    ...
    async fn file_diff_for_path(
        ...
        let image_preview = if is_binary || matches!(status, GitFileStatus::Untracked) {
            Self::load_image_preview(repo_path, file_path, status, merge_base.unwrap_or("HEAD"))
                .await
        } else {
            None
        };
    ...
    /// MIME types previewable in the code review panel: raster formats only.
    /// SVG is text/XML, renders as a normal text diff, and never reaches the
    /// binary branch.
    const SUPPORTED_PREVIEW_MIME_TYPES: &'static [&'static str] =
        &["image/png", "image/jpeg", "image/gif", "image/webp"];

    /// Classifies raw bytes as an image preview side by their contents only.
    /// No extension fallback (product spec §8): a text file named `.png` or a
    /// Git LFS pointer must not classify as an image. Acceptance requires a
    /// supported magic-byte signature AND a successful decode under
    /// decompression-bomb limits.
    fn classify_image_bytes(bytes: Vec<u8>) -> ImageSide {
        let byte_len = bytes.len() as u64;

        let sniff_len = bytes.len().min(crate::util::image::MIME_SNIFF_BYTES);
        let Some(mime) = infer::get(&bytes[..sniff_len]).map(|kind| kind.mime_type()) else {
            return ImageSide::Rejected;
        };
        if !Self::SUPPORTED_PREVIEW_MIME_TYPES.contains(&mime) {
            return ImageSide::Rejected;
        }

        // The byte cap only bounds *compressed* input; read the dimensions
        // cheaply from the header before decoding so a small buffer declaring
        // huge dimensions never allocates its pixel buffer.
        ...
        {
            return ImageSide::TooLarge { byte_len };
        }

        // Full decode proves the bytes are a valid image; the decoder-level
        // limits keep even a lying header from over-allocating.
    ...
    /// Loads and classifies the base-revision side of an image preview from
    /// the `<base_rev>:<base_path>` blob. For renamed/copied files
    /// `base_path` must be the pre-rename path.
    async fn load_base_image_side(repo_path: &Path, base_path: &str, base_rev: &str) -> ImageSide {
        let blob_spec = format!("{base_rev}:{base_path}");

        // Check the blob size up front so an oversized side is never
        // buffered into memory.
        let byte_len = match run_git_command(repo_path, &["cat-file", "-s", &blob_spec]).await {
            Ok(output) => match output.trim().parse::<u64>() {
                Ok(byte_len) => byte_len,
                Err(_) => return ImageSide::Unavailable,
            },
            Err(_) => return ImageSide::Unavailable,
        };
        if byte_len > MAX_IMAGE_PREVIEW_BYTES {
            return ImageSide::TooLarge { byte_len };
        }

        log::debug!("[GIT OPERATION] local.rs load_base_image_side git show {blob_spec}");
        match run_git_command_bytes(repo_path, &["show", &blob_spec]).await {
            Ok(bytes) => Self::classify_image_bytes(bytes),
            Err(_) => ImageSide::Unavailable,
        }
    }

    /// Whether the first bytes of a file carry a supported image magic-byte
    /// signature. Callers should pass at most `MIME_SNIFF_BYTES`.
    fn sniffs_as_supported_image(header: &[u8]) -> bool {
        infer::get(header)
            .is_some_and(|kind| Self::SUPPORTED_PREVIEW_MIME_TYPES.contains(&kind.mime_type()))
    }

    /// Loads and classifies the working-tree side of an image preview from
    /// disk.
    async fn load_working_image_side(repo_path: &Path, file_path: &str) -> ImageSide {
        let absolute_path = repo_path.join(file_path);

        let byte_len = match std::fs::metadata(&absolute_path) {
            Ok(metadata) if metadata.is_file() => metadata.len(),
            _ => return ImageSide::Unavailable,
        };
        if byte_len > MAX_IMAGE_PREVIEW_BYTES {
            return ImageSide::TooLarge { byte_len };
        }

        // Cheap magic-byte sniff before buffering the whole file: untracked
        // files reach here without a prior binary check (they never appear
        // in `git diff --numstat`), so a plain text or non-image file must
        // be rejected from its first bytes rather than fully read.
        let mut header = vec![0u8; crate::util::image::MIME_SNIFF_BYTES];
        let header_len =
            match File::open(&absolute_path).and_then(|mut file| file.read(&mut header)) {
                Ok(header_len) => header_len,
                Err(_) => return ImageSide::Unavailable,
            };
        if !Self::sniffs_as_supported_image(&header[..header_len]) {
            return ImageSide::Rejected;
        }
    ...
    /// Attempts to load an inline image preview. Called for binary files
    /// and for untracked files (untracked files never appear in
    /// `git diff --numstat`, so an untracked image is not flagged
    /// `is_binary`; the working-side magic-byte sniff cheaply rejects
    /// untracked non-images). `base_rev` mirrors the revision used for
    /// `content_at_head` (HEAD, or the merge base in branch mode). Returns
    /// `Some` only when at least one applicable side is previewable — a
    /// file whose every applicable side is unreadable or not a valid image
    /// keeps its existing presentation (product spec §6/§8).
    async fn load_image_preview(
        repo_path: &Path,
        file_path: &str,
        status: &GitFileStatus,
        base_rev: &str,
    ) -> Option<ImagePreviewData> {
        if !FeatureFlag::ImagePreviewInCodeReview.is_enabled() {
            return None;
        }

        let mut old = match status {
            GitFileStatus::New | GitFileStatus::Untracked => None,
            // The base-side blob lives at the pre-rename path.
            GitFileStatus::Renamed { old_path } | GitFileStatus::Copied { old_path } => {
                Some(Self::load_base_image_side(repo_path, old_path, base_rev).await)
            }
            _ => Some(Self::load_base_image_side(repo_path, file_path, base_rev).await),
        };
        let new = match status {
            GitFileStatus::Deleted => None,
            _ => Some(Self::load_working_image_side(repo_path, file_path).await),
        };

        // A renamed/copied file without a content change shows a single
        // image; the rename is carried by the existing path/status
        // presentation, not duplicated as a fake before/after (product spec
        // §2).
        if matches!(
            status,
        ...
        let preview = ImagePreviewData { old, new };
        preview.has_previewable_side().then_some(preview)
    }
