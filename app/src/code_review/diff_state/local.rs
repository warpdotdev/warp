...
use crate::code_review::diff_size_limits::{
    is_numstat_unrenderable, is_raw_diff_unrenderable, DiffSize,
};
...
impl LocalDiffStateModel {
    ...
    async fn diff_state_against_head(repo_path: &Path) -> Result<GitDiffWithBaseContent> {
        ...
        // Get numstat metadata — used for both binary detection and large-file
        // pre-filtering to avoid buffering multi-GB diff output.
        let numstat_metadata = Self::get_diff_metadata_using_numstat(repo_path, "HEAD").await?;
        ...
        for (file_path, status) in changed_files {
            let numstat = numstat_metadata.get(&file_path);
            let is_binary = numstat.is_some_and(|m| m.is_binary_file);

            // Pre-filter: skip `git diff --patch` and content loading for files
            // whose numstat line counts guarantee the diff will be unrenderable.
            if let Some(meta) = numstat {
                if is_numstat_unrenderable(meta.lines_added, meta.lines_removed) {
                    log::info!(
                        "Pre-skipping large diff for {file_path}: +{} -{} lines",
                        meta.lines_added,
                        meta.lines_removed
                    );
                    total_additions += meta.lines_added;
                    total_deletions += meta.lines_removed;
            ...
            // Skip loading content at HEAD for unrenderable diffs to avoid
            // another multi-MB allocation that will never be rendered.
            let content_at_head = if file_diff.size == DiffSize::Unrenderable {
                None
            } else {
            ...
            // Use numstat for totals when the diff was early-bailed (empty hunks)
            // so summary stats remain accurate.
            if file_diff.size == DiffSize::Unrenderable {
                if let Some(meta) = numstat {
                    total_additions += meta.lines_added;
                    total_deletions += meta.lines_removed;
                }
    ...
    async fn file_diff_for_path(
        ...
        // Don't skip Unrenderable diffs — they should appear in the file list
        // as "too large to display".
        if !is_binary
            && file_diff.size != DiffSize::Unrenderable
        ...
        // Skip loading content at base/HEAD for unrenderable diffs to avoid
        // another large allocation that will never be rendered.
        let content_at_head = if file_diff.size == DiffSize::Unrenderable {
            None
        } else {
            match &merge_base {
    ...
    async fn diff_state_against_specific_branch(
        ...
        for (file_path, status) in changed_files {
            ...
            if let Some(file_diff) = file_diff {
                // Use numstat for totals when the diff was early-bailed.
                if file_diff.file_diff.size == DiffSize::Unrenderable {
                    if let Some(meta) = numstat {
                        total_additions += meta.lines_added;
                        total_deletions += meta.lines_removed;
                    }
    ...
    async fn get_file_diff(
        ...
        // Early bail: if the raw diff output exceeds the unrenderable threshold,
        // skip parsing to avoid multi-GB allocations in parse_diff_hunks.
        if is_raw_diff_unrenderable(diff_output.len()) {
            log::info!(
                "Skipping diff parsing for {file_path}: raw output size ({} bytes) exceeds limit",
                diff_output.len()
            );
            return Ok(FileDiff {
                file_path: file_path.to_owned(),
