...
impl LocalDiffStateModel {
    ...
    /// Forces a refresh of the shared per-repo `GitRepoStatusModel`, if a
    /// consumer (e.g. a terminal's top-panel git chip) is currently
    /// subscribed to it. This is a no-op when nothing is subscribed.
    ///
    /// Working-tree mutations made from the Code Review panel (discard,
    /// etc.) only update this model's own diff/metadata state, which is not
    /// what feeds the top-panel git chip. Calling this after such mutations
    /// keeps the chip in sync immediately, instead of relying on the
    /// filesystem watcher or an unrelated terminal event to catch up.
    #[cfg(feature = "local_fs")]
    fn refresh_git_repo_status(&self, ctx: &mut ModelContext<Self>) {
        let Some(repo_path) = self.active_repository_path(ctx) else {
            return;
        };
        let repo = LocalOrRemotePath::Local(repo_path);
        GitRepoModels::handle(ctx).update(ctx, |models, ctx| models.refresh_repo(&repo, ctx));
    }
