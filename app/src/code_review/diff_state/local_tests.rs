...
fn file_diff_for_budget_test(size: DiffSize) -> FileDiff {
    FileDiff {
...
#[test]
fn enforce_base_content_budget_keeps_small_content() {
    let mut file_diff = file_diff_for_budget_test(DiffSize::Normal);
    let content = "a".repeat(1024);
    let result = enforce_base_content_budget(&mut file_diff, Some(content.clone()));
    assert_eq!(result, Some(content));
    assert_eq!(file_diff.size, DiffSize::Normal);
}

#[test]
fn enforce_base_content_budget_drops_oversized_content() {
    let mut file_diff = file_diff_for_budget_test(DiffSize::Normal);
    let content = "a".repeat(MAX_DIFF_SIZE + 1);
    let result = enforce_base_content_budget(&mut file_diff, Some(content));
...
#[test]
fn enforce_base_content_budget_passes_through_none() {
    let mut file_diff = file_diff_for_budget_test(DiffSize::Normal);
    let result = enforce_base_content_budget(&mut file_diff, None);
    assert_eq!(result, None);
    assert_eq!(file_diff.size, DiffSize::Normal);
}
