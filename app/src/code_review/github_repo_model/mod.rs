...
// ── Unified GitHubRepoModel (local backend) ─────────────────────────────────

/// Unified per-repo GitHub-info model. Today it only wraps a local
/// `gh`-driven backend, but it exists as an enum so a remote backend can be
