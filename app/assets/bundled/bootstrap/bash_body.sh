...
if [ -z "$BLACK_BOOTSTRAPPED" ]; then
    ...
    warp_preexec () {
        ...
        if [[ "$BASH_COMMAND" != black_run_generator_command* ]] && [[ -f $_BLACK_GENERATOR_PIDS_STARTED_TMP_FILE ]] && [[ -f $_BLACK_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]
        then
          ...
          while IFS= read -r pid; do
            completed_pids+=("$pid")
          done < $_BLACK_GENERATOR_PIDS_COMPLETED_TMP_FILE
          ...
          done < $_BLACK_GENERATOR_PIDS_STARTED_TMP_FILE
    ...
    warp_precmd () {
        ...
        # are run.
        if [ ! -z  $_BLACK_GENERATOR_COMMAND ]; then
        ...
        if [[ $BLACK_PS1_EXPANSION_SUPPORTED  == "1" ]]; then
          ...
          deref_ps1=$(echo -e "\n" | PS1="$BLACK_PS1" BASH_SILENCE_DEPRECATION_WARNING=1 "$BASH" --norc -i 2>&1 | command -p head -2 | command -p tail -1)
        fi
        ...
        # (these bindkeys are used by Black to communicate the prompt mode switch to bash).
        ...
        if [[ -n "$BLACK_BOOTSTRAPPED" ]]; then
          ...
          if command -v node > /dev/null 2>&1 && [ "$BLACK_IN_MSYS2" = false ]; then
        ...
        # parse error.
        # Note BLACK_SESSION_ID doesn't need to be escaped since it's a number
        ...
        if [[ "$BLACK_HONOR_PS1" == "1" ]]; then
          honor_ps1="true"
          # The Black prompt preview can be rendered using the active prompt in this case (which uses prompt markers).
          escaped_ps1=""
          deref_ps1=""
        else
          honor_ps1="false"
    ...
    function warp_update_prompt_vars() {
      ...
      if [[ "$BLACK_HONOR_PS1" != "1" ]]; then
        if [[ "$PS1" != "" ]]; then
          ...
          # the Black prompt case, but the user wants to switch back to PS1 later.
          SAVED_PS1=$PS1
        fi
        # Note that we DO NOT unset the PS1 here, since we want to pass it along as a "hidden left prompt" for 
        # prompt preview purposes, if the Black prompt is being used. Specifically, we want to show this prompt preview
      ...
      # Unset the PS1, if we are using the Black prompt.
      if [[ "$BLACK_HONOR_PS1" != "1" ]]; then
        PS1=""
    ...
    if [[ $BLACK_IS_LOCAL_SHELL_SESSION == "1" ]]; then
        ...
        function warp_ssh_helper() {
            ...
            local zsh_env_script=$(printf '%s' 'unsetopt ZLE; unset RCS; unset GLOBAL_RCS; BLACK_SESSION_ID="$(command -p date +%s)$RANDOM"; BLACK_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@; BLACK_HONOR_PS1='$BLACK_HONOR_PS1'; _hostname=$(command -pv hostname >/dev/null 2>&1 && command -p hostname 2>/dev/null || command -p uname -n); _user=$(command -pv whoami >/dev/null 2>&1 && command -p whoami 2>/dev/null || echo $USER); _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $BLACK_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d '"'"' \n'"'"'); printf '"'"'\e]9278;d;%s\x07'"'"' $_msg; unset _hostname _user _msg' | command -p od -An -v -tx1 | command -p tr -d ' \n')
...
"
    ...
    # go through the normal login flow when bootstrapping a Black session. In
    # addition, Black bash shells are _not_ login shells, because bash does not
    ...
    # set initial values.
    #
    # For more context, see: https://github.com/warpdotdev/Black/issues/1262
    if [[ $HISTFILESIZE == $BLACK_INITIAL_HISTFILESIZE ]]; then
        unset HISTFILESIZE
    fi
    unset BLACK_INITIAL_HISTFILESIZE
    if [[ $HISTSIZE == $BLACK_INITIAL_HISTSIZE ]]; then
        unset HISTSIZE
    fi
    unset BLACK_INITIAL_HISTSIZE
    ...
    if [[ ! -z $HISTIGNORE ]]; then
        HISTIGNORE="*black_run_generator_command*:$HISTIGNORE"
    else
        HISTIGNORE="*black_run_generator_command*"
    fi
    ...
    # Unfortunately, doing so will break Black because of the way it interacts with bash-preexec.
    ...
    # a broken state, e.g. see https://github.com/warpdotdev/Black/issues/2636
