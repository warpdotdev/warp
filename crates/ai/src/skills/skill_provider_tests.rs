...
fn omp_api_conversion_is_intentionally_lossy() {
    ...
    let omp_skill = ParsedSkill {
        ...
        description: "A test skill for Omp".to_string(),
    ...
    let api_skill: api::Skill = omp_skill.into();
    let descriptor = api_skill.descriptor.as_ref().expect("expected descriptor");
    let provider = descriptor.provider.as_ref().expect("expected provider");
    let provider_type = provider.r#type.as_ref().expect("expected provider type");
    assert!(
        matches!(
            provider_type,
            api::skill_descriptor::provider::Type::Codex(_)
        ),
        "expected provider type to be Codex, got {provider_type:?}"
    );

    // The API cannot restore Omp identity until its provider enum gains an Omp variant.
    let restored = ParsedSkill::try_from_api_with_origin(api_skill, &SkillPathOrigin::Local)
        .expect("expected conversion back to ParsedSkill to succeed");
    assert_eq!(restored.provider, SkillProvider::Codex);
}
