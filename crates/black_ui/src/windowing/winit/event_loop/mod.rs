...
/// winit events into black_ui events.
#[derive(Default)]
struct State {
...
impl EventLoop {
    ...
    /// Handles a request to close the window with the given black_ui and winit
    /// IDs.
    ...
    /// Handle events that may be handled by black_ui, or maybe not in some cases, e.g. window
    ...
    fn handle_converted_warpui_event(
        ...
        // passing the event into black_ui. Skip for touch events as drag_resize_window
        ...
        // If the app didn't handle the event, black_ui might still want to do something
    ...
    ///
    /// Converts `SoftKeyboardInput` events into black_ui `Event`s and dispatches
