...
use black_ui_core::rendering::{GPUBackend, GPUDeviceInfo, GPUDeviceType};
use wgpu::{
...
/// Returns whether or not the provided adapter is an unsupported Nvidia driver version for black_ui
/// to render properly.
...
/// black_ui to render properly. Currently, we limit this to "Intel UHD Graphics 620", but we do have
...
/// alignment.
///
/// See: https://github.com/blackdagger/black/issues/6120
pub fn adapter_has_rendering_offset_bug(adapter_info: &wgpu::AdapterInfo) -> bool {
    ...
    // https://github.com/blackdagger/black/issues/6120.
    let affected_models = [
...
/// (Vulkan llvmpipe) adapter that may not work properly with black_ui.
...
fn adapter_stability_sort_func(
    ...
    // Same with Nvidia drivers, though this is only an issue with a Wayland window server.
        ...
        log::warn!(
            ...
            https://blackdagger.io/help/known-issues#linux-1",
