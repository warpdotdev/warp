    ...
    fn capture_current_demo(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(window) = ctx.windows().platform_window(self.window_id) {
            ...
            if auto_capture && !is_last_demo {
                ctx.spawn(
                    async {
                        black_ui::r#async::Timer::after(std::time::Duration::from_millis(350)).await;
                    },
                    |_, _, ctx| {
    ...
    fn render(&self, _app: &AppContext) -> Box<dyn Element> {
        ...
        Container::new(
            Flex::column()
                ...
                .with_cross_axis_alignment(black_ui::elements::CrossAxisAlignment::Stretch)
                .with_child(
                ...
                if self.capture_config.capture_screenshots {
                    ctx.spawn(
                        async {
                            ...
                            black_ui::r#async::Timer::after(std::time::Duration::from_millis(300))
                                .await;
                        },
                        |view, _, ctx| {
                            view.capture_current_demo(ctx);
