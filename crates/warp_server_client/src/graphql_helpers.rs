...
/// Like [`send_graphql_request`], but layers `extra_headers` on top of the base client's
/// authenticated headers for this one request (e.g. a selected-team scope header). Team scope
/// is deliberately passed this way rather than stored on `BaseClient`, so it stays request-local
/// and cannot leak across concurrent requests for different teams.
