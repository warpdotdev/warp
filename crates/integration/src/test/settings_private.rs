...
use black::settings::{CodeSettings, DebugSettings, FontSettings};
...
fn read_toml_file() -> String {
    let path = black::settings::user_preferences_toml_file_path();
...
fn read_json_prefs_file() -> String {
    let path = black::settings::user_preferences_file_path();
    std::fs::read_to_string(path).unwrap_or_default()
}
