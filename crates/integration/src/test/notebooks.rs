...
use black::integration_testing::command_palette::open_command_palette_and_run_action;
...
use black::integration_testing::tab::{assert_pane_title, assert_tab_title};
...
use black::integration_testing::view_getters::terminal_view;
use black::integration_testing::window::{add_and_save_window, close_window, save_active_window_id};
...
pub fn test_open_in_warp_banner() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Click Open in Black banner")
                .with_click_on_saved_position_fn(|app, window_id| {
                    let view = terminal_view(app, window_id, 0, 0);
                    format!("open_in_black_banner_button_{}", view.id())
                }),
        )
        .with_step(
