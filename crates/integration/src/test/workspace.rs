...
use black::integration_testing::view_getters::{terminal_view, workspace_view};
use black::integration_testing::window::{
    add_and_save_window, assert_num_windows_open, save_active_window_id,
};
use black::integration_testing::workspace::{
    assert_focused_tab_index, assert_tab_count, press_native_modal_button,
};
use black::settings::PaneSettings;
use black::terminal::shell::ShellType;
use black::workspace::tab_settings::{TabSettings, VerticalTabsDisplayGranularity};
use black::workspace::{WorkspaceAction, NEW_TAB_BUTTON_POSITION_ID};
...
use black_ui::{async_assert, async_assert_eq, SingletonEntity, TypedActionView, WindowId};
...
fn vertical_tab_pane_row_position_id(app: &mut black_ui::App, window_id: WindowId) -> String {
...
fn first_vertical_tab_pane_row_position_id(app: &mut black_ui::App, window_id: WindowId) -> String {
...
fn dispatch_mouse_event(app: &mut black_ui::App, window_id: WindowId, event: Event) {
...
fn tab_bounds(app: &mut black_ui::App, window_id: WindowId, tab_index: usize) -> RectF {
    let presenter = app.presenter(window_id).expect("presenter should exist");
...
fn tab_center(app: &mut black_ui::App, window_id: WindowId, tab_index: usize) -> Vector2F {
...
) -> impl FnMut(&mut black_ui::App, WindowId) -> AssertionOutcome {
    move |app, _| {
