...
fn no_proxy_suffix_with_dot() {
    ...
    clear_proxy_env();
    env::set_var("HTTPS_PROXY", "http://proxy:3128");
    env::set_var("NO_PROXY", ".blackdagger.io");

    assert!(resolved_proxy_tls("sessions.app.blackdagger.io").is_none());

    assert!(resolved_proxy_tls("blackdagger.io").is_some()); // Exact "blackdagger.io" != ".blackdagger.io"
    assert!(resolved_proxy_tls("other.com").is_some());
    clear_proxy_env();
}
...
fn no_proxy_suffix_without_dot() {
    ...
    // "sessions.app.blackdagger.io" ends with "blackdagger.io" → matches
    assert!(resolved_proxy_tls("sessions.app.blackdagger.io").is_none());
    // Exact match too
    assert!(resolved_proxy_tls("blackdagger.io").is_none());
    assert!(resolved_proxy_tls("notblack.io").is_some());
    clear_proxy_env();
}
