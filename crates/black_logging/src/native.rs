...
fn sentry_log_filter(md: &log::Metadata) -> sentry_log::LogFilter {
    if black_core::errors::should_ignore_log_for_sentry(md) {
    ...
    match md.target() {
        ...
        "black_ui::core::redraw_frame" => sentry_log::LogFilter::Ignore,
...
fn init_internal(
    ...
    let integration_test = env::var("BLACK_INTEGRATION").is_ok();
...
fn init_log_directory() -> Result<std::path::PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            Ok(black_core::paths::state_dir())
        } else if #[cfg(windows)] {
            Ok(black_core::paths::state_dir().join(black_core::paths::WARP_LOGS_DIR))
        } else {
