//! Shared disk-to-disk smart-cut and overlay processing for recorded video.
...
fn subtitles_filter_context(ass_path: &Path) -> Result<(&Path, String), RecordingError> {
    let directory = ass_path
        .parent()
        .filter(|parent| !parent.as_os_str().is_empty())
        .unwrap_or_else(|| Path::new("."));
    let filename = ass_path
        .file_name()
        .and_then(|filename| filename.to_str())
        .ok_or_else(|| RecordingError::Finalize {
            reason: "overlay subtitle path has no UTF-8 filename".to_string(),
        })?;
    Ok((directory, format!("subtitles=filename='{filename}'")))
}
...
async fn burn_overlays_into_cut(
    ...
    let status = Command::new("ffmpeg")
        ...
        .args(["-movflags", "+faststart"])
        .arg(&output_path)
        .current_dir(subtitle_directory)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .status()
...
pub(crate) async fn post_process_recording(
    ...
    let ass_path = std::env::temp_dir().join(format!(
        "warp-recording-overlay-{}.ass",
        uuid::Uuid::new_v4()
    ));
...
pub(crate) fn build_cut_only_filtergraph(segments: &[crate::overlay::KeepSegment]) -> String {
...
#[cfg(test)]
#[path = "recording_post_process_tests.rs"]
mod tests;
