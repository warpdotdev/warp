...
///
/// The builder uses const-generic state so `enable_tools()` can't be invoked
/// conditionally — each combination needs its own builder chain. The prompts
/// flag is set on the built value because the builder API exposes no
/// `enable_prompts()`.
...
/// A server that responded to `initialize` but advertised no capabilities must
/// not be queried for prompts.
#[test]
...
/// `PromptsCapability::list_changed` is informational (whether the server will
/// send `notifications/prompts/list_changed`); it must not affect the initial
/// listing decision.
#[test]
...
/// When the prompts capability is not advertised, the helper must not call the
/// list function.
#[tokio::test]
...
/// Capability advertised but listing is also valid when the server has zero
/// prompts (empty vec). Distinct from the "skipped" case in that we still made
/// the call; we just got an empty response.
#[tokio::test]
...
/// MCP-protocol errors also fail soft.
#[tokio::test]
...
/// The list function must be called exactly once per query — not zero and not
/// multiple times.
#[tokio::test]
...
/// `query_prompts_for` behavior must not depend on whether tools or resources
/// are also advertised.
#[tokio::test]
...
/// Sanity: a `Prompt` constructed via the rmcp API survives passage through
/// `query_prompts_for` unchanged.
#[tokio::test]
