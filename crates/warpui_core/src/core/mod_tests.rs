...
/// A neutral helper for `test_nested_focus_with_unrendered_view`: the child
/// views are never rendered, so focus/blur propagation relies purely on
/// creation-time structural parentage (`add_typed_action_view`).
struct UnrenderedChildView {
    name: String,
    children: Vec<ViewHandle<UnrenderedChildView>>,
    events: Rc<RefCell<Vec<String>>>,
}

impl UnrenderedChildView {
    fn new(
        name: String,
        children: Vec<ViewHandle<UnrenderedChildView>>,
        events: Rc<RefCell<Vec<String>>>,
...
impl Entity for UnrenderedChildView {
    type Event = String;
}

impl super::View for UnrenderedChildView {
    fn render(&self, _: &AppContext) -> RenderOutput {
        crate::test::empty_render_output()
    }
...
impl TypedActionView for UnrenderedChildView {
    type Action = ();
}
    ...
    App::test((), |mut app| async move {
        ...
        let (_window_id, view_1) = app.add_window(WindowStyle::NotStealFocus, |ctx| {
            let view_2 = ctx.add_typed_action_view(|_ctx| {
                UnrenderedChildView::new("View 2".to_string(), vec![], events.clone())
            });
            UnrenderedChildView::new("View 1".to_string(), vec![view_2], events.clone())
        });
