...
pub use scene::{TuiClipBounds, TuiLocalPoint, TuiScene, TuiScreenPoint, TuiScreenRect, TuiZIndex};
...
pub struct TuiPaintContext<'a> {
    ...
    pub scene: TuiScene,
    /// Buffer-local to terminal-space offsets for nested scratch buffers.
    buffer_to_screen_offsets: Vec<(i32, i32)>,
...
impl<'a> TuiPaintContext<'a> {
    ...
    pub fn new(rendered_views: &'a mut EntityIdMap<Box<dyn TuiElement>>) -> Self {
        Self {
            ...
            buffer_to_screen_offsets: vec![(0, 0)],
            repaint_at: None,
            terminal_cursor: None,
        }
    }

    /// Converts a buffer-local point to signed terminal-space geometry.
    pub fn screen_point(&self, point: TuiPoint) -> TuiScreenPoint {
        let (x_offset, y_offset) = *self
            .buffer_to_screen_offsets
            .last()
            .expect("the TUI paint context always has a root buffer mapping");
        TuiScreenPoint::new(
            i32::from(point.x).saturating_add(x_offset),
            i32::from(point.y).saturating_add(y_offset),
            self.scene.z_index(),
        )
    }

    /// Converts a buffer-local rectangle to terminal-space geometry.
    pub fn screen_rect(&self, rect: TuiRect) -> TuiScreenRect {
        TuiScreenRect::new(
            self.screen_point(TuiPoint::new(rect.x, rect.y)),
            rect.as_size(),
        )
    }

    /// Runs `f` with a scratch-buffer mapping to `screen_origin`.
    pub fn with_buffer_mapping<R>(
        &mut self,
        buffer_origin: TuiPoint,
        screen_origin: TuiScreenPoint,
        f: impl FnOnce(&mut Self) -> R,
    ) -> R {
        self.buffer_to_screen_offsets.push((
            screen_origin.x.saturating_sub(i32::from(buffer_origin.x)),
            screen_origin.y.saturating_sub(i32::from(buffer_origin.y)),
        ));
        let result = f(self);
        self.buffer_to_screen_offsets.pop();
        result
    }
    ...
    pub(crate) fn finish(self) -> (TuiScene, Option<Instant>, Option<TuiScreenPoint>) {
        debug_assert_eq!(self.buffer_to_screen_offsets.len(), 1);
...
/// - [`render`](TuiElement::render): paint at a buffer-local origin, retaining
///   the corresponding signed screen origin through [`TuiPaintContext`].
...
pub trait TuiElement {
    ...
    /// Paints this element at `buffer_origin`. `ctx` carries the
