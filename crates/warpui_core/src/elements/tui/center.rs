...
impl TuiElement for TuiCenter {
    fn layout(&mut self, constraint: TuiConstraint, ctx: &mut TuiLayoutContext) -> TuiSize {
        ...
        self.child_size = self.child.layout(TuiConstraint::loose(constraint.max), ctx);
        constraint.max
    }
    ...
    fn render(&self, area: TuiRect, buffer: &mut TuiBuffer, ctx: &mut TuiLayoutContext) {
        ...
        self.child.render(self.centered_rect(area), buffer, ctx);
    }
    ...
    fn dispatch_event(
        ...
        self.child.dispatch_event(event, rect, event_ctx, ctx, app)
    }
}
