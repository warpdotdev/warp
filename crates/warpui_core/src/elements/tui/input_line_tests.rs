...
fn is_always_a_single_row() {
    ...
    assert_eq!(
        TuiInputLine::new("hello", 0)
            .layout(TuiConstraint::loose(TuiSize::new(10, 5)), &mut ctx)
            .height,
        1
    );
    assert_eq!(
...
fn scrolls_horizontally_to_keep_the_cursor_visible() {
    ...
    assert_eq!(
        line.cursor_position(TuiRect::new(0, 0, 3, 1), &mut ctx),
        Some((2, 0))
    );
}
...
fn renders_placeholder_when_empty() {
    ...
    assert_eq!(
        line.cursor_position(TuiRect::new(0, 0, 8, 1), &mut ctx),
        Some((0, 0))
    );
}
