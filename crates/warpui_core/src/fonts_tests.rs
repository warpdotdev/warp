...
impl EmWidthFontDB {
    fn cache(bounds: Option<RectI>, advance: Vector2I) -> Cache {
...
fn em_width_uses_typographic_bounds_width_when_available() {
    let cache = EmWidthFontDB::cache(Some(RectI::new(vec2i(0, 0), vec2i(10, 12))), vec2i(20, 0));
...
fn em_width_uses_horizontal_advance_when_typographic_bounds_are_missing() {
    let cache = EmWidthFontDB::cache(None, vec2i(20, 0));
