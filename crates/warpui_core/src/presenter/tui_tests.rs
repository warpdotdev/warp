...
use crate::elements::tui::{
    ...
    TuiLayoutContext, TuiPaintContext, TuiPoint, TuiPresentationContext, TuiRect, TuiRectExt,
...
impl TuiElement for ColumnDouble {
    ...
    fn render(
        ...
        for (child, size) in self.children.iter_mut().zip(&self.child_sizes) {
            ...
            child.render(TuiPoint::new(child_area.x, child_area.y), buffer, ctx);
            remaining = rest;
        }
    }
...
impl TuiElement for ContainerDouble {
    ...
    fn render(
        ...
        self.child
            .render(TuiPoint::new(child_area.x, child_area.y), buffer, ctx);
    }
