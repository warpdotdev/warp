...
pub fn spawn_tui_driver<T: TuiView>(
    ...
    let task = ctx.foreground_executor().spawn(async move {
        while let Ok(event) = receiver.recv().await {
            ...
            if handled {
                // Redraws are invalidation-driven: `update_windows` fires the
                // window's invalidation callback (registered above), which draws.
                // It normally runs at the end of a flush, but input dispatch here
                // doesn't flush app effects, and a pure-element scroll never marks
                // a view dirty — so without this the redraw would be deferred until
                // some later, unrelated flush (e.g. PTY output), making scrolling
                // feel laggy and intermittent. Invalidate then drive `update_windows`
                // directly, mirroring the repaint-task pattern in `app.rs`.
