...
fn whole_file_truncated_on_first_line_yields_empty_range() {
    // Budget smaller than the first line, so nothing is buffered. The
    // truncated range must be empty, not reversed.
...
fn range_truncated_on_first_in_range_line_yields_empty_range() {
    // The first line inside the requested range exceeds the byte budget. The
    // returned range must be empty, not reversed.
