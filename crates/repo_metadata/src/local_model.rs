...
use crate::entry::{
    matches_ignored_path_interest, BudgetExceededBehavior, BuildTreeError, BuildTreeOptions,
    Entry, FileId, IgnoredPathStrategy,
};
        ...
        for (repo_path, repo_scoped_update) in repo_updates {
            if let Some(IndexedRepoState::Indexed(state)) = self.repositories.get_mut(&repo_path) {
                ...
                ctx.spawn(
                    ...
                    |model,
                     (mutations, newly_failed, discovered_results, removed_roots, repo_path, lazy_load),
                     ctx| {
