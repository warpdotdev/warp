...
pub(crate) enum OnboardingStateEvent {
    ...
    UpgradeRequested,
    AuthStateChanged,
    /// The "Are you sure you don't want AI?" modal was shown or dismissed. The
    /// modal is rendered by `AgentOnboardingView`, which re-renders on emitted
    /// events (not bare model notifies), so toggling it must emit.
...
impl OnboardingStateModel {
    ...
    pub(crate) fn set_ai_setup_choice(
        ...
        // Choosing third-party agents means the user isn't using Warp's agent,
        // so Warp AI (Oz) is turned off; choosing Warp Agent turns it back on.
        // This both disables Warp AI in the applied settings and (via
        // `is_ai_enabled`) gates the post-onboarding login prompt to the
        // Warp-harness path.
    ...
    pub(crate) fn back(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        let prev = if theme_picker_last {
            match self.step {
                ...
                // Only the agent → "Use third party agents" choice reaches the
                // third-party slide now (the terminal path skips it), so back
                // always returns to the AI-setup fork.
                OnboardingStep::ThirdParty => Some(OnboardingStep::AiSetup),
                OnboardingStep::Project => Some(OnboardingStep::ThirdParty),
                // Customize is the last slide before the theme picker on every
        ...
        if theme_picker_last {
            ...
            match self.step {
                ...
                OnboardingStep::Customize => match self.intention {
                    // Terminal-only users skip the third-party-agents slide.
