...
pub fn ai_features() -> &'static [&'static str] {
    ...
    static FEATURES: OnceLock<[&'static str; 6]> = OnceLock::new();
    FEATURES.get_or_init(|| {
        [
            menu_label(
                "onboarding.agent.features.frontier_models",
                "Use frontier and open-weight models with Warp Agent",
            ),
            menu_label(
                "onboarding.agent.features.cloud_agents",
                "Hand off agent work to cloud agents",
            ),
            menu_label(
                "onboarding.agent.features.auto_fixes",
                "Automatically diagnose and fix terminal errors",
            ),
            menu_label(
                "onboarding.agent.features.agentic_control",
                "Agentic control of long-running commands and TUIs",
            ),
            menu_label(
                "onboarding.agent.features.code_review",
                "Review code diffs and send comments directly to agents",
            ),
            menu_label(
                "onboarding.agent.features.remote_control",
...
/// User-facing descriptions of the features enabled when the agent intention is selected
/// with Warp Drive turned on. Shared by the login slide's skip-login confirmation dialog
/// so the list stays in sync with any future surfaces that need it.
pub fn warp_drive_features() -> &'static [&'static str] {
    use std::sync::OnceLock;
    static FEATURES: OnceLock<[&'static str; 2]> = OnceLock::new();
    FEATURES.get_or_init(|| {
        [
            menu_label("workspace.warp_drive", "Warp Drive"),
            menu_label(
