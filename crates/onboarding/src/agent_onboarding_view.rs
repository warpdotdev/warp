...
/// How long the "Plan successfully activated!" toast stays visible before it
/// auto-dismisses.
...
pub enum AgentOnboardingEvent {
    ...
    /// The user clicked "+ Add key" on the "Choose how to access AI" slide; the
    /// parent hosts the (app-crate) provider-key modal.
    AddApiKeyRequested,
    /// The user clicked "+ Add custom endpoint" on the "Choose how to access AI"
    /// slide; the parent hosts the (app-crate) custom-endpoint modal.
...
pub enum AgentOnboardingAction {
    ...
    CmdOrCtrlEnterKey,
    Escape,
    /// Modal: confirm "I don't want AI".
    NoAiConfirm,
    /// Modal: "Give me AI features".
    NoAiCancel,
    /// Modal: dismiss without changing the path (ESC / X).
    NoAiDismiss,
    /// Dismiss the "Plan successfully activated!" toast.
...
impl AgentOnboardingView {
    ...
    /// Updates whether the user has a BYOK key/endpoint configured, which gates
    /// "Next" on the AI-access slide's bring-your-own path.
    pub fn set_byok_configured(&mut self, configured: bool, ctx: &mut ViewContext<Self>) {
        self.ai_access_slide.update(ctx, |slide, ctx| {
            slide.set_byok_configured(configured, ctx);
        });
    }
...
impl View for AgentOnboardingView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
            ...
            .is_some()
        {
            ...
            // Dim the slide behind the modal. Added before the dialog's dismiss
            // layer so the scrim sits underneath and clicking it still dismisses.
            stack.add_child(
                Rect::new()
...
impl TypedActionView for AgentOnboardingView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        // While the no-AI confirmation modal is open it owns all input: route its
        // buttons (and Enter/Escape) to the model and swallow everything else.
        if self
            .onboarding_state
            .as_ref(ctx)
            .no_ai_confirmation()
