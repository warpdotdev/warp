...
pub struct AiAccessSlide {
    ...
    /// Whether the user has at least one BYOK key/endpoint configured (mirrors
    /// the app's `ApiKeyManager`). Gates "Next" on the bring-your-own path.
...
impl AiAccessSlide {
    ...
    fn can_advance(&self, app: &AppContext) -> bool {
        match self.choice(app) {
            ...
            AiAccessChoice::Byok => self.byok_configured,
        }
    }
...
impl AiAccessSlide {
    ...
    pub(crate) fn set_byok_configured(&mut self, configured: bool, ctx: &mut ViewContext<Self>) {
        if self.byok_configured == configured {
            return;
        }
        self.byok_configured = configured;
        ctx.notify();
    }
}
