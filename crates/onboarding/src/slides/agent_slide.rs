...
impl button::Theme for UpgradeButtonTheme {
    ...
    fn text_color(
        &self,
        background: Option<black_core::ui::theme::Fill>,
        appearance: &Appearance,
    ) -> ColorU {
        let bg = background
...
pub struct AgentSlide {
    onboarding_state: black_ui::ModelHandle<OnboardingStateModel>,
...
impl AgentSlide {
            ...
            match event {
                ...
                OnboardingStateEvent::AuthStateChanged => {
                        ...
                        && me.last_auth_state != OnboardingAuthState::PayingUser
                    {
                        ...
                        let _ = ctx.spawn(
                            black_ui::r#async::Timer::after(PLAN_ACTIVATED_TOAST_DURATION),
                            |me: &mut Self, _, ctx| {
    ...
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        let title = appearance
            .ui_builder()
            .paragraph("Customize your Black Agent")
    ...
    fn render_disable_oz_section(
        ...
        let label = Text::new("Disable Black Agent", appearance.ui_font_family(), 14.0)
            .with_color(internal_colors::text_sub(theme, background_for_text))
            .with_style(Properties {
                weight: Weight::Normal,
                ..Default::default()
    ...
    fn render_bottom_nav(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let step_index = 2;
        let step_count = if black_core::features::FeatureFlag::OpenWarpNewSettingsModes.is_enabled()
        {
            5
        } else {
            4
...
impl View for AgentSlide {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        stack.add_child(
            black_ui::elements::Align::new(bottom_overlay)
                .bottom_center()
                .finish(),
        );
        stack.finish()
