...
///
/// The caller renders the three buttons itself (so each view keeps its own
/// `button::Button` state and dispatches its own actions); this renderer only
/// owns the shared card layout. The dialog is the same one used by the login
/// skip-confirmation and the onboarding "Are you sure you don't want AI?" modal.
pub struct FeatureOptOutDialog {
    pub title: &'static str,
    pub body: &'static str,
    pub features: &'static [&'static str],
    /// Top-right close affordance (typically an `X` with an ESC badge).
    pub close_button: Box<dyn Element>,
    /// Footer-left button that keeps the features (cancels the opt-out).
    pub cancel_button: Box<dyn Element>,
    /// Footer-right button that proceeds with the opt-out.
    pub confirm_button: Box<dyn Element>,
}

/// Renders the opt-out confirmation card (surface, title row, body + crossed-out
/// feature list, and footer). Callers are responsible for overlaying it (e.g.
/// centering it and wrapping it in a dismiss backdrop).
