...
macro_rules! send_telemetry_from_ctx {
    ($event:expr, $ctx:expr) => {
        ...
        if event.enablement_state().is_enabled() {
            let auth_state =
                <$crate::telemetry::TelemetryContextModel as black_ui::SingletonEntity>::handle($ctx)
                    .as_ref($ctx);
            let user_id = auth_state.user_id($ctx);
            let anonymous_id = auth_state.anonymous_id($ctx);
            black_ui::record_telemetry_from_ctx!(
                user_id,
                anonymous_id,
                event.name().into(),
                event.payload(),
...
macro_rules! send_telemetry_from_app_ctx {
    ($event:expr, $app_ctx:expr) => {
        ...
        if event.enablement_state().is_enabled() {
            let auth_state =
                <$crate::telemetry::TelemetryContextModel as black_ui::SingletonEntity>::handle(
                    $app_ctx,
                )
                .as_ref($app_ctx);
