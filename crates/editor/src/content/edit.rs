...
/// Maximum total content size (in bytes) for full CoreText/platform text layout in a
/// single `layout_delta` call. When the total content of an `EditDelta` exceeds this
/// threshold, text blocks are laid out with lightweight estimated-height frames that
/// skip expensive glyph shaping, caret computation, and platform text layout. This
...
impl EditDelta {
    ...
    pub fn layout_delta(
        ...
        // layout (CoreText on macOS) to avoid multi-GiB memory spikes.
...
fn layout_text_block_lightweight(
    ...
    let remaining_content_length = text_block.content_length;
    ...
    if paragraphs.is_empty() {
        ...
        paragraphs.push(Paragraph::new(
            frame,
            OffsetMap::direct(remaining_content_length.as_usize()),
