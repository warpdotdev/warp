...
void tree_sitter_sql_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
  LexerState *state = (LexerState *)payload;
  // Free the buffer the state already owns before reallocating: tree-sitter
  // calls this hook every time it restores saved external-scanner state
  // (incremental re-lexing, GLR error-recovery backtracking), and the
  // previous `start_tag` allocation would otherwise leak on every restore.
  // See https://github.com/DerekStride/tree-sitter-sql/pull/361 (upstream
  // fix, not yet merged/released as of arborium-sql 2.18.2).
  if (state->start_tag != NULL) {
    free(state->start_tag);
    state->start_tag = NULL;
  }
