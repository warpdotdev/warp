...
Base branch: `varoon/va-extend-overlays` (the recording stack that already contains the post-stop smart cut and overlay preservation). This change is a **stacked client PR** on top of that branch; there is no `warp-server` code change.
...
- **Smart-cut aware.** The recording is already cut to effective action windows post-stop (`build_keep_segments` + `cut_to_segments`), which removes idle/thinking gaps and remaps overlays onto the compacted timeline. The click ring and drag trail must be treated exactly like the existing pills: their full animation must survive the cut (see behavior invariant 7).
...
3. **Animation survives the cut by construction.** Extend `build_keep_segments` so every retained window covers each pointer event's full animation tail, independent of the pill margins.
...
7. **The click/drag animation survives the smart cut.** The recording is cut to retained action windows and overlays are remapped onto the compacted output timeline. Each pointer event's full animation tail (up to 900 ms after the event) must fall inside a single retained segment: `build_keep_segments` extends each committed group's kept window end to cover its pointer events' animation tail, in addition to the existing `SEGMENT_MARGIN_POST` pill margin. This is guaranteed independently of the pill margin so future margin tuning cannot clip a ripple or trail.
...
- Constants centralized: `CLICK_RING_DURATION = 900 ms`, `CLICK_RING_MIN_RADIUS = 18`, `CLICK_RING_MAX_RADIUS = 36`, `CLICK_RING_THICKNESS = 4`, `MOUSE_DOWN_RADIUS = 16`, `DRAG_ANCHOR_RADIUS = 10`, `DRAG_TRAIL_THICKNESS = 4`, `DRAG_TRAIL_FADE_DURATION = 600 ms`, orange `[255, 80, 40]`, and `POINTER_ANIMATION_TAIL = 900 ms` (= max ring/fade).
...
- **Smart-cut coverage**: extend `build_keep_segments` so each group's kept window end is `max(finish + SEGMENT_MARGIN_POST, max(pointer_event.offset) + POINTER_ANIMATION_TAIL)`. Because a group's animation starts within its action window and ends within its kept window, the whole animation lies inside one contiguous retained segment and remaps linearly (no internal gap). Text-pill timing/behavior is unchanged.
...
## Orchestration
**Decision:** Use child agents. After landing the shared type/plumbing contract, the two remaining halves touch disjoint files and are independently verifiable, so they run in parallel to cut wall-clock time.
**Dependencies and ordering:** Foundational contract commit (types, `Options.pointer_sink`, `ActionLogEntry.pointer_events`, updated construction sites) must land first. The renderer child then works only in `overlay.rs`/`overlay_tests.rs`; the capture plumbing is done against the same contract. The x11 actor emission (Linux-only, not compilable on the macOS dev host) and final integration are owned by the orchestrator.
**Launch config:** One local batch off the foundational commit; see the plan-attached orchestration config for run-wide settings.
**Child agents:**
- **renderer** — computer_use ASS renderer. Owns `crates/computer_use/src/overlay.rs` (pointer vector rendering + `build_keep_segments` animation coverage) and `crates/computer_use/src/overlay_tests.rs`. Verifies with `cargo nextest run -p computer_use`. Fully unit-testable on macOS because the code is `#[cfg(any(linux, test))]`.
**Merge strategy:** The orchestrator integrates the renderer branch (disjoint from the capture files), adds the x11 actor emission and app-layer plumbing, then runs the full validation and opens the single stacked PR.
...
2. **Smart-cut coverage test**: a click/drag whose animation tail extends past `finish + SEGMENT_MARGIN_POST` is fully inside one retained segment; assert the extended `build_keep_segments` window and that the remapped animation interval is contiguous (no internal gap). Assert existing pill remapping is unchanged.
