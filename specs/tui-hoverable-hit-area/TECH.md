...
The TUI implementation will adopt that model. The terminal-specific adaptation is that paint geometry and buffer coordinates cannot be identical. `TuiClipped` renders a complete child into a temporary ratatui buffer rooted at `(0, 0)` and copies only the visible rows into the destination buffer ([`clipped.rs:61-139 @ eba8ef7`](https://github.com/warpdotdev/warp/blob/eba8ef71648f49028052e7a8dfc7803900ee4b82/crates/warpui_core/src/elements/tui/clipped.rs#L61-L139)). The scratch buffer remains, while `TuiPaintContext` tracks how buffer-local coordinates map to signed terminal-space coordinates.
...
Add signed, cell-based scene geometry alongside ratatui's unsigned buffer geometry:

- `TuiScreenPoint`: signed `x`/`y` plus `TuiZIndex`.
...
The `buffer_origin` render parameter is buffer-local because ratatui widgets require unsigned buffer coordinates. The `origin()` accessor is always the retained signed screen origin. An element must obtain that value by converting `buffer_origin` through `TuiPaintContext`; it must never store the buffer-local point directly. Its paint rectangle is `TuiRect::new(buffer_origin.x, buffer_origin.y, size.width, size.height)`.
...
### Paint context and scratch-buffer transforms
...
- a mutable `TuiScene`;
- a stack of signed buffer-to-screen offsets;
- helpers that convert a buffer-local point or rectangle into screen geometry;
- scoped push/pop helpers for buffer mappings and scene layers;
- paint-time hardware cursor submission;
- the existing child-view map and repaint deadline.

At the root, buffer `(0, 0)` maps to terminal `(0, 0)`. Normal containers pass buffer-local child origins, and each element converts its origin through the active mapping before caching it.

`TuiClipped` keeps its current scratch-buffer rendering strategy:

1. Cache the viewport's screen origin and start a scene layer clipped to its visible bounds.
2. Allocate the full logical child scratch buffer at local `(0, 0)`.
3. Push a buffer-to-screen mapping from scratch `(0, 0)` to the child's signed logical terminal origin, offset upward by `viewport_origin_y`.
4. Paint the child into the scratch buffer. Child origins and hit records are converted through the mapping and clipped by the scene layer.
5. Copy the visible scratch rows into the destination buffer.
6. Pop the mapping and scene layer.

Mapping and layer helpers must restore state through scoped guards or closure-based APIs, including early returns. Debug assertions verify balanced stacks at the end of root paint.

This separates two concerns:

- ratatui buffer coordinates remain unsigned and local to the current render target;
- retained geometry and hit testing always use signed terminal-space coordinates.

Scene clipping does not proxy or intercept ratatui buffer writes. Outside `TuiClipped`, the layout invariant guarantees that each retained element size fits its parent constraint and destination buffer. Every element paints the exact rectangle formed from `buffer_origin + size()`. `TuiClipped` allocates a scratch buffer large enough for both the child's retained logical size and the visible viewport before painting it. Elements that perform manual cell writes must continue using checked `cell_mut` access.
...
Paint becomes the single top-down placement pass, matching GUI. Parents use cached child sizes to calculate child origins once and call mutable child `render`. Cursor-owning leaves submit the terminal cursor during that pass, and event dispatch consumes retained child origins rather than reconstructing slots.

For `TuiFlex`:

- cache the flex's size during layout and screen origin during paint;
- place each child from its retained measured size;
...
- `TuiContainer` caches its own bounds and paints its child at the retained padding/border offset.
...
2. Add signed scene geometry, `TuiScene`, and transform/clip support to `TuiPaintContext`.
3. Change `TuiElement` render, geometry, presentation, and dispatch APIs; move terminal cursor output to `TuiPaintContext`.
...
- Buffer-local points map to screen coordinates through nested offsets.
...
- `TuiClipped` maps scratch rows to signed logical screen rows and exposes only the viewport intersection.
...
- **Buffer/screen coordinate drift:** keep conversion APIs on `TuiPaintContext`; elements must not construct screen origins directly. Cover nested clipping and nonzero root origins in tests.
...
- **Unbalanced transform or layer stacks:** use scoped helpers and assert root paint restores both stacks.
