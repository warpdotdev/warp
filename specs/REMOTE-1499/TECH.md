...
- `app/src/ai/blocklist/agent_view/agent_input_footer/mod.rs` `HandoffChipClicked` action: emits the `HandoffChipClicked` event unconditionally (under the flag gate). The terminal `Input` subscriber dispatches `WorkspaceAction::OpenLocalToCloudHandoffPane { launch: None, environment_id: None, entry_point: HandoffEntryPoint::FooterChip }` when the input buffer is empty AND the guardrail passes; otherwise calls `activate_cloud_handoff_compose` so any in-flight prompt is preserved.
...
The decision "should the cloud agent skip its initial LLM turn?" is computed fresh per execution by `common.ShouldSkipInitialTurn(task, execution)` in warp-server-4, which routes through the shared `common.ShouldSkipFirstTurn(task, userVisiblePrompt) (bool, error)` predicate. `ShouldSkipFirstTurn` is the single source of truth for "is the first turn empty-but-allowed?" and is also called by the runtime's `buildMessagesFromAmbientRunInput` to gate the "run has no prompt" rejection, so the worker's dispatch-time decision and the runtime's empty-prompt handling cannot drift. With the client-side snapshot-rehydration substitution always carrying a non-empty prompt when a snapshot is present, the predicate simplifies to: empty user-visible prompt + non-empty `AgentConversationID` → skip; empty + no conversation id → invalid (`run has no prompt`); non-empty → has content. The result reaches the sandboxed CLI as the `--skip-initial-turn` flag; the worker is the only producer of that flag and the AgentDriver is the only consumer. The flag is the entire worker→driver contract.
...
### 4.1 Why a dedicated marker is needed
The viewer's cloud-mode `TerminalModel` is constructed with `BlockList::is_executing_oz_environment_startup_commands = true` (`app/src/pane_group/mod.rs`, cloud-mode pane construction). That flag is the canonical gate for the pre-first-exchange UI state:
- `is_cloud_agent_pre_first_exchange` (`app/src/terminal/view/ambient_agent/mod.rs`) reads it directly; while true, the cloud-mode pane hides the input box and renders the loading footer in its place, so the user cannot type at all.
- `maybe_insert_setup_command_blocks` (`app/src/terminal/view/ambient_agent/view_impl.rs`) reads it to classify incoming blocks as setup commands and wrap them under the "Running setup commands…" chip.

Pre-feature, that flag was implicitly cleared by the `AppendedExchange` handler in `app/src/terminal/view.rs` — the act of the first LLM turn starting was treated as proof that setup must be done. The skip-initial-turn path deliberately never fires `AppendedExchange`, so without an explicit signal the flag stays `true` forever, the input box never re-renders, and the user cannot follow up at all. `AmbientSetupPhaseEnded` is that explicit signal, decoupling the UI transition from any LLM turn so the input becomes available the moment the AgentDriver knows environment setup has actually finished.
### 4.2 Testing-only Cargo.toml swap
...
### 4.4 `AgentDriver::execute_run` structure
...
### 4.5 `IdleTimeoutSender::complete_with_optional_idle`
...
### Considered alternatives
- **Reusing `AppendedExchange` as the teardown signal everywhere.** Rejected: the skip-initial-turn path never fires `AppendedExchange`, so the viewer's `is_executing_oz_environment_startup_commands` flag stays `true`, `is_cloud_agent_pre_first_exchange` keeps reporting that the pane is in setup, the input box stays hidden behind the loading footer, and the user cannot type a follow-up at all. A dedicated marker decouples setup-phase teardown from first-LLM-turn semantics.
