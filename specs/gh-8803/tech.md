...
- `crates/lsp/src/descriptor/matcher.rs` — `pub fn match_descriptor<'a>(descriptors: &'a [LspServerDescriptor], file_path: &Path) -> Option<LspMatchedDescriptor<'a>>`. Returns the **last**-in-source-order match per invariant 4 (a later entry overrides an earlier one). Computes the LSP `languageId` per invariants 1 and 33: the file's extension is **lowercased before the table lookup** (the table's keys are lowercase; matching is case-insensitive per invariant 20, so `.H` must hit the same entry as `.h`) — the built-in extension → language-id table when the lowercased extension (or literal basename) is in it, else the file's lowercase extension, else its literal basename. Matching applies to the file's **basename only** (`path.file_name()`), per invariants 1 and 20 — the compiled matchers are never handed the full path.
...
Tests: unit tests in `crates/lsp/src/descriptor/`. Covers invariants 1, 2, 4, 5, 6, 20, 21, 22, 23, 24, 32 (the parse/validate-time logging policy), and 33 (the extension → language-id table default).
...
**File-open dispatch sites.** `LanguageId::from_path` is called from 9 sites in `app/src/` (the "what LSP handles this file?" decision points). Each gets a registry-first lookup added:
