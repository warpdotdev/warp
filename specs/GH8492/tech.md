...
- **Resource bounds (section 8):** a deeply nested layout string returns `Malformed` instead of overflowing the stack; an over-long `%`-line, an over-long title, and pane/window counts past the caps are each rejected/truncated; a per-pane buffer driven past its cap reseeds the pane (asserting no truncated mid-sequence bytes reach the model) and a repeated breach tears the pane down; a parse error or `%begin` without `%end` triggers the teardown path rather than unbounded allocation.
