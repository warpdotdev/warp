...
`TuiBlockListViewportSource` consumes `BlockList`'s existing dirty rich-content queue, measures dirty registered `TuiAgentBlockView`s at the current viewport width through the view-level `desired_height` helper, and writes the resulting heights back through `BlockList::update_rich_content_heights` before slicing. For visible agent blocks, it relies on `TuiViewportedList`'s normal layout result: visible items carry a measured-height id, and `update_visible_item_heights` writes changed measured heights back to `BlockList`. If those visible heights changed, the generic viewport re-requests and re-lays out once so the current frame uses the updated height cache. This avoids an extra visible `desired_height` pre-layout pass on every render.
