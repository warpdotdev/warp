...
Add a private async-controller helper that calls `clear_matches` on every registered rich-content view without clearing the new query's configuration. Invoke it at every query boundary before branching into a full scan or refinement scan. Every early-return path must also clear registered rich-content views: the no-query path continues to use `clear_results`, and an invalid regular expression clears the views before returning without enqueueing scan work. This prevents a badge or highlight from the previous valid query from surviving while the Find UI reports the invalid expression.
...
- Add an async-controller regression test that starts from a valid query with a populated reasoning match, changes to an invalid regular expression, and verifies that the registered rich-content view is cleared even though no scan work is enqueued (Behavior 5-6, 12-13).
