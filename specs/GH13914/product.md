...
1. For a representative set of inline code spans ending in narrow-ink characters (`.`, `,`, `i`, `l`, `)`, `;`), the visually-measured right-side chip padding equals the left-side chip padding (both are the same fixed design padding around the ink).
