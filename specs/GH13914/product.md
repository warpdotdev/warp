...
- As a deliberate consequence of achieving equal padding, the chip's left edge **tightens** by roughly 1px (2 × `block_padding`, ≈`font_size / 5`) versus today, because the legacy left-only `block_padding` shift is compensated out for chips. This is intended behavior, not preservation: the chip reads as slightly snugger on the left and noticeably snugger on the right, with both gaps equal. Non-chip bordered/backgrounded runs keep the legacy left shift and do not move.
...
2. The chip's left edge **tightens** by ~1px (2 × `block_padding`) versus today: because the legacy left-only `block_padding` shift is compensated out for chips to make the padding equal, code spans at the start of a line, or immediately following another inline element, sit ~1px snugger on the left. This is a small, intended shift, not a preserved position.
...
2. As the deliberate consequence of criterion 1, the chip's left edge **tightens** by `2 × block_padding` (≈`font_size / 5`, ~1px at typical UI font sizes) versus today's rendering — this is the intended behavior, verified by a geometry-level test, not a preserved position. Non-chip bordered/backgrounded runs keep the legacy left shift and their left edge does not move (verified by a test asserting a non-chip run retains the legacy advance-based geometry).
