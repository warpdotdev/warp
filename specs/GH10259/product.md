...
11. A collapsed body is not reachable by keyboard or by caret. Tab traversal skips focus stops inside it, including links, and arrow-key or click-to-place caret movement moves across the collapsed region rather than into it. Reaching hidden content requires expanding the section first, so focus and the caret never rest somewhere the user cannot see. This is a narrower rule than behavior 10's: copy reads through a collapsed region because a selection can legitimately span it, while focus and the caret are positions the user must be able to see to use.
