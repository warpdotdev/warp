...
8. Malformed input degrades deterministically, never panics, and never drops content:
   ...
   - A stray `</details>` or `<summary>` outside a details block renders as plain text (current behavior).
...
11. Non-interactive surfaces that render markdown previews (where click/focus interaction is unavailable) render the block expanded, in source order (summary first, then body), with no disclosure indicator. The specific surfaces are enumerated in the tech spec.
