...
`summary` is parsed inline markdown, carrying the literal label `Details` when absent (product behavior 12(e)). `body` is recursively parsed markdown. There is no verbatim-source field: serialization is canonical re-serialization from parsed structure, matching the rest of the rich-text pipeline. `PartialEq` derives over all fields, so streaming delta comparison compares parsed structure.
...
**Signature change.** `parse_markdown_internal` (`markdown_parser.rs:141-144`) gains a parse-state parameter carrying the current nesting depth and the document-wide widget counter, since it holds no context object today. The public wrappers (`markdown_parser.rs:111-134`) construct the initial state, so their signatures are unchanged.

- **Line-start matching** (product behavior 12(c)): optional leading whitespace, then `<details` matched case-insensitively (product behavior 5), optional attributes with `open` recognized by presence alone regardless of any value (product behavior 4) and the rest ignored, optional whitespace, then `>`. A tag preceded by any non-whitespace character on its line, including a backtick, is not matched and falls through to `parse_paragraph`. This is what makes an inline code span containing the tag inert (product behavior 13) with no separate rule. A `/` before the `>` is accepted and produces a region with no distinct closing tag, which then degrades under product behavior 12(d).
- **Fence-aware delimiting** (product behavior 13): the matching `</details>` is not found by raw text scan. The delimiter walks lines, toggling an in-fence flag using the **backtick-fence recognition the parser actually implements** — an opening ` ``` ` after indentation, closed by a fence at the opening indentation or up to three columns deeper, mirroring `parse_code_block`/`parse_closing_fence` (`markdown_parser.rs:888-903`/`:962-977`). There is no `~~~` fence in this parser, so the delimiter recognizes none either. Tags are recognized only on non-fenced lines at line start, so tags on fenced lines never move the balance counter. A fence that never closes leaves the flag set through end of input, so no later `</details>` is recognized and the region degrades under product behavior 12(a) — product behavior 14, which follows from the flag rather than from a special case.
- **Summary extraction** happens on that same fence-aware walk, so a `<summary>` inside a body fence is likewise inert. Only a `<summary>` opening the body is the summary; any later one is body text (product behavior 12(f)).
- **The summary run is scanned as inline text, not blocks.** Between `<summary>` and its `</summary>` the walk looks only for `</summary>`: fence lines inside the summary do not toggle the in-fence flag, and nested `<details>` or `<summary>` tags there open nothing. This is what makes product behavior 12(i) hold — a fence in a summary is literal inline text rather than a region that swallows the rest of the body — and it is why the fence flag governs body lines only. The summary's own text is then parsed with the inline parser, which collapses its line breaks to a single line.
...
- **Limits** (product behaviors 10 and 11): `const MAX_DETAILS_DEPTH: usize = 8;` and `const MAX_DETAILS_PER_DOC: usize = 512;`. On exceeding either, the branch returns `nom::Err::Error`, so the input deterministically falls through to `parse_paragraph` and renders as literal text. There is no panic path, and recursion is bounded by the depth constant. The now-unmatched `</details>` is later recognized at its own depth and renders as literal text under product behavior 12(b).
- **Counter placement.** `alt` backtracks, so a `parse_details` attempt can begin and then fail. The widget counter is therefore incremented **only on successful completion of a region**, after its body parse returns, never on entry. A backtracked attempt leaves the counter untouched, which is what makes product behavior 11 deterministic and makes a depth-capped fallback consume no slot. Depth needs no such care: it is a value in the parse state passed down by value, so it unwinds with the call.
- **Unclosed input** (product behavior 12(a)): the rest of the current parse input becomes the body. Because parsing re-runs on streaming updates, a still-arriving block renders progressively (product behavior 17). `compute_formatted_text_delta` compares lines positionally, so growth inside the details line changes only that line's own `num_lines` and content; the lines above it compare equal and keep their prefix identity.
- **Block-level scope.** The line-start rule means a `<details>` indented under a list item matches as its own block and therefore terminates the list rather than nesting inside it, and a blockquoted `> <details>` never matches. This is the parser consequence of the product spec's second open question.

### 3. Imported-HTML path

In `crates/markdown_parser/src/html_parser.rs`, handle `details` and `summary` elements explicitly, building the same `FormattedDetails` from the DOM that html5ever already provides. No new scanning is needed, and html5ever normalizes tag case and attribute presence, so product behaviors 4 and 5 hold there without extra work. The depth and count constants from §2 apply identically.
...
**Depth beyond the cap in the buffer.** Parsing enforces `MAX_DETAILS_DEPTH` (§2), but editing can nest markers deeper than 8 without going through the parser. The buffer does not reject those edits — rejecting an edit mid-typing would violate product behavior 18 — so the cap is re-applied at the buffer's read boundary: `to_formatted_text` emits a `Details` container only while the `DetailsDepth` at that offset is below the cap, and serializes deeper regions' markers as literal `<details>`/`</details>` text. A depth-9 region therefore renders and round-trips exactly as a parsed depth-9 region does, so product behavior 10 holds identically whether a region arrived through the parser or through editing.
...
**Edit rebalancing** (product behavior 18). All of these fall out of the pairing scan rather than stored state, so no edit sequence yields an undefined document, exactly as unbalanced link markers degrade rather than corrupt:

- An unmatched `Start` owns the range to end of buffer — the product behavior 12(a) rule.
- An unmatched `End` is inert and is skipped on serialization — the product behavior 12(b) rule.
- A `Start` with no `EndSummary` before its `End` has an empty summary, with the whole run as body — the product behavior 12(g) rule, applied to the buffer.
- A stray `EndSummary` with no open `Start` is inert, like a stray `End`.
...
1. **`FormattedTextElement`** (`crates/warpui_core/src/elements/gui/formatted_text_element.rs:145`) gains layout for the `Details` variant: summary row, then body lines, expanded, with no disclosure indicator — the product behavior 16 fallback. This covers modals, banners, changelog entries, and settings pages. The interactive widget is an opt-in builder method on this element, consistent with its existing `with_*`/`disable_mouse_interaction` API (`:521`), and is enabled only by the agent conversation path (`app/src/ai/agent/util.rs:35`/`:189`).
...
- **Rejecting over-deep edits in the buffer** (rejected): blocking an edit because it would nest past the cap makes typing order determine document validity and conflicts with product behavior 18. Applying the cap at the read boundary instead keeps every edit legal and every render defined.
...
- **Depth 8 and count 512**: these bound what each widget adds beyond its content — a level of parser recursion, and a focusable element with accessibility-tree and event bookkeeping — not content size. The cost of nesting content inside `<details>` is otherwise comparable to tracking the same content as plain text. GFM content in the wild rarely nests past 3, so 8 is generous. Both are constants, so behavior is reproducible rather than load-dependent.
...
| 9, 10 | nested `<details>` at depth ≤ 8 parses as nested `Details`; depth 9 falls back to literal text while depth 8 still renders, with `raw_text` identical either way; the orphaned `</details>` renders as literal text |
| 11 | the 512th widget renders and the 513th falls back; a document whose `parse_details` attempts backtrack before the limit still renders its 512th widget, proving the counter tracks completions rather than attempts; repeated parses of the same input agree |
| 12(a) | an unclosed `<details>` consumes to end of input; an unclosed nested one ends at its parent's close |
| 12(b), 12(h) | a stray `</details>` and a stray `</summary>` render as literal text |
| 12(c) | a mid-line `<details>` stays plain text; a line that is exactly `` `<details>` `` in backticks stays plain text |
| 12(d) | a self-closing `<details/>` opens a region that runs to end of input |
| 12(e) | a body with no `<summary>` yields the literal `Details` label |
| 12(f) | a `<summary>` after body content renders as literal text; with two leading `<summary>` elements the first is the summary and the second is literal body text |
| 12(g) | an unclosed `<summary>` consumes the rest of the body as summary, leaving the body empty, with `raw_text` preserved; an unclosed `<summary>` inside an unclosed `<details>` consumes the remainder of the document into a single summary row |
| 12(i) | a code fence, a nested `<details>`, and a nested `<summary>` inside a summary render as literal inline text; a multi-line summary collapses to one line |
| 13 | `<details>`, `</details>`, and `<summary>` inside a fenced code block in the body leave the balance unchanged and appear verbatim in the rendered code block |
| 14 | a code fence opened in the body and never closed swallows a later `</details>`, so the region runs to end of input |
...
Round-trip coverage (canonical, not byte-exact): parse → buffer → `to_markdown` → parse yields an equal `FormattedText` by structural equality, with `open` preserved, over a nested region and a body containing code fences.
...
| depth boundary (10) | markers edited to depth 9 are accepted by the buffer, and `to_formatted_text` emits the depth-9 region as literal `<details>` text rather than a container, matching the parser's depth-9 output |
| rebalancing (18) | deleting `End` extends the region to end of buffer; an orphaned `End` or `EndSummary` is inert and unserialized; a `Start` with no `EndSummary` yields an empty summary with the run as body; results agree across edit orders |
