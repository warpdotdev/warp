...
- **The marker**: add `BufferText::Details(DetailsMarker)` (text.rs:541), mirroring `Link(LinkMarker)`:
  ...
  Like `LinkMarker`, the payload rides on the `Start` case and the marker is **zero-width in the char stream**: it is added to the zero-width arm of `Item::summary()` (text.rs:1817, next to `Marker`/`Link`/`Color`) and left in the skipped `_ => ()` arm of the `Bytes` iterator (text.rs:301). It occupies no `\n`. The summary text lives on the marker (the maintainer's "block marker metadata is the summary text"), not as body line-text — so editing the summary is a marker-data edit surfaced by the disclosure widget, the same shape as editing a link's URL, rather than inline character editing. (This resolves the summary-placement question I flagged last round in favor of marker metadata.)

- **Region and body**: a details region is the range between a `Start` and its matching `End`. The body is whatever blocks fall in that range — **each keeping its own `BufferBlockStyle`** (paragraph, code block, list, table). Nesting is just nested `Start`/`End` pairs. Because details is orthogonal to block style, a renderer can consult the `SumTree` for *both* "am I inside a details span, and at what depth" *and* "what is this block's own style," and adjust per-block styling (colors/fonts) for details content — the capability the block-style model foreclosed.
...
- **Round-trip** (`crates/editor/src/content/markdown.rs`): `to_markdown` (lines ~61-160) emits `<details>` (` open` iff `default_open`) + `<summary>…</summary>` from the `Start` marker payload, serializes the body blocks through their normal arms, and emits `</details>` at the matching `End` — canonical re-serialization, no verbatim source (product invariant 10). `to_formatted_text` (lines ~416-520) does the inverse, folding the blocks spanned by a marker pair into a `FormattedTextLine::Details` container (recursively for nested pairs); `core.rs` gains parse arms alongside the list arms (core.rs:~742/781) that flatten a `FormattedTextLine::Details` into `Start` marker + body blocks + `End` marker on the way in.

- **Render model**: the render `SumTree` gains a details-span dimension consulted per block (depth + open state) plus a `render/element/details.rs` that paints the summary line and disclosure triangle above the first body block of each span (the summary is drawn from marker data, not a buffer line). Focus, click, Enter/Space toggle, and renderer-generated accessibility IDs live here.
...
- **Start/End marker lines in the parser IR** (rejected at that layer only): `FormattedText` consumers are stateless per-line renderers with no edit machinery to maintain balance, so the IR keeps container-as-line. The buffer, which *does* maintain paired zero-width span markers (links) and owns edit semantics, uses the paired-marker representation — the split in §1/§4.
...
| Behavior | Test |
|---|---|
| pairing | flattening `FormattedTextLine::Details` produces a `Details(Start{..})` marker + body blocks (each with its own `BufferBlockStyle`) + `Details(End)`; nested pairs increment the details counter; depth-at-offset via the `DetailsDepth` `SumTree` dimension matches the pairing scan |
...
| zero-width | `Details` markers contribute 0 chars/bytes/lines to `TextSummary` and emit nothing from the `Bytes` iterator; a details span adds no `\n` to the char stream |
| rebalancing | deleting the `Details(End)` marker → span extends to end of buffer (8a semantics); an orphaned `Details(End)` is inert and not serialized (8b semantics); both deterministic across edit orders |
...
| editing | `convert_block` to/from a details span inserts/removes the marker pair and preserves body blocks; undo history unaffected by fold/unfold toggles |
