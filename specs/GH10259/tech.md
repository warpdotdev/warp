...
- An unmatched `Start` owns the range to end of buffer — the product behavior 11(a) rule.
- An unmatched `End` is inert and is skipped on serialization — the product behavior 11(b) rule.
- A `Start` with no `EndSummary` before its `End` has an empty summary, with the whole run as body — the product behavior 11(g) rule, applied to the buffer.
...
| 9, 10 | nested `<details>` at depth ≤ 64 parses as nested `Details`; depth 65 falls back to literal text while depth 64 still renders, with `raw_text` identical either way; the orphaned `</details>` renders as literal text |
| 11(a) | an unclosed `<details>` consumes to end of input; an unclosed nested one ends at its parent's close |
| 11(b), 11(h) | a stray `</details>` and a stray `</summary>` render as literal text |
...
| 11(g) | an unclosed `<summary>` consumes the rest of the body as summary, leaving the body empty, with `raw_text` preserved; an unclosed `<summary>` inside an unclosed `<details>` consumes the remainder of the document into a single summary row |
...
| depth boundary (10, 18) | markers edited to depth 65 are accepted by the buffer with no edit rejected, and `to_formatted_text` emits the depth-65 region as literal `<details>` text rather than a container, matching the parser's depth-65 output |
