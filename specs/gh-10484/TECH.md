...
- Changes and history are independently scrolling regions separated by a draggable vertical splitter using warpui `Resizable` with a bottom drag bar. Each region has a 120px minimum height, and double-clicking the splitter resets it to half of the measured split area. The splitter is hidden while HISTORY is collapsed or before a snapshot loads.
