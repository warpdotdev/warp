...
When Warp is pinned to the Windows taskbar, right-clicking its taskbar icon shows the user's saved Tab Configs as jump list entries, plus a "New Window" task. Selecting a Tab Config opens it in Warp (as a new tab, or a new window), mirroring the existing `warp://tab_config/<name>` deeplink behavior. The "New Window" task opens a fresh Warp window. This brings Warp to parity with Windows Terminal, which exposes its profiles via the taskbar jump list.
...
2. Selecting "New Window" opens a fresh Warp window at the default cwd (home directory), matching the existing `warp://action/new_window` deeplink behavior.

3. Each entry under "Tab Configs" is labeled with the Tab Config's display `name`.

4. Selecting a Tab Config entry opens the corresponding config via the existing `warp://tab_config/<file-stem>` deeplink path: as a new tab in the active window if Warp is running, or in a new window if Warp is not running.
...
11. When the `WindowsJumpList` feature flag is disabled, no jump list entries are populated and no AUMID is set by this feature; existing behavior is unchanged.
...
14. Windows usually allows up to 12 items per category in jump lists. When the number of saved Tab Configs exceeds that, only the capped subset appears. The subset is deterministic, ordered alphabetically by file stem to match the `+` menu source order.
