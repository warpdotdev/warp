...
When Warp is pinned to the Windows taskbar, right-clicking its taskbar icon shows the user's saved Tab Configs as jump list entries, plus a "New Window" task. Selecting a Tab Config opens it in Warp (as a new tab, or a new window), mirroring the existing `warp://tab_config/<name>` deeplink behavior. The "New Window" task opens a fresh Warp window at the home directory. This brings Warp to parity with Windows Terminal, which exposes its profiles via the taskbar jump list.
...
2. Selecting "New Window" opens a fresh Warp window at the default cwd (home directory), via `warp://action/new_window?path=~`.
...
11. When the `WindowsJumpList` feature flag is disabled, no jump list entries are populated; existing behavior is unchanged. The process AUMID is set independently by the existing app builder initialization and is unaffected by this flag.
