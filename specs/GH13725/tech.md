...
- **(ii) `<a id>`/`<a name>` targets: SMALL.** No new content-model field. Resolution reads **buffer source text** — the same access pattern the heading path already uses — so an anchor's `id` is visible to it whether or not the parser tokenized the tag. That covers both the bare `<a id="x">` form (which never matches the `href`-requiring grammar and stays literal) and the dual-role `<a href="…" id="…">` form (whose non-`href` attributes the parser discards). See item 3 for the mechanism, including code exclusion and attribute-order independence. The cost of this choice is that a bare tag stays visible (product invariant 5); hiding it needs a save-round-trippable representation and is deferred to #13982.
...
**The slug algorithm preserves Unicode letters and strips Unicode punctuation.** GitHub's heading slugger is not part of `cmark-gfm` — that library emits no heading ids — it lives downstream in GitHub's rendering pipeline, whose behavior the widely-used `github-slugger` reproduces. It keeps non-ASCII letters and removes punctuation by Unicode general category. An ASCII-only `[a-z0-9 -]` filter would silently break every non-English README anchor — a real regression against product invariant 4, not a cosmetic gap. The compatibility target is GitHub parity in both directions: letters survive in every script, and punctuation is removed in every script. This is the single authoritative statement of the algorithm; everything else in this spec refers back to it:

1. Lowercase the input with Unicode-aware lowercasing (Rust's `str::to_lowercase`, not `to_ascii_lowercase`).
2. Remove every character in a Unicode punctuation or symbol general category (`P*` and `S*`), except the literal ASCII hyphen `-`, which is preserved. The scope is Unicode-category-based, not an ASCII range: non-ASCII punctuation is removed on the same footing as ASCII punctuation, so `。` (U+3002), `？` (U+FF1F), `¿` (U+00BF), and every non-hyphen dash in `Pd` (em dash, en dash, fullwidth `－`) are all stripped. Unicode letters, digits, and marks are left untouched in every script. This is the one place a naive ASCII-range reading goes wrong, and it is what GitHub does: `github-slugger`, whose character class is generated from the Unicode category tables and was calibrated against GitHub's live rendered output, strips these categories, and GitHub's own `html-pipeline` `TableOfContentsFilter` used the Unicode-aware `/[^\p{Word}\- ]/u` for the same purpose.
...
**The scan reads buffer source text, not parser output.** This is the load-bearing fact for everything below, so it is stated once here and referred to elsewhere: the resolver reads the document's *authored characters* out of the buffer — the same access pattern `find_matching_header` already uses to pull heading text via `content.text_in_range(...)` (`app/src/notebooks/editor/model.rs:1371-1373`) — not a parsed anchor node. Whatever the inline parser did or did not tokenize, the buffer still holds what the author typed.

Because the `<a href>` grammar from item 1 requires an `href` attribute to match, a bare `<a id="x"></a>` never becomes an anchor token. It falls through to plain text, so its raw markup is present in the buffer both as source *and* as rendered content. Resolution scans that text for anchor-tag markup and extracts the `id`/`name` value, in the same click-time pass that walks headings.

**Dual-role `<a href="…" id="…">` tags must also register.** Product invariant 11 requires a tag carrying both roles to render as a link *and* register a jump target. That case needs explicit handling, because the two roles are consumed by different mechanisms and a naive implementation drops the target half:

- The tag matches item 1's `<a href>` grammar, so the parser consumes the whole opening tag and keeps only `href`. Every other attribute — including `id`/`name` — is discarded per invariant 8's parse-and-ignore rule, so the id is absent from the *rendered* fragment text.
- A scan pattern anchored on `id`/`name` appearing immediately after `<a` therefore sees a dual-role tag only when the author happens to write the id first (`<a id="x" href="#foo">`) and misses it when the href comes first (`<a href="#foo" id="x">`) — an attribute-order dependence that is invisible in testing unless both orders are exercised.

Two consequences the implementation must honor. First, the scan must run over **buffer source text**, where the full opening tag survives regardless of what the parser retained. Second, the anchor-tag pattern must be **attribute-order independent**: it matches an opening `<a …>` tag and extracts `id` or `name` from anywhere in its attribute list, rather than requiring that attribute to come first. A pattern that binds `id`/`name` directly to `<a` satisfies the bare-anchor case and silently fails half the dual-role cases, so the test suite pins both attribute orders (see Testing).

This preserves invariant 11 without a content-model change: the `href` role is carried by the parser, the target role by the source-text scan, and the two are independent.
...
Inline code spans need separate treatment, because they live *within* a block rather than as their own block: a span is a fragment-level style (`inline_code`, `crates/markdown_parser/src/lib.rs:550`), not a block type. Within a non-code block, an anchor-tag match must therefore be rejected when it falls inside an inline-code span. The implementer picks the concrete test — checking the matched range against the block's inline-code fragment ranges, or scanning fragment-wise and skipping `inline_code` fragments — but the requirement is fixed: a match inside a code span does not register a target.

This is the one place where the scan needs structural information rather than raw text, and it is why the scan is specified as a per-block walk rather than a single document-wide pass.
...
- An `<a>` carrying both `href` and `id` renders as a link and does not panic (invariant 11), asserted in **both attribute orders** (`href` first and `id` first) since the parser keeps only `href` and discards the rest.
...
- Non-ASCII **punctuation** removal, the other half of the rule and the half a letters-only table misses: `"日本語です。"` → `日本語です` (ideographic full stop U+3002 removed); `"Café？"` → `café` (fullwidth question mark U+FF1F removed, matching ASCII `?`); `"Español ¿Qué?"` → `español-qué`; and `"Ma—Dash"` → `madash` (em dash removed entirely, **not** converted to `-`, while a literal ASCII `-` in `"Well-Known"` → `well-known` survives). These fail if the exclusion set is written as an ASCII range.
...
- **Dual-role tags register a target in both attribute orders** (invariant 11): `<a href="#other" id="x">text</a>` and `<a id="x" href="#other">text</a>` each make `#x` resolve to that tag. The href-first case is the one an id-anchored scan pattern silently misses, so both orders are required rather than representative.
