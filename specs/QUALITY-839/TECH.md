...
- One new field on `ConversationQueueState`: `queue_next_lrc_prompt_override: Option<bool>` — a manual toggle made during an eligible agent-requested LRC. It is explicitly cleared when the command ends: `TerminalView` calls `clear_queue_next_lrc_prompt_override` on `CLISubagentEvent::FinishedSubagent` (PRODUCT §14-15, §22). Also dropped with the conversation's queue state.
...
- `app/src/terminal/input_tests.rs` (host-level, next to the existing queue host tests): with the active block's agent in control for an agent-requested command and the default settings, a non-empty AI submission queues instead of submitting, with `LrcAutoQueue` origin (§6); user-tagged LRCs do not auto-queue (§2); ghost text returns the queue hint (§12); "Send immediately" → submission routes as today (§19, §21); Queue default mode → row queues with the generic origin (§5); `send_lrc_queued_prompts` fires `LrcAutoQueue` rows in order and leaves other rows queued (§9).
