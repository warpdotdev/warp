...
- `Submit` and `PopFromEditMode` arms unchanged.

The non-clean arm (`Error|Cancelled`) is unchanged: `pop_front` + `replace_buffer_content(query.text())` already restores a command row's text as plain editor text with no mode change (PRODUCT §17).
...
- **§17 (pause)** — error/cancel with a command head restores its text into the empty editor and leaves the rest intact.
