...
Add `image_preview: Option<ImagePreviewData>` to `FileDiffAndContent` (it already carries the "expensive, do-not-clone" payload, so this is the natural home; `Arc` the bytes so the view can register them without a deep copy). `image_preview` is `Some` whenever at least one applicable side is `Image`, `TooLarge`, or `Unavailable`. A file whose every applicable side is **rejected** (or non-image) has `image_preview = None` → generic binary placeholder (§§6/8), never an "unavailable" note. For a *modified* file with one valid `Image` side and one rejected side, `image_preview` is `Some` (the valid side renders); the rejected side yields no note.
...
- `Unavailable` → a **required**, side-specific note ("other side unavailable"), satisfying product spec §7.

The generic `is_binary` text placeholder is reached only when the file has **no applicable previewable side at all** (i.e. `image_preview` is `None`) — e.g. a binary file that isn't a supported raster image. This keeps the graceful-fallback invariants without new error UI while guaranteeing the too-large/unavailable states are always surfaced per side.
