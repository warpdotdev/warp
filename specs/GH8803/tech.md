...
- `crates/lsp/src/descriptor/parse.rs` — serde-driven TOML parsing. Accepts the `[[editor.language_servers]]` array. Bare-string `filetypes` entries are normalized into `LspFiletypePattern { matcher, explicit_language_id: None }` during deserialize. Classification between `Glob` and `LiteralBasename` is by metacharacter presence (`*`, `?`, `[`).
