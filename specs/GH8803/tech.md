...
- `crates/warp_core/src/paths.rs` — `pub fn lsp_server_cache_dir(server_name: &str) -> PathBuf` returning `cache_dir().join("lsp").join(<hashed-name>)`, where `<hashed-name>` is the same shape as `workspace_hash` applied to `server_name` (first 8 bytes of SHA-256, 16 lowercase hex chars). Hashing the path component decouples the cache-dir filesystem segment from `name`'s allowed character set — eliminates platform-specific edge cases like Windows-reserved device names (`CON`, `NUL`, `COM1`, etc.), trailing-dot stripping, and extreme-length issues, regardless of what `name` validation lets through. The descriptor's `name` remains the user-visible identifier in the footer and in Warp's own logs. What the LSP server itself receives via `{{cache_dir}}` substitution is a path that ends in the hashed form (e.g. `~/Library/Caches/dev.warp.Warp-Stable/lsp/8a3f...c0e1/`), not the literal `name`; the LSP never sees `name` directly. Hash is deterministic, so the same `name` always resolves to the same cache directory across launches.
...
Built-in rows: `kind = 'BuiltIn'`, `language_server_name` = serialized `LSPServerType` variant name (`"RustAnalyzer"`, `"GoPls"`, etc.). Custom rows: `kind = 'Custom'`, `language_server_name = descriptor.name` (e.g. `"ruby-lsp"`). The `(workspace_id, kind, language_server_name)` triple is the effective key — built-in and custom rows occupy disjoint subspaces, so a custom can carry the same string as a built-in variant name without an on-disk conflict. Per invariant 23, the validator separately reserves the five built-in **binary display names** (`rust-analyzer`, `gopls`, `pyright-langserver`, `typescript-language-server`, `clangd`, case-insensitively) from custom `name` — not for SQLite safety (the `kind` column handles that) and not for cache-dir safety (the path is hashed), but solely to prevent footer-label ambiguity (a built-in and a custom both rendering "Enable rust-analyzer" in the same workspace).
...
`LspServerDescriptor` derives `Serialize`, `Deserialize`, and `JsonSchema`. Its fields are the user-facing field set: `name: String`, `command: String`, `args: Vec<String>` (with `#[serde(default)]`), `filetypes: Vec<LspFiletypePattern>` (non-empty), `env: BTreeMap<String, String>` (with `#[serde(default)]`), and `initialization_options: Option<Value>`. Each field carries a `///` doc comment that schemars derives into the JSON Schema `description` (per invariant 25); the doc comments are the only place placeholder semantics are documented for external editor tooling.

`LspFiletypePattern` derives the same three traits but has one private, runtime-only field that needs serde-side handling. The user-typed fields are `pattern: String` and `language_id: Option<String>`. The private compiled matcher (`globset::GlobMatcher`) is **not** user-typed; it is computed at parse time from `pattern`. For serde, the matcher field is annotated with both `#[serde(skip, default = "placeholder_matcher")]` (skip means the derived `Deserialize` impl does not look for a `matcher` key in the input JSON; the `default = "..."` is required because `globset::GlobMatcher` does not implement `Default` — the function returns a sentinel matcher that will never match any real filename, used only on the impossible path where serde is constructing one of these without going through `parse::compile_pattern`). For schemars, the field is annotated with `#[schemars(skip)]` so the matcher is absent from the generated JSON Schema (the user-facing schema describes only `pattern` and `language_id`). This is the codebase convention — see `app/src/settings/privacy.rs::CustomSecretRegex::pattern: Regex`, which uses `#[schemars(with = "String")]` for the same pattern of "private compiled artifact, user-typed source string" applied to a regex.
