...
4. `rc.elv` is sourced exactly once per session, from wherever Elvish itself would load it (including a custom `XDG_CONFIG_HOME`). A missing `rc.elv` is not an error. An `rc.elv` that throws shows its error in the first block, but the session still bootstraps. Functions, variables and `use`d modules defined there are callable from commands typed in Warp.
...
9. Environment variables applied through Warp (session env, "Run with env vars", Warp Drive env var collections) are rendered in Elvish syntax (`set-env FOO 'bar'; cmd`), never POSIX `FOO=bar cmd` or `$(…)`. This holds for all three value kinds: constants (including values containing `'` or `$`), command-backed values and secret-backed values. Each resolves to the same string it would in bash/zsh, including trailing newlines being stripped from command output. As with fish, "Run with env vars" sets the variables for the rest of the session, not just the one command.
10. Commands Warp chains on the user's behalf keep their meaning in Elvish. A step that should only run if the previous one succeeded is skipped when that one fails (a failing command throws and aborts the rest of the line). A step that should run regardless still runs after a failure (the earlier step is wrapped in `try { … } catch { }`).
11. Context chips that run a shell command (git branch, cwd, node version, etc.) work in Elvish sessions.
12. If Elvish is older than the minimum supported version (0.19, the first release with the `runtime:` module Warp uses to find the rc file the way Elvish itself does), Warp does not attempt to bootstrap it and falls back to another supported shell, the same way an unsupported login shell is handled today, and logs the `UnsupportedShell` telemetry event with the version.
