...
- **A one-shot recalculation trigger.** The existing recalculation triggers assume content settles via replacement; delta-seeded content settles via an ordinary edit, so neither fires. `hide_lines_outside_of_active_diff` now records `pending_hidden_lines_recalculation: Some(buffer_version)`; the diff-updated handler recalculates when a diff computes for that version or later, then clears it. Version-gated so an in-flight compute of stale content (the seeding reset's empty diff) can't consume the request early; one-shot so per-keystroke GUI behavior is untouched. In existing GUI flows it is a no-op, absorbed by the recalculation `ContentReplaced` already performs.
