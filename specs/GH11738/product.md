...
Make text in every Tools pane tab follow Warp's existing **Font size (px)** preference instead of adding a Project Explorer-only font-size setting. The Project Explorer, Conversation History, Global Search, and Warp Drive update together when the existing value changes.
...
- Keep one source of truth for terminal and Tools pane text size.
...
- Project Explorer file and directory names, the inline create/rename editor, drag previews, and in-pane unavailable/error heading and body use the selected primary size. The error heading keeps its semibold distinction.
...
Secondary, heading, and overline text scale proportionally from the selected primary size. At a 14 px primary size, the existing 14 px primary, 12 px secondary, 16 px heading, and 11 px overline relationships remain unchanged.
...
3. Primary Tools pane content uses the selected whole-pixel value in the existing inclusive `1..=120` range. Secondary, heading, and overline roles are positive proportional derivatives that preserve `heading > primary > secondary > overline`, including at 1 and 120 px.
...
9. Conversation History query, item title, zero-state, active rename, timestamp/subtext, action, and section-label text all derive from the same primary value while retaining their current hierarchy.
10. Warp Drive index rows and tab-local primary, secondary, heading, warning, and inline title text all derive from the same primary value. Separate dialogs and full panes launched from Drive are outside this behavior.
...
14. Terminal font size and Tools pane text change from the same saved value. Notebook font size remains governed by its current setting, and changing the Tools pane behavior does not rewrite notebook state.
...
- Change **Font size (px)** to exactly 1, the current 13 px default, 14, and exactly 120 and compare all four Tools pane tabs in the same window.
