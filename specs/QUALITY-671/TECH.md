...
## QUALITY-1702 addendum: live credit estimate for loaded remote children

The rollup gate itself (`compute_orchestration_rollup`) was always correct: it sums whatever `credits_spent()` a *loaded* descendant reports. The bug was upstream — a loaded remote-child placeholder (e.g. a factory subagent) could sit at `credits_spent() == 0` indefinitely, because the only path that ever wrote real numbers into its `conversation_usage_metadata` was cloud-transcript hydration (`BlocklistAIHistoryModel::hydrate_remote_child_placeholder_with_cloud_transcript`), which only runs once `decide_child_pane_materialization` / `decide_remote_child_hydration_action` picks `LoadTranscript` — i.e. the child's task is already terminal and a server token is known. A child that is still running, or one that attached live and then went terminal without a later re-drive, never got that write, so its row (and its share of the total) stayed invisible.

Fix: `AgentConversationsModel` already polls/fetches `AmbientAgentTask` data independent of pane materialization (`update_model_with_new_tasks`, `async_fetch_task`), and `AmbientAgentTask::credits_used()` (inference + compute + platform cost) is populated by the server on the task itself — continuously, not only at terminal state (it already backs the task detail panel and the agent-management list rows regardless of run state). Each time task data is fetched or refreshed, `AgentConversationsModel::apply_task_credit_estimate` resolves the task's `run_id` to a local conversation via `BlocklistAIHistoryModel::conversation_id_for_agent_id` and calls `BlocklistAIHistoryModel::apply_remote_child_task_credit_estimate`, which applies `AIConversation::apply_remote_child_task_credit_estimate` (advance-only: only takes effect for `is_remote_child()` conversations and only when the new figure exceeds the currently known total) and emits `ConversationUsageMetadataUpdated` so the footer re-renders. Cloud-transcript hydration remains authoritative and unconditionally overwrites this estimate once it lands.

Scope: this fixes the credits gap for any *loaded* remote-child placeholder, live or terminal. It does not change PRODUCT invariant 10's unloaded-descendant gate (a descendant absent from `conversations_by_id` is still silently skipped) — that remains a possible follow-up via a server-side rollup query.
