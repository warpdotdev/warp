...
## Context
This spec covers the `warp-4` client side of GitHub issue #8642. User-facing behavior is defined in `PRODUCT.md`. The server-side rename API, metadata persistence, `title_is_manual`, generated-title guards, task-title sync, and stored `ConversationData` updates are documented in `../warp-server-4/specs/gh-8642/TECH.md`.
...
- `app/src/ai/blocklist/history_model.rs` owns local conversation mutation, SQLite persistence, and `UpdatedConversationMetadata`.
...
4. Update local canonical title on success:
   ...
   - Emit `UpdatedConversationMetadata`.
   - Do not add or persist a separate client-side title override field.
5. Refresh all title-bearing client surfaces:
   - Pane title and tab title refresh through `TerminalView` handling of `UpdatedConversationMetadata`.
   - Vertical tabs and workspace chrome refresh from the same history event.
   - Command palette and conversation search read fresh `ConversationNavigationData`.
   - Conversation list and management panel rebuild on `ConversationUpdateKind::MetadataChanged`.
   - Task-backed rows in `AgentConversationsModel` update cached `AmbientAgentTask.title` for the renamed conversation so list and management surfaces do not wait for a poll.
...
2. Title validation: trim, empty rejection, Unicode scalar limit.
...
6. Conversation list and management panel refresh cached task-backed titles on `UpdatedConversationMetadata`.
7. Pane title refreshes immediately after `UpdatedConversationMetadata`.
Validation commands:
- `cargo fmt --all`.
- `cargo check -p warp`.
