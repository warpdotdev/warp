...
3. The yield state is durable across restart so a restored conversation does not regress to looking complete.
4. The yield state clears automatically when the next user input or inbound event resumes the agent.
...
4. Re-architecting `ConversationStatus` consumers beyond what is required to disambiguate `is_quiescent` from `is_terminal`.
...
10. The waiting state is durable: if the application restarts while a conversation is in `WaitingForEvents`, it restores as `WaitingForEvents`, not as `Success`.
...
22. The orchestrator pill bar badge is driven by `aggregated_orchestrator_status` over the orchestration tree. The aggregator's precedence is `InProgress > Blocked > WaitingForEvents > Error > Cancelled > Success`. Rationale:
    - `InProgress` wins because something is actively streaming, which is the most "present" state.
...
28. Any caller that today asks "is this conversation done?" should ask "is this conversation terminal?" and get `false` for `WaitingForEvents`. Any caller that asks "is the agent currently working?" should ask "is this conversation in_progress?" and also get `false`. New helpers `is_terminal()` and `is_quiescent()` make the distinction explicit; `is_done()` is retired or redefined to mean `is_terminal()`.
