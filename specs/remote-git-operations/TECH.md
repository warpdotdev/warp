...
- `GitCreatePrRequest { repo_path, title?, body?, branch, autogenerate_content }` → `GitCreatePrResponse { PrInfo | GitOpError }`. `branch` + `autogenerate_content` support daemon-side AI generation (§8); base is left to `gh` (no `base` field).
...
Also extend `Commit` with `repeated FileChangeEntry files` and add `FileChangeEntry { path, additions, deletions }` so per-commit file lists ride along (§5). Add `repeated FileChangeEntry files` to `DiffMetadataAgainstBase` too, so the dialog's "Changes" box renders for remote repos from synced metadata (§4). (An earlier `DiffMetadata.has_staged_changes` bit was dropped — proto field 9 is now `reserved` — in favor of the authoritative empty-commit guard in `run_commit` (§2), which rejects an empty staged set rather than pre-gating Confirm on a synced bit.)

Wire each into the envelopes in `crates/remote_server/proto/remote_server.proto`: the requests go into the `HostScopedRequest` oneof (field numbers 15–18) and the responses into the `ServerMessage` oneof (27–30). §8 adds the 5th request/response pair (19 / 31). `build.rs` already compiles both protos, so no codegen wiring changes.
...
- `app/src/code_review/diff_state/mod.rs`: `apply_git_op_delta` is the one unified seam (Local + Remote both implemented). The op dispatchers (`git_commit_chain` / `git_push` / `create_pr_remote` / `generate_commit_message` / `fetch_pr_info`) and `pr_info` are remote-only — their Local arms are no-ops, because local commit/push/create-PR run `util::git` directly from the dialog.

### 4. GitDialog + CodeReviewView wiring
- `GitDialog` drops `repo_path: PathBuf` and instead holds `repo_location: LocalOrRemotePath` + a `diff_state_model: ModelHandle<DiffStateModel>` (two fields, not a wrapper type). Each mode's `start_confirm` branches on `repo_location`: the Local arm runs `util::git` directly and then applies the post-op delta via `DiffStateModel::apply_git_op_delta` for an immediate header refresh; the Remote arm dispatches through the remote-only `DiffStateModel` methods (`git_commit_chain` / `git_push` / `create_pr_remote`) and completes on a model event.
- Reads come from already-synced state where possible: branch/base/`unpushed_commits` come from `DiffMetadata`, and per-commit file lists now ride on `Commit`, so the push dialog renders them for both local and remote without a fetch. The commit/PR "Changes" file list also rides along in synced metadata: `DiffMetadataAgainstBase.files` (per-file path + adds/dels) is captured from the same numstat that builds the aggregate stats, so `commit::refresh_remote_file_changes` / `pr::refresh_remote_file_changes` populate the box for remote repos without a working-tree read (local repos still read it directly). AI commit-message / PR-title/body generation for remote repos runs on the daemon (see §8), so the client never reconstructs the diff or calls AI itself.
...
- Completion: `GitDialog` subscribes to `DiffStateModel` and handles `GitOpCompleted` / `CommitMessageGenerated` in `handle_diff_state_event` (remote); the local arm finishes from its own spawn callback. Both backends share one completion helper per mode: `commit::finish_commit_chain` / `push::finish_push` / `pr::finish_create_pr` (toast + telemetry + close), so local and remote produce identical UX. The local commit callback collapses its `CommitOutcome` to the `Result<Option<PrInfo>, String>` shape `finish_commit_chain` shares with the remote arm.
...
New model: each op produces its post-op delta (§1), and the dialog applies it via `DiffStateModel::apply_git_op_delta`, which updates `metadata` and emits `MetadataRefreshed`. That emission drives `CodeReviewView`'s existing subscriptions → `update_git_operations_ui` → buttons/menu re-render. Local applies the delta in the dialog's spawn callback (after computing `compute_unpushed_state`); remote applies it in the manager-event handler before emitting `GitOpCompleted`. So:
...
The local dialog generates the open-time commit message and the PR title/body on the **client**: it reads the working tree via `get_diff_for_commit_message` / `get_diff_for_pr` and calls `AIClient::generate_code_review_content`. Remote repos have no local working tree, so this work moves to the daemon, which already holds an authenticated `ServerApiProvider` / `AIClient` — the user's bearer token is forwarded at `Initialize` / `Authenticate` (`apply_initialize_auth`), and the existing `UploadHandoffSnapshot` handler already calls warp-server from the daemon. Running AI there also keeps the (potentially large) diff on the remote host: only the resulting strings cross the SSH link, so this is one round trip with a small payload rather than shipping the raw diff to the client.
Two mechanisms:
- **Commit message** — new `GitGenerateCommitMessageRequest { repo_path, include_unstaged, branch_name }` → `GitGenerateCommitMessageResponse { message | error }` (host-scoped; request field 19, response field 31). At commit-dialog open, the remote arm of `GitDialog::new_for_commit` calls `commit::maybe_start_remote_commit_message_autogen`, which dispatches through `DiffStateModel::generate_commit_message` → `RemoteServerManager::git_generate_commit_message` → `HostRequestHandle::git_generate_commit_message`. The daemon's `handle_generate_git_commit_message` runs `get_diff_for_commit_message` then `generate_code_review_content(CommitMessage)` and returns the trimmed message. The result arrives as `RemoteServerManagerEvent::GenerateCommitMessageResponse`, which the remote model relays as the new `DiffStateModelEvent::CommitMessageGenerated`; `GitDialog::handle_diff_state_event` fills the editor via `commit::apply_generated_commit_message` (shared with the local path), handled outside the `loading` gate since generation happens before any op is initiated.
- **PR title/body** — `create_pr_with_ai_content` (bumped from `pub(super)` to `pub(crate)`) is the single AI-title/body-with-`--fill`-fallback helper, so local and remote PRs are produced identically. The standalone Create-PR dialog reaches it via `GitCreatePrRequest { autogenerate_content }` → `handle_create_pr`; the `COMMIT_AND_CREATE_PR` chain reaches it via `GitCommitChainRequest { autogenerate_pr_content }` → `handle_git_commit_chain`'s create-PR stage. Both run on the daemon, so the diff never crosses the SSH link.
...
```mermaid
flowchart LR
  Open["new_for_commit (remote)"] --> Gen["DiffStateModel::generate_commit_message"]
  Gen --> Mgr["manager.git_generate_commit_message -> client"]
  ...
  Ev --> CMG["DiffStateModelEvent::CommitMessageGenerated"]
