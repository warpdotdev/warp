...
- Add a `LifecycleEventType → ConversationStatus` mapping helper. Mostly 1:1 with the existing `conversation_status_from_state(AmbientAgentTaskState)` collapsing rules; the only call-out is `Blocked`, which today is emitted without a `blocked_action` payload (we accept empty, matching the REST path).
...
### Cursor consistency
The cursor is the only client-side state that has to survive across reconnects and across client restarts. Four points:
1. **Where it lives.** Reuse `AIConversation::last_event_sequence` on the viewer's local orchestrator-placeholder conversation. The streamer's existing `persist_event_cursor` writes to this field; the new viewer-mode code path can use the same plumbing. No new SQLite column needed. Each viewer pane keeps its own local copy — the cursor is per-pane local state, not server-observable.
2. **Server-side update is suppressed.** The existing `persist_event_cursor` ALSO pushes the cursor to the server via `update_event_sequence_on_server` so other clients of the same agent run can resume from the same point. For viewer-mode conversations this push must be suppressed: the viewer does not own the task, and writing to the server's task cursor would interfere with the orchestrator-owner's process resuming its own SSE. Gate the server-update branch on `!is_viewing_shared_session()`.
3. **`on_restored_conversations` already early-returns** for viewer-side conversations (`orchestration_event_streamer.rs:729-731` checks `if is_remote_view { continue; }` before any cursor write). So no additional gating is needed there — the change in (2) is the only place that needs new gating.
...
### Hotfix candidate
The kick-guard bug exists today and will keep wasting requests until PR 2 lands. If that turns out to be more than a few weeks, ship a small hotfix to `maybe_kick_polling` (track the fetch handle separately from the timer handle, and gate the kick on `in_flight_fetch.is_some()` rather than `polling_handle.is_none()`) ahead of PR 1. It does not affect the PR 1–2 design. PR 2 deletes the polling path so the hotfix evaporates naturally.
