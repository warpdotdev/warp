...
- Audit every comment (inline or doc comment) added or modified in the diff against the Comments guidance in `AGENTS.md`'s Development Guidelines: flag comments that narrate what/how the code does instead of explaining a non-obvious "why", line-by-line narrations, doc comments that enumerate callers instead of documenting behavior, and "transformation comments" that describe the edit itself rather than the code's current state. Also flag existing comments removed or altered without a change to the logic they describe. Treat `AGENTS.md` as the source of truth rather than restating its guidance in review comments.
