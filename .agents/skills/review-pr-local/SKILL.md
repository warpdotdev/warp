...
- When a PR adds or changes comments, audit them against the "Comments" guidance under "Development Guidelines" in `AGENTS.md`. Common issues to flag: comments that narrate WHAT/HOW instead of a non-obvious WHY (business rationale, third-party workarounds, complex algorithms, edge cases); line-by-line narrations restating the code; docstrings that narrate implementation steps or enumerate callers instead of documenting behavior; and "transformation comments" that describe the edit itself (e.g. "this used to do X") rather than the code's current state. Read the full list in `AGENTS.md` rather than relying on these examples alone — it also covers single-sourcing documentation and not removing existing comments in unrelated changes.
