...
- Audit every comment the PR adds or changes against the "Comments" guidance under "Development Guidelines" in `AGENTS.md`. Comments carry a maintenance cost, so a new comment has to earn its place. Walk the added and changed comments one at a time and check each against the named sub-rules individually (Minimalist Comments, Strictly "Why" Only, No Line-by-Line Narrations, Clean Docstrings, Single-source of documentation, Don't enumerate function call sites in doc comments, No "transformation comments") instead of forming a single overall impression. Recurring things to flag: a comment restating what the code already says rather than explaining a non-obvious *why*; a "transformation" comment describing the edit instead of the current state (e.g. "this used to ..."); a doc comment narrating a function's internal steps or listing its callers; an explanation at a call site or reference that the declaration's doc comment already covers; and an existing comment deleted as collateral of an otherwise unrelated change. Read the full list in `AGENTS.md` rather than relying on those examples alone. A comment being accurate, well-written, or about a subtle issue does not exempt it from the rules. Treat a confirmed violation as `⚠️ [IMPORTANT]`, not a nit.
