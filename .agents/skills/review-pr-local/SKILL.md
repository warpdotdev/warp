...
- Audit any comments a PR adds or changes against the "Comments" guidance under "Development Guidelines" in `AGENTS.md` — comments carry a maintenance cost, so a new or changed comment should earn its place. Check each one against every named sub-rule (Minimalist Comments, Strictly "Why" Only, No Line-by-Line Narrations, Clean Docstrings, Single-source of documentation, Don't enumerate function call sites, No "transformation comments") rather than forming one overall impression. Common issues to flag: a comment restating what the code already says instead of explaining non-obvious *why*; a "transformation" comment describing the edit rather than the current state (e.g. "this used to ..."); a doc comment that narrates a function's internal steps or enumerates its callers; an explanation duplicated at a call site that the declaration's doc comment already covers; and an existing comment removed as collateral of an otherwise unrelated change. Treat a confirmed violation as `⚠️ [IMPORTANT]`, not a nit.
