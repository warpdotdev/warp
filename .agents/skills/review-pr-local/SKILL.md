...
- Audit the comments a PR adds or changes against the Comments guidance in the Development Guidelines section of `AGENTS.md`, which reserves comments for non-obvious rationale: business reasons, third-party workarounds, complex algorithms, unidiomatic code, and surprising edge cases. Flag comments that narrate WHAT or HOW self-documenting code already says, line-by-line narration of syntax, doc comments that recite internal implementation steps or enumerate a function's callers, explanations repeated at call sites when the declaration already documents them, and "transformation" comments describing the author's edit ("this used to...") rather than the current state of the code. Also flag the inverse: unrelated existing comments the PR deleted, and comments not reflowed to the formatter's 100-column `max_width`.
