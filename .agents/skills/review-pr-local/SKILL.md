...
- When a PR adds or changes comments (inline comments or doc comments), audit them against the `**Comments**` sub-rules under Development Guidelines in `AGENTS.md` rather than restating that guidance wholesale. Common issues to flag: comments narrating WHAT/HOW when self-documenting names already make it obvious (violates Minimalist Comments); comments that aren't reserved for non-obvious business rationale, third-party workarounds, complex algorithms, unidiomatic code, or edge cases (violates Strictly "Why" Only); line-by-line narrations restating the syntax; doc comments that narrate internal implementation steps instead of documenting the public API, arguments, types, and returns (violates Clean Docstrings); an explanation repeated at a reference or call site instead of living once at the declaration, or a doc comment enumerating a function's callers (violates Single-source of documentation / Don't enumerate call sites); "transformation comments" that describe the edit itself (e.g. "this used to do so-and-so") instead of the code's current state; and existing comments removed or altered even though the logic they describe hasn't changed.
