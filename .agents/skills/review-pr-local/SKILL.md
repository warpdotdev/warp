...
- Audit every comment the PR adds or changes against the "Comments" guidance under "Development Guidelines" in `AGENTS.md`, which is the source of truth — comments carry a maintenance cost, so each one has to earn its place. Check each comment against the named sub-rules one by one (Minimalist Comments; Strictly "Why" Only; No Line-by-Line Narrations; Clean Docstrings; Single-source of documentation; don't enumerate call sites in doc comments; no "transformation comments"; don't drop existing comments as collateral) rather than forming a single overall impression of the comment block. A comment that is accurate, well written, or explains something genuinely subtle is still subject to every rule. Common things to flag: restating what the code already says instead of the non-obvious *why*; narrating the edit rather than the current state (e.g. "this used to ..."); doc comments that walk through internal implementation steps or list callers; an explanation duplicated at a call site or reference that the declaration's doc comment already covers; and an existing comment deleted by an otherwise unrelated change.
