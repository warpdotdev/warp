...
- Audit every comment the diff adds or changes against the "Comments" rules under "Development Guidelines" in `AGENTS.md`. Comments carry a maintenance cost, so each one has to earn its place: flag comments that narrate what the code already says, line-by-line narrations, doc comments that describe a function's internal steps or enumerate its callers, explanations duplicated at a call site that the declaration already documents, "transformation" comments that describe the edit rather than the current state (e.g. "this used to ..."), and existing comments deleted as collateral of an unrelated change. Also check that comments are reflowed to the 100-column `max_width` the formatter uses instead of wrapping early. Read the full list in `AGENTS.md` rather than relying on these examples.
