...
- Audit every comment the diff adds or changes against the "Comments" guidance in the "Development Guidelines" section of `AGENTS.md` (minimalist, why-only, no line-by-line narrations, clean docstrings, single-sourced documentation, no enumerating call sites in doc comments, no "transformation comments" describing the edit rather than the current state, and no incidental removal of existing comments). Flag violations rather than restating the full list; point the author at `AGENTS.md` as the source of truth.
