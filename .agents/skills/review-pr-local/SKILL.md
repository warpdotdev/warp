...
- Audit the comments a PR adds or changes against the "Comments" guidance under "Development Guidelines" in `AGENTS.md`; comments carry a maintenance cost, so a new comment has to earn its place. Common issues to flag: comments that restate what the code already says instead of explaining non-obvious *why*; line-by-line narration of the syntax; doc comments that narrate a function's internal steps or enumerate its callers; an explanation duplicated at a call site or reference that the declaration's doc comment already covers; "transformation" comments that describe the edit rather than the current state (e.g. "this used to ..."); existing comments removed as collateral of an otherwise unrelated change; and comments wrapped well before the formatter's 100-column `max_width` instead of reflowed to fill it.
