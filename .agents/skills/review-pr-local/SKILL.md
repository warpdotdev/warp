...
- Audit every comment (inline or doc comment) a PR adds or changes against the "Comments" guidance under "Development Guidelines" in `AGENTS.md` rather than skimming for typos; treat a confirmed violation as `⚠️ [IMPORTANT]`, not a nit. Watch for: comments that restate what the code already says instead of explaining non-obvious "why"; "transformation" comments that describe the edit (e.g. "this used to do X") rather than the code's current state; doc comments that narrate internal implementation steps or enumerate callers instead of documenting behavior; explanations duplicated at a call site when the declaration's own doc comment already covers them; and existing comments removed as collateral damage of an unrelated change.
