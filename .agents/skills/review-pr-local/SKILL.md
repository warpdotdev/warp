...
- Audit every comment the diff adds or changes against the "Comments" guidance under "Development Guidelines" in `AGENTS.md`. Comments carry a maintenance cost, so each new comment has to earn its place: flag comments that restate what the code already says instead of explaining a non-obvious *why*, line-by-line narrations of the syntax, doc comments that narrate a function's internal steps or enumerate its callers, explanations duplicated at a call site that the declaration's doc comment already covers, "transformation" comments that describe the edit rather than the current state (e.g. "this used to ..."), and existing comments deleted as collateral of an otherwise unrelated change. Read the full list in `AGENTS.md` rather than relying on these examples.
