...
- TUI caveat: for changes to the headless TUI (`crates/warp_tui` or the cell-grid element library at `crates/warpui_core/src/elements/tui`), acceptable "visual evidence" is a terminal transcript, a `render_to_lines` / `TuiBuffer::to_lines` snapshot diff, or a `./script/run-tui` capture — NOT a `computer_use` screenshot or real-display recording (those are for the GUI desktop app). See the `verify-tui-change` skill. Note also that the `MouseStateHandle` and other WarpUI GUI-render heuristics (e.g. the inline `MouseStateHandle::default()` rule above) do not apply to TUI element code, which has no pixel/mouse model.
